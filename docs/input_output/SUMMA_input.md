...
* <a id="forcing_file_dimensions">Forcing dimensions</a>: SUMMA expects the dimensions in the forcing NetCDF file to be defined as `(hru,time)`. Note that different programming languages interact with NetCDF dimensions in different ways. For example, a NetCDF file with dimensions `(hru,time)` generated by Python will be read by Fortran as having dimensions `(time,hru)`. A NetCDF file with dimensions `(hru,time)` generated by Matlab will be read by Fortran as having dimensions `(hru,time)`.
