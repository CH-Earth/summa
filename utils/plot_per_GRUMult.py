...
def run_loop(j,var,the_max):
    ...
    for i,(m,f_x,f_y) in enumerate(zip(method_name,f_x_mat,f_y_mat)):
        ...
        if stat == 'rmse' or stat == 'rmnz' or stat == 'mean': cbr.ax.set_ylabel(stat_word + ' [{}]'.format(leg_titl[j]), labelpad=40, rotation=270)
        if stat == 'maxe' or stat == 'amax': cbr.ax.set_ylabel(stat_word + ' [{}]'.format(leg_titlm[j]), labelpad=40, rotation=270)
