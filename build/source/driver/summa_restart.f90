...
contains
 ...
 do iGRU=1,nGRU
  ...
  ! Start with this full, since easier to spin up by draining than filling (filling we need to wait for precip). 
  ! There will be an initial pulse of water, but the spinup runs should be discarded.
  ...
  select case(model_decisions(iLookDECISIONS%spatial_gw)%iDecision)
   ...
   case(localColumn)
    ...
    do iHRU=1,gru_struc(iGRU)%hruCount
      progStruct%gru(iGRU)%hru(iHRU)%var(iLookPROG%scalarAquiferStorage)%dat(1) = 1._rkind
    end do
