...
contains
 ...
 real(summa_prec)                              :: notUsed_canopyDepth   ! NOT USED: canopy depth (m)
 real(summa_prec)                              :: notUsed_exposedVAI    ! NOT USED: exposed vegetation area index (m2 m-2)
 ...
 real(summa_prec),  allocatable                :: timeGRUcompleted(:)   ! time required to complete each GRU
 real(summa_prec),  allocatable                :: timeGRU(:)            ! time spent on each GRU
 ! ---------------------------------------------------------------------------------------
 ...
 do jGRU=1,nGRU
  totalFluxCalls(jGRU) = 0._summa_prec
 ...
 do jGRU=1,nGRU  ! loop through GRUs
  ...
  timeGRU(iGRU)          = real(openMPend - timeGRUstart(iGRU), kind(summa_prec))
