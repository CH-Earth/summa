  ...
  ! standard case (not time)
   ...
   select case(metaData(iVar)%varType)
    ...
    case(iLookvarType%scalarv)
     if(spatialDesire==needGRU .and. timeDesire==needTime) call cloneStruc(dimensionIDs, lowerBound=1, source=(/     gru_DimID,Timestep_DimID/), err=err, message=cmessage); chunksize=(/ gruChunk, int(timeChunk/gruChunk)+1 /)
     if(spatialDesire==needHRU .and. timeDesire==needTime) call cloneStruc(dimensionIDs, lowerBound=1, source=(/     hru_DimID,Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, int(timeChunk/hruChunk)+1 /)
     ...
     if(spatialDesire==  noHRU .and. timeDesire==  noTime) call cloneStruc(dimensionIDs, lowerBound=1, source=(/  scalar_DimID/) , err=err, message=cmessage);               chunksize=(/ hruChunk, int(timeChunk/hruChunk)+1 /)

    ! (other variables)
    case(iLookvarType%wLength); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, wLength_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%midSnow); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, midSnow_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%midSoil); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, midSoil_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%midToto); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, midToto_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%ifcSnow); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, ifcSnow_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%ifcSoil); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, ifcSoil_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
    case(iLookvarType%ifcToto); call cloneStruc(dimensionIDs, lowerBound=1, source=(/hru_DimID, ifcToto_DimID, Timestep_DimID/), err=err, message=cmessage); chunksize=(/ hruChunk, layerChunk, int(timeChunk/hruChunk)+1 /)
