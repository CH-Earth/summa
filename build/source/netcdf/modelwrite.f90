    ...
    if (meta(iVar)%varName.eq.'time') then    
     selecttype(stat)
      typeis (dlength)
       ...
       cycle
     class default
      err=20; message=trim(message)//'time variable must be of type dlength'; return; 
     endselect
    endif
    ...
    do iStat = 1,maxVarStat
     ...
     selectcase (meta(iVar)%varType)
      case (iLookVarType%scalarv)
       ...
       endselect  ! stat structure

      case(iLookVarType%wLength) 
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,1,oStep(iFreq)/),count=(/1,maxSpectral,1/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%midToto)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,midTotoStartIndex/),count=(/1,nLayers/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%midSnow)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,midSnowStartIndex/),count=(/1,nSnow/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%midSoil)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,midSoilStartIndex/),count=(/1,nSoil/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%ifcToto)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,ifcTotoStartIndex/),count=(/1,nLayers/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%ifcSnow)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,ifcSnowStartIndex/),count=(/1,nSnow/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect
      case(iLookVarType%ifcSoil)
       selecttype(dat)
        typeis (dlength)
         err = nf90_put_var(ncid(iFreq),meta(iVar)%ncVarID(iStat),(/dat(iVar)%dat/),start=(/iHRU,ifcSoilStartIndex/),count=(/1,nSoil/))
        class default; err=20; message=trim(message)//'dats must be dlength'; return
       endselect

      case default
       err=40; message=trim(message)//"unknownVariableType[name='"//trim(meta(iVar)%varName)//"'; &
                                     &type='"//trim(get_varTypeName(meta(iVar)%varType))//"']"; return
