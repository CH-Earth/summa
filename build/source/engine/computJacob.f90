...
contains
 ...
 real(rk),intent(in)               :: dBaseflow_dMatric(:,:) ! derivative in baseflow w.r.t. matric head (s-1)
 ! input-output: Jacobian and its diagonal
 real(rk),intent(inout)            :: dMat(:)         ! diagonal of the Jacobian matrix
 real(rk),intent(out)              :: aJac(:,:)       ! Jacobian matrix
 ! output variables
 ...
 ! conversion factors
 real(rk)                          :: convLiq2tot     ! factor to convert liquid water derivative to total water derivative
 ! --------------------------------------------------------------
 ...
 aJac(:,:) = 0._rk  ! analytical Jacobian matrix
 ...
 select case(ixMatrix)
  ...
  case(ixBandMatrix)
   ...
   if(computeVegFlux)then  ! (derivatives only defined when vegetation protrudes over the surface)
    ...
    if(ixVegHyd/=integerMissing) aJac(ixDiag,ixVegHyd) = -scalarFracLiqVeg*(dCanopyEvaporation_dCanLiq - scalarCanopyLiqDeriv)*dt + 1._rk     ! ixVegHyd: CORRECT
    ...
    if(ixVegHyd/=integerMissing)then
     ...
     if(ixVegNrg/=integerMissing) aJac(ixOffDiag(ixVegNrg,ixVegHyd),ixVegHyd) = (dt/canopyDepth)   *(-dCanopyNetFlux_dCanLiq) - (1._rk - scalarFracLiqVeg)*LH_fus/canopyDepth   ! dF/dLiq
   ...
   if(nSnowOnlyHyd>0)then
    do iLayer=1,nSnow  ! loop through layers in the snow domain
     ...
     select case( ixHydType(iLayer) )
      ...
      case default;         convLiq2tot = 1._rk
     end select
     ...
     ! - lower-diagonal elements
     if(iLayer > 1)then
      if(ixSnowOnlyHyd(iLayer-1)/=integerMissing) aJac(ixOffDiag(ixSnowOnlyHyd(iLayer-1),watState),watState) = 0._rk  ! sub-diagonal: no dependence on other layers
     endif
     ...
     if(nSnowOnlyNrg>0)then
      ...
      if(nrgstate/=integerMissing)then       ! (energy state for the current layer is within the state subset)
       ...
       aJac(ixOffDiag(nrgState,watState),watState) = -(1._rk - mLayerFracLiqSnow(iLayer))*LH_fus*iden_water     ! (dF/dLiq)
    ...
    do iLayer=1,nSoilOnlyHyd
     ...
     if(nrgstate/=integerMissing)then
      ...
      if(mLayerdTheta_dTk(jLayer) > verySmall)then  ! ice is present
       ...
       aJac(ixOffDiag(nrgState,watState),watState) = 0._rk
      endif
  ...
  case(ixFullMatrix)
   ...
   if(computeVegFlux)then  ! (derivatives only defined when vegetation protrudes over the surface)
    ...
    if(ixVegHyd/=integerMissing) aJac(ixVegHyd,ixVegHyd) = -scalarFracLiqVeg*(dCanopyEvaporation_dCanLiq - scalarCanopyLiqDeriv)*dt + 1._rk
    ...
    if(ixVegHyd/=integerMissing)then
     ...
     ! cross-derivative terms w.r.t. canopy liquid water (J m-1 kg-1)
     ! NOTE: dIce/dLiq = (1 - scalarFracLiqVeg); dIce*LH_fus/canopyDepth = J m-3; dLiq = kg m-2
     if(ixVegNrg/=integerMissing) aJac(ixVegNrg,ixVegHyd) = (dt/canopyDepth)   *(-dCanopyNetFlux_dCanLiq) - (1._rk - scalarFracLiqVeg)*LH_fus/canopyDepth   ! dF/dLiq
   ...
   if(nSnowOnlyHyd>0)then
    do iLayer=1,nSnow  ! loop through layers in the snow domain
     ...
     ! - lower-diagonal elements
     if(iLayer > 1)then
      if(ixSnowOnlyHyd(iLayer-1)/=integerMissing) aJac(ixSnowOnlyHyd(iLayer-1),watState) = 0._rk  ! sub-diagonal: no dependence on other layers
     endif
     ...
     if(nSnowOnlyNrg>0)then
      ...
      if(nrgstate/=integerMissing)then       ! (energy state for the current layer is within the state subset)
       ...
       ! (cross-derivative terms for the current layer)
       aJac(nrgState,watState) = -(1._rk - mLayerFracLiqSnow(iLayer))*LH_fus*iden_water     ! (dF/dLiq)
