...
contains
 ...
 ! input: forcing for the snow domain
 real(summa_prec),intent(in)             :: scalarThroughfallRain      ! computed throughfall rate (kg m-2 s-1)
 real(summa_prec),intent(in)             :: scalarCanopyLiqDrainage    ! computed drainage of liquid water (kg m-2 s-1)
 ! input: model state vector
 real(summa_prec),intent(in)             :: mLayerVolFracLiqTrial(:)   ! trial value of volumetric fraction of liquid water at the current iteration (-)
 ! input-output: data structures
 ...
 ! output: fluxes and derivatives
 real(summa_prec),intent(inout)          :: iLayerLiqFluxSnow(0:)      ! vertical liquid water flux at layer interfaces (m s-1)
 real(summa_prec),intent(inout)          :: iLayerLiqFluxSnowDeriv(0:) ! derivative in vertical liquid water flux at layer interfaces (m s-1)
 ! output: error control
 ...
 real(summa_prec)                        :: multResid                  ! multiplier for the residual water content (-)
 real(summa_prec),parameter              :: residThrs=550._summa_prec          ! ice density threshold to reduce residual liquid water content (kg m-3)
 real(summa_prec),parameter              :: residScal=10._summa_prec           ! scaling factor for residual liquid water content reduction factor (kg m-3)
 real(summa_prec),parameter              :: maxVolIceContent=0.7_summa_prec    ! maximum volumetric ice content to store water (-)
 real(summa_prec)                        :: availCap                   ! available storage capacity [0,1] (-)
 real(summa_prec)                        :: relSaturn                  ! relative saturation [0,1] (-)
 ! ------------------------------------------------------------------------------------------------------------------------------------------
 ...
 if(mw_exp<1._summa_prec)then; err=20; message=trim(message)//'meltwater exponent < 1'; return; end if
 ...
 iLayerLiqFluxSnowDeriv(0) = 0._summa_prec
 ...
 if(firstFluxCall)then
  ...
  do iLayer=1,nSnow
   ...
   multResid = 1._summa_prec / ( 1._summa_prec + exp( (mLayerVolFracIce(iLayer)*iden_ice - residThrs) / residScal) )
   ! compute the pore space (-)
   mLayerPoreSpace(iLayer)  = 1._summa_prec - mLayerVolFracIce(iLayer)
 ...
 do iLayer=ixTop,ixBot  ! (loop through snow layers)
  ...
  if(mLayerVolFracLiqTrial(iLayer) > mLayerThetaResid(iLayer))then
   ...
   iLayerLiqFluxSnow(iLayer)      = k_snow*relSaturn**mw_exp
   iLayerLiqFluxSnowDeriv(iLayer) = ( (k_snow*mw_exp)/availCap ) * relSaturn**(mw_exp - 1._summa_prec)
   ...
   iLayerLiqFluxSnow(iLayer)      = 0._summa_prec
   iLayerLiqFluxSnowDeriv(iLayer) = 0._summa_prec
