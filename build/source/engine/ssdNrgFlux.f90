...
public::ssdNrgFlux
! global parameters
real(summa_prec),parameter            :: dx=1.e-10_summa_prec             ! finite difference increment (K)
...
contains
 ...
 ! input: fluxes and derivatives at the upper boundary
 real(summa_prec),intent(in)             :: groundNetFlux              ! net energy flux for the ground surface (W m-2)
 real(summa_prec),intent(in)             :: dGroundNetFlux_dGroundTemp ! derivative in net ground flux w.r.t. ground temperature (W m-2 K-1)
 ! input: liquid water fluxes
 real(summa_prec),intent(in)             :: iLayerLiqFluxSnow(0:)      ! intent(in): liquid flux at the interface of each snow layer (m s-1)
 real(summa_prec),intent(in)             :: iLayerLiqFluxSoil(0:)      ! intent(in): liquid flux at the interface of each soil layer (m s-1)
 ! input: trial value of model state variables
 real(summa_prec),intent(in)             :: mLayerTempTrial(:)         ! trial temperature of each snow/soil layer at the current iteration (K)
 ! input-output: data structures
 ...
 ! output: fluxes and derivatives at all layer interfaces
 real(summa_prec),intent(out)            :: iLayerNrgFlux(0:)          ! energy flux at the layer interfaces (W m-2)
 real(summa_prec),intent(out)            :: dFlux_dTempAbove(0:)       ! derivatives in the flux w.r.t. temperature in the layer above (J m-2 s-1 K-1)
 real(summa_prec),intent(out)            :: dFlux_dTempBelow(0:)       ! derivatives in the flux w.r.t. temperature in the layer below (J m-2 s-1 K-1)
 ! output: error control
 ...
 real(summa_prec)                        :: qFlux                      ! liquid flux at layer interfaces (m s-1)
 real(summa_prec)                        :: dz                         ! height difference (m)
 real(summa_prec)                        :: flux0,flux1,flux2          ! fluxes used to calculate derivatives (W m-2)
 ! ------------------------------------------------------------------------------------------------------------------------------------------------------
 ...
 do iLayer=ixTop,ixBot ! (loop through model layers)
  ...
  if(iLayer==nLayers)then
   ...
   select case(ix_bcLowrTdyn) ! (identify the lower boundary condition for thermodynamics
    case(prescribedTemp); iLayerConductiveFlux(nLayers) = -iLayerThermalC(iLayer)*(lowerBoundTemp - mLayerTempTrial(iLayer))/(mLayerDepth(iLayer)*0.5_summa_prec)
    case(zeroFlux);       iLayerConductiveFlux(nLayers) = 0._summa_prec
  ...
  if(iLayer==nLayers)then  ! (lower boundary)
   ...
   select case(ix_bcLowrTdyn)
    ...
    case(prescribedTemp)
     ...
     dz = mLayerDepth(iLayer)*0.5_summa_prec
     ...
     case(zeroFlux)
      dFlux_dTempAbove(iLayer) = 0._summa_prec
