...
type, public :: split_select_type  ! class for selecting operator splitting methods
  ! opSplittin loop indices (in order)
  ...
  logical(lgt)             :: stateTypeSplitting,stateThenDomain,domainSplit
 contains
...
!write(*,*) "A:",ixCoupling,split_select%stateTypeSplitting,split_select%stateThenDomain,split_select%domainSplit
...
!write(*,*) "B:",iStateTypeSplit,split_select%stateTypeSplitting,split_select%stateThenDomain,split_select%domainSplit 
      end if
     end if
    end if
    if (split_select % stateTypeSplitting.eqv..true.) then 
...
!write(*,*) "C:",ixStateThenDomain,split_select%stateTypeSplitting,split_select%stateThenDomain,split_select%domainSplit 
         if (ixStateThenDomain.gt.(1+tryDomainSplit)) then
...
!write(*,*) "D:",iDomainSplit,split_select%stateTypeSplitting,split_select%stateThenDomain,split_select%domainSplit
         if (split_select % iDomainSplit > nDomainSplit) then
       ...
       if (split_select % domainSplit.eqv..true.) then
         ...
         solution: do ixSolution=1,nSolutions ! trial with the vector then scalar solution
           ...
           stateSplit: do iStateSplit=1,nStateSplit ! loop through layers (NOTE: nStateSplit=1 for the vector solution, hence no looping)
             ...
             if (cycle_domainSplit) then; call split_select % advance_iDomainSplit; cycle split_select_loop; end if
             ...
             if (cycle_stateThenDomain) then; call split_select % advance_ixStateThenDomain; split_select % domainSplit=.false.; cycle split_select_loop; end if ! deactivate flags for inner loops
             if (cycle_solution)        cycle solution
             ...
             if (exit_stateThenDomain) then; call split_select % initialize_ixStateThenDomain; split_select % stateThenDomain=.false.; split_select % domainSplit=.false.; exit solution; end if ! exit loops if necessary -- exit last available loop to avoid unnecessary operations -- deactivate inner loops
...
!!       end do domainSplit ! domain type splitting loop
       if (split_select % domainSplit.eqv..false.) then
        if (split_select % stateThenDomain.eqv..true.) call split_select % advance_ixStateThenDomain
       end if
      end if ! stateThenDomain
      if (split_select % domainSplit.eqv..false.) then
       if (split_select % stateThenDomain.eqv..false.) then
...
!   nSplit=nCoupling*nStateTypeSplit*(1+tryDomainSplit)*nDomainSplit*nSolutions*nStateSplit
  ...
  subroutine finalize_stateThenDomain
   ...
   !if (ixStateThenDomain == 2+tryDomainSplit) ixStateThenDomain=1+tryDomainSplit ! correct index value if stateThenDomain loop is completed fully 
...
subroutine split_select_initialize_flags(split_select)
 ! *** Initialize flags for opSplittin loops ***
...
subroutine split_select_advance_iStateTypeSplit(split_select)
 ! *** Advance index for stateTypeSplit loop ***
 class(split_select_type),intent(inout) :: split_select               ! class object for operator splitting selector
...
subroutine split_select_advance_ixStateThenDomain(split_select)
 ! *** Advance index for stateThenDomain loop ***
 class(split_select_type),intent(inout) :: split_select               ! class object for operator splitting selector
...
subroutine split_select_advance_iDomainSplit(split_select)
 ! *** Advance index for domainSplit loop ***
 class(split_select_type),intent(inout) :: split_select               ! class object for operator splitting selector
...
subroutine split_select_load_indices(split_select,ixCoupling,ixSolution,ixStateThenDomain,iStateTypeSplit,iDomainSplit,iStateSplit)
 ...
 class(split_select_type),intent(inout) :: split_select               ! class object for operator splitting selector
 integer(i4b),intent(in)                :: ixCoupling       
 integer(i4b),intent(in)                :: ixSolution       
 integer(i4b),intent(in)                :: ixStateThenDomain
 integer(i4b),intent(in)                :: iStateTypeSplit  
 integer(i4b),intent(in)                :: iDomainSplit     
 integer(i4b),intent(in)                :: iStateSplit      
 split_select % ixCoupling        = ixCoupling       
 split_select % ixSolution        = ixSolution       
 split_select % ixStateThenDomain = ixStateThenDomain
 split_select % iStateTypeSplit   = iStateTypeSplit  
 split_select % iDomainSplit      = iDomainSplit     
