 ...
 do iState=1,size(ixMapSubset2Full)
  ...
  if(ixStateType(ixFullVector)==iname_nrgCanair .or. ixStateType(ixFullVector)==iname_nrgCanopy .or. ixStateType(ixFullVector)==iname_nrgLayer)then
   ...
   select case(ixDomainType)
    ...
    ! ---------------------
    case(iname_cas)
     err=20; message=trim(message)//'still need to implement canopy air space'
     return
    ...
    ! -----------------------
    case(iname_veg)
     err=20; message=trim(message)//'still need to implement vegetation canopy'
     return
    ...
    ! ---------
    case(iname_snow)
     err=20; message=trim(message)//'still need to implement snow'
     return
