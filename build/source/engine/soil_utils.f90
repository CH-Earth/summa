...
! constant parameters
real(summa_prec),parameter     :: valueMissing=-9999._summa_prec    ! missing value parameter
real(summa_prec),parameter     :: verySmall=epsilon(1.0_summa_prec) ! a very small number (used to avoid divide by zero)
real(summa_prec),parameter     :: dx=-1.e-12_summa_prec             ! finite difference increment
contains
 ...
 implicit none
 ! input variables
 real(summa_prec),intent(in)     :: volFracIce        ! volumetric fraction of ice (-)
 real(summa_prec),intent(in)     :: f_impede          ! ice impedence parameter (-)
 ! output variables
 real(summa_prec)                :: iceImpedeFactor   ! ice impedence factor (-)
 ...
 iceImpedeFactor = 10._summa_prec**(-f_impede*volFracIce)
 dIceImpede_dLiq = 0._summa_prec
 ...
 ! input variables
 real(summa_prec),intent(in)     :: volFracIce        ! volumetric fraction of ice (-)
 real(summa_prec),intent(in)     :: dTheta_dT         ! derivative in volumetric liquid water content w.r.t temperature (K-1)
 real(summa_prec),intent(in)     :: f_impede          ! ice impedence parameter (-)
 ! output variables
 real(summa_prec)                :: dIceImpede_dT     ! derivative in the ice impedance factor w.r.t. temperature (K-1)
 ! --
 ...
 implicit none
 ! input
 real(summa_prec),intent(in)            :: matricHeadTotal                           ! total water matric potential (m)
 real(summa_prec),intent(in)            :: volFracLiq                                ! volumetric fraction of liquid water (-)
 real(summa_prec),intent(in)            :: volFracIce                                ! volumetric fraction of ice (-)
 real(summa_prec),intent(in)            :: vGn_alpha,vGn_n,theta_sat,theta_res,vGn_m ! soil parameters
 real(summa_prec),intent(in)  ,optional :: dVolTot_dPsi0                             ! derivative in the soil water characteristic (m-1)
 real(summa_prec),intent(in)  ,optional :: dTheta_dT                                 ! derivative in volumetric total water w.r.t. temperature (K-1)
 ! output
 real(summa_prec),intent(out)           :: matricHeadLiq                             ! liquid water matric potential (m)
 real(summa_prec),intent(out) ,optional :: dPsiLiq_dPsi0                             ! derivative in the liquid water matric potential w.r.t. the total water matric potential (-)
 real(summa_prec),intent(out) ,optional :: dPsiLiq_dTemp                             ! derivative in the liquid water matric potential w.r.t. temperature (m K-1)
 ! output: error control
 integer(i4b),intent(out)       :: err                                       ! error code
 character(*),intent(out)       :: message                                   ! error message
 ! local
 real(summa_prec)                       :: xNum,xDen                                 ! temporary variables (numeratir, denominator)
 real(summa_prec)                       :: effSat                                    ! effective saturation (-)
 real(summa_prec)                       :: dPsiLiq_dEffSat                           ! derivative in liquid water matric potential w.r.t. effective saturation (m)
 real(summa_prec)                       :: dEffSat_dTemp                             ! derivative in effective saturation w.r.t. temperature (K-1)
 ! ------------------------------------------------------------------------------------------------------------------------------
 ! initialize error control
 err=0; message='liquidHead/'

 ! ** partially frozen soil
 if(volFracIce > verySmall .and. matricHeadTotal < 0._summa_prec)then  ! check that ice exists and that the soil is unsaturated
  ...
  matricHeadLiq = matricHead(effSat,vGn_alpha,0._summa_prec,1._summa_prec,vGn_n,vGn_m)  ! argument is effective saturation, so theta_res=0 and theta_sat=1
  ...
  if(present(dPsiLiq_dPsi0).or.present(dPsiLiq_dTemp))then
   dPsiLiq_dEffSat = dPsi_dTheta(effSat,vGn_alpha,0._summa_prec,1._summa_prec,vGn_n,vGn_m)
  endif
  ...
  if(present(dPsiLiq_dTemp))then
   ...
   ! (compute derivative in the liquid water matric potential w.r.t. the total water matric potential)
   dPsiLiq_dPsi0 = dVolTot_dPsi0*dPsiLiq_dEffSat*xNum/(xDen**2._summa_prec)
  ...
  if(present(dPsiLiq_dTemp))then
   ...
   ! (compute the derivative in the liquid water matric potential w.r.t. temperature)
   dEffSat_dTemp = -dTheta_dT*xNum/(xDen**2._summa_prec) + dTheta_dT/xDen
 ...
 ! ** unfrozen soil
  ...
  if(present(dPsiLiq_dTemp)) dPsiLiq_dPsi0 = 1._summa_prec  ! derivative=1 because values are identical
  if(present(dPsiLiq_dTemp)) dPsiLiq_dTemp = 0._summa_prec  ! derivative=0 because no impact of temperature for unfrozen conditions
 ...
 implicit none
 ! dummies
 real(summa_prec),intent(in) :: volFracLiq    ! volumetric liquid water content (-)
 real(summa_prec),intent(in) :: theta_sat     ! soil porosity (-)
 real(summa_prec),intent(in) :: theta_mp      ! minimum volumetric liquid water content for macropore flow (-)
 real(summa_prec),intent(in) :: mpExp         ! empirical exponent in macropore flow equation (-)
 real(summa_prec),intent(in) :: satHydCond_ma ! saturated hydraulic conductivity for macropores (m s-1)
 real(summa_prec),intent(in) :: satHydCond_mi ! saturated hydraulic conductivity for micropores (m s-1)
 real(summa_prec)            :: hydCondMP_liq ! hydraulic conductivity (m s-1)
 ! locals
 real(summa_prec)            :: theta_e     ! effective soil moisture
 ...
 implicit none
 ! dummies
 real(summa_prec),intent(in)     :: psi           ! soil water suction (m)
 real(summa_prec),intent(in)     :: k_sat         ! saturated hydraulic conductivity (m s-1)
 real(summa_prec),intent(in)     :: alpha         ! scaling parameter (m-1)
 real(summa_prec),intent(in)     :: n             ! vGn "n" parameter (-)
 real(summa_prec),intent(in)     :: m             ! vGn "m" parameter (-)
 real(summa_prec)                :: hydCond_psi   ! hydraulic conductivity (m s-1)
 if(psi<0._summa_prec)then
  hydCond_psi = k_sat * &
                ( ( (1._summa_prec - (psi*alpha)**(n-1._summa_prec) * (1._summa_prec + (psi*alpha)**n)**(-m))**2._summa_prec ) &
                 / ( (1._summa_prec + (psi*alpha)**n)**(m/2._summa_prec) ) )
 else
  hydCond_psi = k_sat
 end if
 ...
 real(summa_prec),intent(in) :: volFracLiq  ! volumetric liquid water content (-)
 real(summa_prec),intent(in) :: k_sat       ! saturated hydraulic conductivity (m s-1)
 real(summa_prec),intent(in) :: theta_res   ! residual volumetric liquid water content (-)
 real(summa_prec),intent(in) :: theta_sat   ! soil porosity (-)
 real(summa_prec),intent(in) :: m           ! vGn "m" parameter (-)
 real(summa_prec)            :: hydCond_liq ! hydraulic conductivity (m s-1)
 ! locals
 real(summa_prec)            :: theta_e     ! effective soil moisture
 if(volFracLiq < theta_sat)then
  theta_e = (volFracLiq - theta_res) / (theta_sat - theta_res)
  hydCond_liq = k_sat*theta_e**(1._summa_prec/2._summa_prec) * (1._summa_prec - (1._summa_prec - theta_e**(1._summa_prec/m) )**m)**2._summa_prec
 else
  hydCond_liq = k_sat
 end if
 ...
 real(summa_prec),intent(in) :: psi         ! soil water suction (m)
 real(summa_prec),intent(in) :: alpha       ! scaling parameter (m-1)
 real(summa_prec),intent(in) :: theta_res   ! residual volumetric water content (-)
 real(summa_prec),intent(in) :: theta_sat   ! porosity (-)
 real(summa_prec),intent(in) :: n           ! vGn "n" parameter (-)
 real(summa_prec),intent(in) :: m           ! vGn "m" parameter (-)
 real(summa_prec)            :: volFracLiq  ! volumetric liquid water content (-)
 if(psi<0._summa_prec)then
  volFracLiq = theta_res + (theta_sat - theta_res)*(1._summa_prec + (alpha*psi)**n)**(-m)
 else
 ...
 implicit none
 ! dummy variables
 real(summa_prec),intent(in) :: theta       ! volumetric liquid water content (-)
 real(summa_prec),intent(in) :: alpha       ! scaling parameter (m-1)
 real(summa_prec),intent(in) :: theta_res   ! residual volumetric water content (-)
 real(summa_prec),intent(in) :: theta_sat   ! porosity (-)
 real(summa_prec),intent(in) :: n           ! vGn "n" parameter (-)
 real(summa_prec),intent(in) :: m           ! vGn "m" parameter (-)
 real(summa_prec)            :: matricHead  ! matric head (m)
 ! local variables
 real(summa_prec)            :: effSat      ! effective saturation (-)
 real(summa_prec),parameter  :: verySmall=epsilon(1._summa_prec)  ! a very small number (avoid effective saturation of zero)
 ! compute effective saturation
 effSat = max(verySmall, (theta - theta_res) / (theta_sat - theta_res))
 ! compute matric head
 if (effSat < 1._summa_prec .and. effSat > 0._summa_prec)then
  matricHead = (1._summa_prec/alpha)*( effSat**(-1._summa_prec/m) - 1._summa_prec)**(1._summa_prec/n)
 else
  matricHead = 0._summa_prec
 end if
 ...
 real(summa_prec),intent(in) :: n           ! vGn "n" parameter (-)
 real(summa_prec),intent(in) :: m           ! vGn "m" parameter (-)
 real(summa_prec)            :: dTheta_dPsi ! derivative of the soil water characteristic (m-1)
 if(psi<=0._summa_prec)then
  dTheta_dPsi = (theta_sat-theta_res) * &
     (-m*(1._summa_prec + (psi*alpha)**n)**(-m-1._summa_prec)) * n*(psi*alpha)**(n-1._summa_prec) * alpha
 ...
 real(summa_prec),intent(in) :: n           ! vGn "n" parameter (-)
 real(summa_prec),intent(in) :: m           ! vGn "m" parameter (-)
 real(summa_prec)            :: dPsi_dTheta ! derivative of the soil water characteristic (m)
 ! locals
 real(summa_prec)            :: y1,d1       ! 1st function and derivative
 real(summa_prec)            :: y2,d2       ! 2nd function and derivative
 real(summa_prec)            :: theta_e     ! effective soil moisture
 ...
 if(volFracLiq < theta_sat)then
  ...
  y1 = theta_e**(-1._summa_prec/m) - 1._summa_prec
  d1 = (-1._summa_prec/m)*theta_e**(-1._summa_prec/m - 1._summa_prec) / (theta_sat - theta_res)
  ! compute the 2nd function and derivative
  y2 = y1**(1._summa_prec/n)
  d2 = (1._summa_prec/n)*y1**(1._summa_prec/n - 1._summa_prec)
  ! compute the final function value
  dPsi_dTheta = d1*d2/alpha
 else
  dPsi_dTheta = 0._summa_prec
 end if
 ...
 real(summa_prec)                :: dPsi_dTheta2 ! derivative of the soil water characteristic (m)
 ! locals for analytical derivatives
 real(summa_prec)                :: xx           ! temporary variable
 real(summa_prec)                :: y1,d1        ! 1st function and derivative
 real(summa_prec)                :: y2,d2        ! 2nd function and derivative
 real(summa_prec)                :: theta_e      ! effective soil moisture
 ! locals for numerical derivative
 real(summa_prec)                :: func0,func1  ! function evaluations
 ! check if less than saturation
 if(volFracLiq < theta_sat)then
  ...
  if(lTangent)then
   ...
   y1 = (-1._summa_prec/m)*theta_e**(-1._summa_prec/m - 1._summa_prec) / (theta_sat - theta_res)
   d1 = ( (m + 1._summa_prec) / (m**2._summa_prec * (theta_sat - theta_res)**2._summa_prec) ) * theta_e**(-1._summa_prec/m - 2._summa_prec)
   ! get the second function and derivative
   xx = theta_e**(-1._summa_prec/m) - 1._summa_prec
   y2 = (1._summa_prec/n)*xx**(1._summa_prec/n - 1._summa_prec)
   d2 = ( -(1._summa_prec - n)/((theta_sat - theta_res)*m*n**2._summa_prec) ) * xx**(1._summa_prec/n - 2._summa_prec) * theta_e**(-1._summa_prec/m - 1._summa_prec)
 ...
 real(summa_prec),intent(in)     :: m          ! vGn "m" parameter (-)
 logical(lgt),intent(in) :: lTangent   ! method used to compute derivative (.true. = analytical)
 real(summa_prec)                :: dHydCond_dPsi  ! derivative in hydraulic conductivity w.r.t. matric head (s-1)
 ! locals for analytical derivatives
 real(summa_prec)                :: f_x1          ! f(x) for part of the numerator
 real(summa_prec)                :: f_x2          ! f(x) for part of the numerator
 real(summa_prec)                :: f_nm          ! f(x) for the numerator
 real(summa_prec)                :: f_dm          ! f(x) for the denominator
 real(summa_prec)                :: d_x1          ! df(x)/dpsi for part of the numerator
 real(summa_prec)                :: d_x2          ! df(x)/dpsi for part of the numerator
 real(summa_prec)                :: d_nm          ! df(x)/dpsi for the numerator
 real(summa_prec)                :: d_dm          ! df(x)/dpsi for the denominator
 ! locals for numerical derivatives
 real(summa_prec)                :: hydCond0   ! hydraulic condictivity value for base case
 real(summa_prec)                :: hydCond1   ! hydraulic condictivity value for perturbed case
 ! derivative is zero if saturated
 if(psi<0._summa_prec)then
  ! ***** compute analytical derivatives
  if(lTangent)then
   ! compute the derivative for the numerator
   f_x1 = (psi*alpha)**(n - 1._summa_prec)
   f_x2 = (1._summa_prec + (psi*alpha)**n)**(-m)
   d_x1 = alpha * (n - 1._summa_prec)*(psi*alpha)**(n - 2._summa_prec)
   d_x2 = alpha * n*(psi*alpha)**(n - 1._summa_prec) * (-m)*(1._summa_prec + (psi*alpha)**n)**(-m - 1._summa_prec)
   f_nm = (1._summa_prec - f_x1*f_x2)**2._summa_prec
   d_nm = (-d_x1*f_x2 - f_x1*d_x2) * 2._summa_prec*(1._summa_prec - f_x1*f_x2)
   ! compute the derivative for the denominator
   f_dm = (1._summa_prec + (psi*alpha)**n)**(m/2._summa_prec)
   d_dm = alpha * n*(psi*alpha)**(n - 1._summa_prec) * (m/2._summa_prec)*(1._summa_prec + (psi*alpha)**n)**(m/2._summa_prec - 1._summa_prec)
   ! and combine
   dHydCond_dPsi = k_sat*(d_nm*f_dm - d_dm*f_nm) / (f_dm**2._summa_prec)
  else
   ! ***** compute numerical derivatives
 ...
 real(summa_prec),intent(in)     :: m          ! vGn "m" parameter (-)
 logical(lgt),intent(in) :: lTangent   ! method used to compute derivative (.true. = analytical)
 real(summa_prec)                :: dHydCond_dLiq  ! derivative in hydraulic conductivity w.r.t. matric head (s-1)
 ! locals for analytical derivatives
 real(summa_prec)                :: theta_e  ! effective soil moisture
 real(summa_prec)                :: f1       ! f(x) for the first function
 real(summa_prec)                :: d1       ! df(x)/dLiq for the first function
 real(summa_prec)                :: x1,x2    ! f(x) for different parts of the second function
 ...
 if(volFracLiq < theta_sat)then
  ...
  if(lTangent)then
   ...
   f1 = k_sat*theta_e**0.5_summa_prec
   d1 = k_sat*0.5_summa_prec*theta_e**(-0.5_summa_prec) / (theta_sat - theta_res)
   ! compute the function and derivative of the second function
   ! (first part)
   x1 = 1._summa_prec - theta_e**(1._summa_prec/m)
   p1 = (-1._summa_prec/m)*theta_e**(1._summa_prec/m - 1._summa_prec) / (theta_sat - theta_res)   ! differentiate (1.d - theta_e**(1.d/m)
   ! (second part)
   x2 = x1**m
   p2 = m*x1**(m - 1._summa_prec)
   ! (final)
   f2 = (1._summa_prec - x2)**2._summa_prec
   p3 = -2._summa_prec*(1._summa_prec - x2)
   ! (combine)
   d2 = p1*p2*p3
 ...
 implicit none
 real(summa_prec),intent(in) :: matpot        ! soil water suction -- matric potential (m)
 real(summa_prec),intent(in) :: Tk            ! temperature (K)
 real(summa_prec)            :: RH_soilair    ! relative humidity of air in soil pore space
 ...
 implicit none
 real(summa_prec),intent(in) :: psi           ! matric head (m)
 real(summa_prec)            :: crit_soilT    ! critical soil temperature (K)
 crit_soilT = Tfreeze + min(psi,0._summa_prec)*gravity*Tfreeze/LH_fus
 ...
 implicit none
 real(summa_prec),intent(in) :: Tk            ! temperature (K)
 real(summa_prec),intent(in) :: theta_res     ! residual liquid water content (-)
 real(summa_prec),intent(in) :: theta_sat     ! porosity (-)
 real(summa_prec),intent(in) :: alpha         ! vGn scaling parameter (m-1)
 real(summa_prec),intent(in) :: n             ! vGn "n" parameter (-)
 real(summa_prec),intent(in) :: m             ! vGn "m" parameter (-)
 real(summa_prec)            :: dTheta_dTk    ! derivative of the freezing curve w.r.t. temperature (K-1)
 ! local variables
 real(summa_prec)            :: kappa         ! constant (m K-1)
 real(summa_prec)            :: xtemp         ! alpha*kappa*(Tk-Tfreeze) -- dimensionless variable (used more than once)
 ! compute kappa (m K-1)
 ...
 dTheta_dTk = (alpha*kappa) * n*xtemp**(n - 1._summa_prec) * (-m)*(1._summa_prec + xtemp**n)**(-m - 1._summa_prec) * (theta_sat - theta_res)
 ...
 FUNCTION gammp(a,x)
 IMPLICIT NONE
 real(summa_prec), INTENT(IN) :: a,x
 real(summa_prec) :: gammp
 if (x<a+1.0_summa_prec) then
  gammp=gser(a,x)
 else
  gammp=1.0_summa_prec-gcf(a,x)
 end if
 END FUNCTION gammp
 ...
 IMPLICIT NONE
 real(summa_prec), INTENT(IN) :: a,x
 real(summa_prec), OPTIONAL, INTENT(OUT) :: gln
 real(summa_prec) :: gcf
 INTEGER(I4B), PARAMETER :: ITMAX=100
 real(summa_prec), PARAMETER :: EPS=epsilon(x),FPMIN=tiny(x)/EPS
 INTEGER(I4B) :: i
 real(summa_prec) :: an,b,c,d,del,h
 if (x == 0.0) then
  gcf=1.0
  RETURN
 end if
 b=x+1.0_summa_prec-a
 c=1.0_summa_prec/FPMIN
 d=1.0_summa_prec/b
 h=d
 do i=1,ITMAX
  an=-i*(i-a)
  ...
  del=d*c
  h=h*del
  if (abs(del-1.0_summa_prec) <= EPS) exit
 end do
 ...
 real(summa_prec), INTENT(IN) :: a,x
 real(summa_prec), OPTIONAL, INTENT(OUT) :: gln
 real(summa_prec) :: gser
 INTEGER(I4B), PARAMETER :: ITMAX=100
 real(summa_prec), PARAMETER :: EPS=epsilon(x)
 INTEGER(I4B) :: n
 real(summa_prec) :: ap,del,summ
 if (x == 0.0) then
  gser=0.0
  RETURN
 end if
 ap=a
 summ=1.0_summa_prec/a
 del=summ
 do n=1,ITMAX
  ap=ap+1.0_summa_prec
  del=del*x/ap
  summ=summ+del
 ...
 IMPLICIT NONE
 real(summa_prec), INTENT(IN) :: xx
 real(summa_prec) :: gammln
 real(summa_prec) :: tmp,x
 real(summa_prec) :: stp = 2.5066282746310005_summa_prec
 real(summa_prec), DIMENSION(6) :: coef = (/76.18009172947146_summa_prec,&
  -86.50532032941677_summa_prec,24.01409824083091_summa_prec,&
  -1.231739572450155_summa_prec,0.1208650973866179e-2_summa_prec,&
  -0.5395239384953e-5_summa_prec/)
 if(xx <= 0._summa_prec) stop 'xx > 0 in gammln'
 x=xx
 tmp=x+5.5_summa_prec
 tmp=(x+0.5_summa_prec)*log(tmp)-tmp
 gammln=tmp+log(stp*(1.000000000190015_summa_prec+&
  sum(coef(:)/arth(x+1.0_summa_prec,1.0_summa_prec,size(coef))))/x)
 END FUNCTION gammln
