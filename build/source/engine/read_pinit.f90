 ...
 ! populate parameters that were not included in the original control files
  ...
  if(isLocal)then
   if(model_decisions(iLookDECISIONS%cIntercept)%iDecision == unDefined)then
    parFallback(iLookPARAM%canopyWettingFactor)%default_val = 1._summa_prec             ! maximum wetted fraction of the canopy (-)
    parFallback(iLookPARAM%canopyWettingExp)%default_val    = 0.666666667_summa_prec    ! exponent in canopy wetting function (-)
   end if
  end if
 end if
 ! close file unit
