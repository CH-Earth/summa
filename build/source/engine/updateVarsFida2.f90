...
USE updatStateFida_module,only:updateVegFida     ! update snow states
USE updatStateFida_module,only:updateSnowFida     ! update snow states
USE updatStateFida_module,only:updateSoilFida2     ! update soil states
...
USE soil_utils_fida_module,only:liquidHeadFida     ! compute the liquid water matric potential
USE soil_utils_fida_module,only:d2Theta_dPsi2
USE soil_utils_fida_module,only:d2Theta_dTk2
...
implicit none
private
public::updateVarsFida2

contains

 ! **********************************************************************************************************
 ! public subroutine updateVarsFida2: compute diagnostic variables
 ! **********************************************************************************************************
 subroutine updateVarsFida2(&
                       ! input
 ...
 real(dp),intent(out)            :: mLayerd2Theta_dTk2(:)           ! reza 
 real(dp),intent(out)            :: d2VolTot_d2Psi0(:)              ! reza
 real(dp),intent(out)            :: dFracLiqSnow_dTk(:)             ! reza
 real(dp),intent(out)            :: d2Theta_dTkCanopy2
 real(dp),intent(out)            :: dFracLiqVeg_dTkCanopy
 ! output: error control
 ...
 do iState=1,size(ixMapSubset2Full)
  ...
  iterations: do iter=1,niter
   ...
   if(ixDomainType==iname_soil)then
    select case( ixStateType(ixFullVector) )
     case(iname_lmpLayer) 
       dVolTot_dPsi0(ixControlIndex) = dTheta_dPsi(mLayerMatricHeadLiqTrial(ixControlIndex),vGn_alpha(ixControlIndex),0._dp,1._dp,vGn_n(ixControlIndex),vGn_m(ixControlIndex))*avPore
       d2VolTot_d2Psi0(ixControlIndex) = d2Theta_dPsi2(mLayerMatricHeadLiqTrial(ixControlIndex),vGn_alpha(ixControlIndex),0._dp,1._dp,vGn_n(ixControlIndex),vGn_m(ixControlIndex))*avPore
     case default
   ...
   if(xTemp<Tcrit)then
    select case(ixDomainType)
     case(iname_veg)
        ...
        d2Theta_dTkCanopy2 = 2._dp * snowfrz_scale**2._dp * ( (Tfreeze - xTemp) * 2._dp * fracliquid(xTemp,snowfrz_scale) * dFracLiq_dTk(xTemp,snowfrz_scale) - fracliquid(xTemp,snowfrz_scale)**2._dp ) * scalarCanopyWatTrial/(iden_water*canopyDepth)
     case(iname_snow)
       ...
       mLayerd2Theta_dTk2(iLayer) = 2._dp * snowfrz_scale**2._dp * ( (Tfreeze - xTemp) * 2._dp * fracliquid(xTemp,snowfrz_scale) * dFracLiq_dTk(xTemp,snowfrz_scale) - fracliquid(xTemp,snowfrz_scale)**2._dp ) * mLayerVolFracWatTrial(iLayer)
     case(iname_soil)
      dFracLiqSnow_dTk(iLayer) = 0._dp
   ...
   ! --> unfrozen: no dependence of liquid water on temperature
    ...
    select case(ixDomainType)
     case(iname_veg);             dTheta_dTkCanopy         = 0._dp; d2Theta_dTkCanopy2 = 0; dFracLiqVeg_dTkCanopy = 0
     case(iname_snow, iname_soil);   mLayerdTheta_dTk(iLayer) = 0._dp; mLayerd2Theta_dTk2(iLayer) = 0._dp; dFracLiqSnow_dTk(iLayer) = 0
     case default; err=20; message=trim(message)//'expect case to be iname_veg, iname_snow, iname_soil'; return
    end select  ! domain type
   endif
    ...
    select case(ixDomainType)
     ...
     case(iname_veg)   
      ...
      call updateVegFida(&
                      ...
                      scalarCanopyWatTrial,                         & ! intent(in)   : volumetric fraction of total water (-)
                      snowfrz_scale,                                & ! intent(in)   : scaling parameter for the snow freezing curve (K-1)
                      scalarCanopyTempPrime,                        & ! intent(in)
                      scalarCanopyWatPrime,                         & ! intent(in)   : volumetric fraction of total water (-)
                      ...
                      scalarVolFracLiqPrime,                        & ! intent(out)  : trial volumetric fraction of liquid water (-)
                      scalarVolFracIcePrime,                        & ! intent(out)  : trial volumetric fraction if ice (-)
 ...
 end subroutine updateVarsFida2
...
end module updateVarsFida2_module
