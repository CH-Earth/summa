...
contains
 ...
 real(summa_prec)                                :: fracHRU                ! fractional area of a given HRU (-)
 ...
 bvarData%var(iLookBVAR%basin__SurfaceRunoff)%dat(1)    = 0._summa_prec  ! surface runoff (m s-1)
 bvarData%var(iLookBVAR%basin__ColumnOutflow)%dat(1)    = 0._summa_prec  ! outflow from all "outlet" HRUs (those with no downstream HRU)

 ! initialize baseflow variables
 bvarData%var(iLookBVAR%basin__AquiferRecharge)%dat(1)  = 0._summa_prec ! recharge to the aquifer (m s-1)
 bvarData%var(iLookBVAR%basin__AquiferBaseflow)%dat(1)  = 0._summa_prec ! baseflow from the aquifer (m s-1)
 bvarData%var(iLookBVAR%basin__AquiferTranspire)%dat(1) = 0._summa_prec ! transpiration loss from the aquifer (m s-1)

 ! initialize total inflow for each layer in a soil column
 do iHRU=1,gruInfo%hruCount
  fluxHRU%hru(iHRU)%var(iLookFLUX%mLayerColumnInflow)%dat(:) = 0._summa_prec
 end do
