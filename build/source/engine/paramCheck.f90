...
contains
 ...
 real(summa_prec),dimension(4)           :: zmaxLayer_lower      ! lower value of maximum layer depth
 real(summa_prec),dimension(4)           :: zmaxLayer_upper      ! upper value of maximum layer depth
 ...
 select case(model_decisions(iLookDECISIONS%snowLayers)%iDecision)
  ...
  case(sameRulesAllLayers)
   if(mpar_data%var(iLookPARAM%zmax)%dat(1)/mpar_data%var(iLookPARAM%zmin)%dat(1) < 2.5_summa_prec)then
  ...
  case(rulesDependLayerIndex)
   ...
   do iLayer=1,4  ! NOTE: the lower layer does not have a maximum value
    ...
    if(zmaxLayer_upper(iLayer)/zminLayer(iLayer) < 2.5_summa_prec .or. zmaxLayer_upper(iLayer)/zminLayer(iLayer+1) < 2.5_summa_prec)then
