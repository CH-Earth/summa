...
real(summa_prec),dimension(nlook),public  :: E_lookup          ! enthalpy values (J kg-1)
real(summa_prec),dimension(nlook),public  :: T_lookup          ! temperature values (K)
contains
 ...
 real(summa_prec),parameter            :: T_start=260.0_summa_prec     ! start temperature value where all liquid water is assumed frozen (K)
 real(summa_prec)                      :: T_incr,E_incr        ! temperature/enthalpy increments
 real(summa_prec),dimension(nlook)     :: Tk                   ! initial temperature vector
 real(summa_prec),dimension(nlook)     :: Ey                   ! initial enthalpy vector
 real(summa_prec),parameter            :: waterWght=1._summa_prec      ! weight applied to total water (kg m-3) --- cancels out
 real(summa_prec),dimension(nlook)     :: T2deriv              ! 2nd derivatives of the interpolating function at tabulated points
 ...
 T_incr = (Tfreeze - T_start) / real(nlook-1, kind(summa_prec))  ! temperature increment
 ...
 E_incr   = (-Ey(1)) / real(nlook-1, kind(summa_prec))  ! enthalpy increment
 ...
 implicit none
 ! declare dummy variables
 real(summa_prec),intent(in)      :: Ey            ! total enthalpy (J m-3)
 real(summa_prec),intent(in)      :: BulkDenWater  ! bulk density of water (kg m-3)
 real(summa_prec),intent(in)      :: fc_param      ! freezing curve parameter (K-1)
 real(summa_prec),intent(out)     :: Tk            ! initial temperature guess / final temperature value (K)
 integer(i4b),intent(out) :: err           ! error code
 character(*),intent(out) :: message       ! error message
 ! declare local variables
 real(summa_prec),parameter       :: dx=1.d-8      ! finite difference increment (J kg-1)
 real(summa_prec),parameter       :: atol=1.d-12   ! convergence criteria (J kg-1)
 real(summa_prec)                 :: E_spec        ! specific enthalpy (J kg-1)
 real(summa_prec)                 :: E_incr        ! enthalpy increment
 integer(i4b)             :: niter=15      ! maximum number of iterations
 integer(i4b)             :: iter          ! iteration index
 integer(i4b)             :: i0            ! position in lookup table
 real(summa_prec)                 :: Tg0,Tg1       ! trial temperatures (K)
 real(summa_prec)                 :: Ht0,Ht1       ! specific enthalpy, based on the trial temperatures (J kg-1)
 real(summa_prec)                 :: f0,f1         ! function evaluations (difference between enthalpy guesses)
 real(summa_prec)                 :: dh            ! enthalpy derivative
 real(summa_prec)                 :: dT            ! temperature increment
 ! initialize error control
 ...
 if(E_spec<E_lookup(1))then ! process cases below the limit of the look-up tab;e
  ...
  Tg1 = Tg0+dx
  ! compute enthalpy
  Ht0 = temp2ethpy(Tg0,1._summa_prec,fc_param)
  Ht1 = temp2ethpy(Tg1,1._summa_prec,fc_param)
 ...
 implicit none
 ! declare dummy variables
 real(summa_prec),intent(in)  :: Tk            ! layer temperature (K)
 real(summa_prec),intent(in)  :: BulkDenWater  ! bulk density of water (kg m-3)
 real(summa_prec),intent(in)  :: fc_param      ! freezing curve parameter (K-1)
 real(summa_prec)             :: temp2ethpy    ! return value of the function, total specific enthalpy (J m-3)
 ! declare local variables
 real(summa_prec)             :: frac_liq      ! fraction of liquid water
 real(summa_prec)             :: enthTempWater ! temperature component of specific enthalpy for total water (liquid and ice) (J kg-1)
 real(summa_prec)             :: enthMass      ! mass component of specific enthalpy (J kg-1)
 ...
 frac_liq     = 1._summa_prec / ( 1._summa_prec + ( fc_param*( Tfreeze - min(Tk,Tfreeze) ) )**2._summa_prec )
 ...
 ! NOTE: negative enthalpy means require energy to bring to Tfreeze
 enthMass     = -LH_fus*(1._summa_prec - frac_liq)
