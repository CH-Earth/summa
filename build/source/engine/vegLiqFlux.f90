...
contains
 ...
 real(rk),intent(in)             :: scalarCanopyLiqTrial         ! trial mass of liquid water on the vegetation canopy at the current iteration (kg m-2)
 real(rk),intent(in)             :: scalarRainfall               ! rainfall (kg m-2 s-1)
 ! input-output: data structures
 type(var_dlength),intent(in)    :: mpar_data                    ! model parameters
 type(var_dlength),intent(inout) :: diag_data                    ! model diagnostic variables for the local basin
 ! output
 real(rk),intent(out)            :: scalarThroughfallRain        ! rain that reaches the ground without ever touching the canopy (kg m-2 s-1)
 real(rk),intent(out)            :: scalarCanopyLiqDrainage      ! drainage of liquid water from the vegetation canopy (kg m-2 s-1)
 real(rk),intent(out)            :: scalarThroughfallRainDeriv   ! derivative in throughfall w.r.t. canopy liquid water (s-1)
 real(rk),intent(out)            :: scalarCanopyLiqDrainageDeriv ! derivative in canopy drainage w.r.t. canopy liquid water (s-1)
 ...
 if(.not.computeVegFlux)then
  ...
  scalarCanopyLiqDrainage      = 0._rk
  scalarThroughfallRainDeriv   = 0._rk
  scalarCanopyLiqDrainageDeriv = 0._rk
  return
 end if
