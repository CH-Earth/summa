...
contains
 ...
 real(summa_prec),intent(in)             :: scalarCanopyLiqTrial         ! trial mass of liquid water on the vegetation canopy at the current iteration (kg m-2)
 real(summa_prec),intent(in)             :: scalarRainfall               ! rainfall (kg m-2 s-1)
 ! input-output: data structures
 type(var_dlength),intent(in)    :: mpar_data                    ! model parameters
 type(var_dlength),intent(inout) :: diag_data                    ! model diagnostic variables for the local basin
 ! output
 real(summa_prec),intent(out)            :: scalarThroughfallRain        ! rain that reaches the ground without ever touching the canopy (kg m-2 s-1)
 real(summa_prec),intent(out)            :: scalarCanopyLiqDrainage      ! drainage of liquid water from the vegetation canopy (kg m-2 s-1)
 real(summa_prec),intent(out)            :: scalarThroughfallRainDeriv   ! derivative in throughfall w.r.t. canopy liquid water (s-1)
 real(summa_prec),intent(out)            :: scalarCanopyLiqDrainageDeriv ! derivative in canopy drainage w.r.t. canopy liquid water (s-1)
 ...
 if(.not.computeVegFlux)then
  ...
  scalarCanopyLiqDrainage      = 0._summa_prec
  scalarThroughfallRainDeriv   = 0._summa_prec
  scalarCanopyLiqDrainageDeriv = 0._summa_prec
  return
 end if
 ...
 select case(ixCanopyInterception)
  ...
  case(unDefined)
   scalarThroughfallRain      = 0._summa_prec
   scalarThroughfallRainDeriv = 0._summa_prec
