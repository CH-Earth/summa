...
! global parameters
real(rk),parameter  :: verySmall=1e-3_rk      ! tiny number
real(rk),parameter  :: smallOffset=1.e-8_rk   ! small offset (units=days) to force ih=0 at the start of the day
...
contains
 ...
 real(rk)                          :: startJulDay      ! julian day at the start of the year
 real(rk)                          :: currentJulday    ! Julian day of current time step
 ...
 call compjulday(time_data(iLookTIME%iyyy),          & ! input  = year
                 1, 1, 1, 1, 0._rk,                  & ! input  = month, day, hour, minute, second
 ...
 call compjulday(time_data(iLookTIME%iyyy),           & ! input  = year
                 ...
                 time_data(iLookTIME%imin),0._rk,     & ! input  = minute/second
 ...
 implicit none
 ! define input
 real(rk),intent(in)               :: currentJulday    ! Julian day of current time step
 ! define input-output variables
 integer(i4b),intent(inout)        :: iFile            ! index of current forcing file in forcing file list
 integer(i4b),intent(inout)        :: iRead            ! index of read position in time dimension in current netcdf file
 integer(i4b),intent(inout)        :: ncid             ! netcdf file identifier
 ...
 real(rk)                          :: timeVal(1)       ! single time value (restrict time read)
 real(rk),allocatable              :: fileTime(:)      ! array of time from netcdf file
 real(rk),allocatable              :: diffTime(:)      ! array of time differences
 ! Start procedure here
 ...
 select case(trim(NC_TIME_ZONE))
  ...
  case('utcTime');   tmZoneOffsetFracDay = 0._rk
  case('localTime'); tmZoneOffsetFracDay = 0._rk
 ...
 select case( trim( refTimeString(1:index(refTimeString,' ')) ) )
  case('seconds'); forcFileInfo(iFile)%convTime2Days=86400._rk
  case('minutes'); forcFileInfo(iFile)%convTime2Days=1440._rk
  case('hours');   forcFileInfo(iFile)%convTime2Days=24._rk
  case('days');    forcFileInfo(iFile)%convTime2Days=1._rk
 ...
 real(rk)                          :: varTime(1)       ! time variable of current forcing data step being read
 ! other local variables
 integer(i4b)                      :: iGRU,iHRU        ! index of GRU and HRU
 ...
 real(rk)                          :: dataJulDay       ! julian day of current forcing data step being read
 real(rk),dimension(nHRUlocal)     :: dataVec          ! vector of data
 real(rk),dimension(1)             :: dataVal          ! single data value
 real(rk),parameter                :: dataMin=-1._rk   ! minimum allowable data value (all forcing variables should be positive)
