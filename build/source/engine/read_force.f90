...
! global parameters
real(summa_prec),parameter  :: verySmall=1e-3_summa_prec      ! tiny number
real(summa_prec),parameter  :: smallOffset=1.e-8_summa_prec   ! small offset (units=days) to force ih=0 at the start of the day
...
contains
 ...
 real(summa_prec)                          :: startJulDay      ! julian day at the start of the year
 real(summa_prec)                          :: currentJulday    ! Julian day of current time step
 ...
 call compjulday(time_data(iLookTIME%iyyy),          & ! input  = year
                 1, 1, 1, 1, 0._summa_prec,                  & ! input  = month, day, hour, minute, second
 ...
 call compjulday(time_data(iLookTIME%iyyy),           & ! input  = year
                 ...
                 time_data(iLookTIME%imin),0._summa_prec,     & ! input  = minute/second
 ...
 implicit none
 ! define input
 real(summa_prec),intent(in)               :: currentJulday    ! Julian day of current time step
 ! define input-output variables
 integer(i4b),intent(inout)        :: iFile            ! index of current forcing file in forcing file list
 integer(i4b),intent(inout)        :: iRead            ! index of read position in time dimension in current netcdf file
 integer(i4b),intent(inout)        :: ncid             ! netcdf file identifier
 ...
 real(summa_prec)                          :: timeVal(1)       ! single time value (restrict time read)
 real(summa_prec),allocatable              :: fileTime(:)      ! array of time from netcdf file
 real(summa_prec),allocatable              :: diffTime(:)      ! array of time differences
 ! Start procedure here
 ...
 real(summa_prec)                          :: dsec,dsec_tz       ! seconds
 integer(i4b)                      :: varId              ! variable identifier
 ...
 select case(trim(NC_TIME_ZONE))
  ...
  case('utcTime');   tmZoneOffsetFracDay = 0._summa_prec
  case('localTime'); tmZoneOffsetFracDay = 0._summa_prec
 ...
 select case( trim( refTimeString(1:index(refTimeString,' ')) ) )
  case('seconds'); forcFileInfo(iFile)%convTime2Days=86400._summa_prec
  case('minutes'); forcFileInfo(iFile)%convTime2Days=1440._summa_prec
  case('hours');   forcFileInfo(iFile)%convTime2Days=24._summa_prec
  case('days');    forcFileInfo(iFile)%convTime2Days=1._summa_prec
 ...
 real(summa_prec)                          :: varTime(1)       ! time variable of current forcing data step being read
 ! other local variables
 integer(i4b)                      :: iGRU,iHRU        ! index of GRU and HRU
 ...
 real(summa_prec)                          :: dataJulDay       ! julian day of current forcing data step being read
 real(summa_prec),dimension(nHRUlocal)     :: dataVec          ! vector of data
 real(summa_prec),dimension(1)             :: dataVal          ! single data value
 real(summa_prec),parameter                :: dataMin=-1._summa_prec   ! minimum allowable data value (all forcing variables should be positive)
