...
contains
 ...
 real(summa_prec),intent(in)                 :: fc_param                   ! freeezing curve parameter for snow (K-1)
 ! input: diagnostic scalar variables
 real(summa_prec),intent(in)                 :: scalarSnowfallTemp         ! computed temperature of fresh snow (K)
 real(summa_prec),intent(in)                 :: scalarNewSnowDensity       ! computed density of new snow (kg m-3)
 real(summa_prec),intent(in)                 :: scalarThroughfallSnow      ! throughfall of snow through the canopy (kg m-2 s-1)
 real(summa_prec),intent(in)                 :: scalarCanopySnowUnloading  ! unloading of snow from the canopy (kg m-2 s-1)
 ! input/output: state variables
 real(summa_prec),intent(inout)              :: scalarSWE                  ! SWE (kg m-2)
 real(summa_prec),intent(inout)              :: scalarSnowDepth            ! total snow depth (m)
 real(summa_prec),intent(inout)              :: surfaceLayerTemp           ! temperature of each layer (K)
 real(summa_prec),intent(inout)              :: surfaceLayerDepth          ! depth of each layer (m)
 real(summa_prec),intent(inout)              :: surfaceLayerVolFracIce     ! volumetric fraction of ice in each layer (-)
 real(summa_prec),intent(inout)              :: surfaceLayerVolFracLiq     ! volumetric fraction of liquid water in each layer (-)
 ! output: error control
 integer(i4b),intent(out)            :: err                        ! error code
 character(*),intent(out)            :: message                    ! error message
 ! define local variables
 real(summa_prec)                            :: newSnowfall                ! new snowfall -- throughfall and unloading (kg m-2 s-1)
 real(summa_prec)                            :: newSnowDepth               ! new snow depth (m)
 real(summa_prec),parameter                  :: densityCanopySnow=200._summa_prec  ! density of snow on the vegetation canopy (kg m-3)
 real(summa_prec)                            :: totalMassIceSurfLayer      ! total mass of ice in the surface layer (kg m-2)
 real(summa_prec)                            :: totalDepthSurfLayer        ! total depth of the surface layer (m)
 ...
 real(summa_prec)                            :: SWE                        ! snow water equivalent after snowfall (kg m-2)
 real(summa_prec)                            :: tempSWE0                   ! temporary SWE before snowfall, used to check mass balance (kg m-2)
 real(summa_prec)                            :: tempSWE1                   ! temporary SWE after snowfall, used to check mass balance (kg m-2)
 real(summa_prec)                            :: xMassBalance               ! mass balance check (kg m-2)
 real(summa_prec),parameter                  :: verySmall=1.e-8_summa_prec         ! a very small number -- used to check mass balance
 ! initialize error control
 ...
 ! add snow to the top layer (more typical case where snow layers already exist)
  ...
  surfaceLayerVolFracIce = (1._summa_prec - fracLiq)*volFracWater*(iden_water/iden_ice)  ! volumetric fraction of ice (-)
