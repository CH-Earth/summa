...
subroutine systemSolv(&
  ...
  USE enthalpyTemp_module,only:T2enthalpy_snowlu          ! convert temperature to enthalpy for a snow layer
 ...
 subroutine initial_function_evaluations
  ...
  associate(&
   ...
   if (nSnow>0) then
     ...
     volEnthalpy = T2enthalpy_snowlu(mLayerTemp(1),bulkDensity,snowfrz_scale)
