...
contains
 ...
 do iLayer=1,nLayers
  ...
  mLayerHeight(iLayer) = iLayerHeight(iLayer-1) + mLayerDepth(iLayer)/2._rk
 ...
 integer(i4b)                    :: iLayer          ! loop through layers
 real(rk)                        :: fracRootLower   ! fraction of the rooting depth at the lower interface
 real(rk)                        :: fracRootUpper   ! fraction of the rooting depth at the upper interface
 real(rk), parameter             :: rootTolerance = 0.05_rk ! tolerance for error in doubleExp rooting option
 real(rk)                        :: error           ! machine precision error in rooting distribution
 ! initialize error control
 ...
 do iLayer=nSnow+1,nLayers
  ...
  select case(ixRootProfile)
   ...
   case(doubleExp)
    ...
    fracRootLower = 1._rk - 0.5_rk*(exp(-iLayerHeight(iLayer-1)*rootScaleFactor1) + exp(-iLayerHeight(iLayer-1)*rootScaleFactor2) )
    fracRootUpper = 1._rk - 0.5_rk*(exp(-iLayerHeight(iLayer  )*rootScaleFactor1) + exp(-iLayerHeight(iLayer  )*rootScaleFactor2) )
    ! compute the root density
    mLayerRootDensity(iLayer-nSnow) = fracRootUpper - fracRootLower
    !write(*,'(a,10(f11.5,1x))') 'mLayerRootDensity(iLayer-nSnow), fracRootUpper, fracRootLower = ', &
    !                             mLayerRootDensity(iLayer-nSnow), fracRootUpper, fracRootLower
 ...
 error = sum(mLayerRootDensity) - 1._rk
 if (error > 2._rk*epsilon(rootingDepth)) then
  message=trim(message)//'problem with the root density calaculation'
  err=20; return
 else
  mLayerRootDensity = mLayerRootDensity - error/real(nSoil,kind(rk))
 end if

 ! compute fraction of roots in the aquifer
 if(sum(mLayerRootDensity) < 1._rk)then
  scalarAquiferRootFrac = 1._rk - sum(mLayerRootDensity)
 else
  scalarAquiferRootFrac = 0._rk
 end if

 ! check that roots in the aquifer are appropriate
 if ((ixGroundwater /= bigBucket).and.(scalarAquiferRootFrac > 2._rk*epsilon(rootingDepth)))then
  if(scalarAquiferRootFrac < rootTolerance) then
   mLayerRootDensity = mLayerRootDensity + scalarAquiferRootFrac/real(nSoil, kind(rk))
   scalarAquiferRootFrac = 0._rk
  else
   select case(ixRootProfile)
 ...
 ! declare local variables
 integer(i4b)                    :: iLayer              ! loop through layers
 real(rk)                        :: ifcDepthScaleFactor ! depth scaling factor (layer interfaces)
 real(rk)                        :: midDepthScaleFactor ! depth scaling factor (layer midpoints)
 ! initialize error control
 ...
 do iLayer=nSnow,nLayers
  select case(model_decisions(iLookDECISIONS%hc_profile)%iDecision)
   ...
   case(constant)
    ...
    if(iLayer==nSnow)then
     ...
     if(iLayer==nLayers)then
      ...
      iLayerSatHydCond(iLayer-nSnow)   = 0.5_rk * (k_soil(iLayer-nSnow) + k_soil(iLayer+1-nSnow) )
     endif
   ...
   case(powerLaw_profile)
    ...
    if(compactedDepth/iLayerHeight(nLayers) /= 1._rk) then    ! avoid divide by zero
     ifcDepthScaleFactor = ( (1._rk - iLayerHeight(iLayer)/iLayerHeight(nLayers))**(zScale_TOPMODEL - 1._rk) ) / &
                           ( (1._rk -       compactedDepth/iLayerHeight(nLayers))**(zScale_TOPMODEL - 1._rk) )
    else
     ifcDepthScaleFactor = 1.0_rk
    endif                           
    if(iLayer==nSnow)then
     ...
     if(iLayer==nLayers)then
      ...
      iLayerSatHydCond(iLayer-nSnow)   = 0.5_rk * (k_soil(iLayer-nSnow) + k_soil(iLayer+1-nSnow) ) * ifcDepthScaleFactor
     endif
     ! - conductivity at layer midpoints
     if(compactedDepth/iLayerHeight(nLayers) /= 1._rk) then    ! avoid divide by zero
      midDepthScaleFactor = ( (1._rk - mLayerHeight(iLayer)/iLayerHeight(nLayers))**(zScale_TOPMODEL - 1._rk) ) / &
                            ( (1._rk -       compactedDepth/iLayerHeight(nLayers))**(zScale_TOPMODEL - 1._rk) )
     else
      midDepthScaleFactor = 1.0_rk
     endif                            
 ...
 implicit none
 ! input variables
 real(rk),intent(in)             :: bpar_data(:)           ! vector of basin-average model parameters
 ! output variables
 type(var_dlength),intent(inout) :: bvar_data              ! data structure of basin-average model variables
 integer(i4b),intent(out)        :: err                    ! error code
 character(*),intent(out)        :: message                ! error message
 ! internal
 real(rk)                        :: dt                     ! data time step (s)
 integer(i4b)                    :: nTDH                   ! number of points in the time-delay histogram
 integer(i4b)                    :: iFuture                ! index in time delay histogram
 real(rk)                        :: aLambda                ! scale parameter in the Gamma distribution
 real(rk)                        :: tFuture                ! future time (end of step)
 real(rk)                        :: pSave                  ! cumulative probability at the start of the step
 real(rk)                        :: cumProb                ! cumulative probability at the end of the step
 real(rk)                        :: sumFrac                ! sum of runoff fractions in all steps
 real(rk),parameter              :: tolerFrac=0.01_rk      ! tolerance for missing fractional runoff by truncating histogram
 ! initialize error control
 ...
 select case(ixRouting)
  ...
  case(qInstant)
   fractionFuture(1)      = 1._rk
   fractionFuture(2:nTDH) = 0._rk

  ! ** time delay histogram
  case(timeDelay)
   ! initialize
   pSave   = 0._rk ! cumulative probability at the start of the step
   aLambda = routingGammaShape / routingGammaScale
   if(routingGammaShape <= 0._rk .or. aLambda < 0._rk)then
   ...
   do iFuture = 1,nTDH
    ...
    fractionFuture(iFuture) = max(0._rk, cumProb - pSave) ! fraction of runoff in the current step
    ...
    if(fractionFuture(iFuture) < tiny(dt))then
     fractionFuture(iFuture:nTDH) = 0._rk
     exit
    end if
   ...
   if(abs(1._rk - sumFrac) > tolerFrac)then
