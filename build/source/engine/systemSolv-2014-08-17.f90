...
contains
 ...
 allocate(dMat(nState),sMul(nState),rAdd(nState),fluxVec0(nState),aJac(nState,nState),rVec(nState),xInc(nState),stat=err)
 ...
 do iter=1,maxiter
  ...
  !write(*,'(a,1x,i4,1x,100(e16.8,1x))') 'iter, sMul(:)*stateVecTrial(:) = ', iter, sMul(:)*stateVecTrial(:)
  !write(*,'(a,1x,i4,1x,100(e16.8,1x))') 'iter, sMul(:)*stateVecInit(:)  = ', iter, sMul(:)*stateVecInit(:)
  !write(*,'(a,1x,i4,1x,100(f16.8,1x))') 'iter, stateVecTrial            = ', iter, stateVecTrial
 ...
 deallocate(stateVecInit,stateVecTrial,dMat,sMul,rAdd,fluxVec0,aJac,rVec,xInc,stat=err)
 ...
 contains
  ...
  ! print input
  !write(*,'(a,1x,10(e15.5,1x))') 'stateVec(1:10) = ', stateVec(1:10)
  ...
  if(nSnow > 0)then; err=20; message='have not implemented snow layers yet'; return; endif
  ...
  ! ************************************************************************************************
  ! internal subroutine: compute soil compressibility (-) and its derivative w.r.t matric head (m-1)
  ! ************************************************************************************************
  ...
  do iLayer=1,nSoil    ! loop through layers in the soil domain
   ...
   if(iLayer > 1)       aJac(jLayer-nVarSnowSoil,jLayer) = (dt/mLayerDepth(kLayer-1))*( dq_dStateBelow(iLayer-1))
   if(iLayer < nLayers) aJac(jLayer+nVarSnowSoil,jLayer) = (dt/mLayerDepth(kLayer+1))*(-dq_dStateAbove(iLayer))
