...
contains
 ...
 real(summa_prec)                          :: fLiq               ! fraction of liquid water on the vegetation canopy (-)
 real(summa_prec)                          :: vGn_m              ! van Genutchen "m" parameter (-)
 real(summa_prec)                          :: tWat               ! total water on the vegetation canopy (kg m-2)
 real(summa_prec)                          :: scalarTheta        ! liquid water equivalent of total water [liquid water + ice] (-)
 real(summa_prec)                          :: h1,h2              ! used to check depth and height are consistent
 integer(i4b)                      :: nLayers            ! total number of layers
 real(summa_prec)                          :: kappa              ! constant in the freezing curve function (m K-1)
 integer(i4b)                      :: nSnow              ! number of snow layers
 real(summa_prec),parameter                :: xTol=1.e-10_summa_prec     ! small tolerance to address precision issues
 ! --------------------------------------------------------------------------------------------------------
 ...
 do iGRU = 1,nGRU
  do iHRU = 1,gru_struc(iGRU)%hruCount
   ...
   if(scalarCanopyIce > 0._summa_prec .and. scalarCanopyTemp > Tfreeze)then
   ...
   scalarCanopyIce = (1._summa_prec - fLiq)*tWat              ! mass of ice on the canopy (kg m-2)
   ...
   do iLayer=1,nLayers
    ...
    if (iLayer>nSnow) then ! soil layer = no volume expansion
     iSoil       = iLayer - nSnow
     vGn_m       = 1._summa_prec - 1._summa_prec/vGn_n(iSoil)
    ...
    select case(mlayerLayerType(iLayer))
     ...
     case(iname_snow)
      ! (check liquid water)
      if(mLayerVolFracLiq(iLayer) < 0._summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with volumetric fraction of liquid water < 0: layer = ',iLayer; err=20; return; end if
      if(mLayerVolFracLiq(iLayer) > 1._summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with volumetric fraction of liquid water > 1: layer = ',iLayer; err=20; return; end if
      ! (check ice)
      if(mLayerVolFracIce(iLayer) > 0.80_summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with volumetric fraction of ice > 0.80: layer = ',iLayer; err=20; return; end if
      if(mLayerVolFracIce(iLayer) < 0.05_summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with volumetric fraction of ice < 0.05: layer = ',iLayer; err=20; return; end if
      ! check total water
      if(scalarTheta > 0.80_summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with total water fraction [liquid + ice] > 0.80: layer = ',iLayer; err=20; return; end if
      if(scalarTheta < 0.05_summa_prec)then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with total water fraction [liquid + ice] < 0.05: layer = ',iLayer; err=20; return; end if
     ...
     case(iname_soil)
      ...
      ! (check ice)
      if(mLayerVolFracIce(iLayer) < 0._summa_prec                )then; write(message,'(a,1x,i0)') trim(message)//'cannot initialize the model with volumetric fraction of ice < 0: layer = '        ,iLayer; err=20; return; end if
   ...
   do iLayer=1,nLayers
    ...
    if(abs(h1 - h2) > 1.e-6_summa_prec)then
