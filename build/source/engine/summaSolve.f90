...
contains
 ...
 ! --------------------------------------------------------------------------------------------------------------------------------
 ! input: model control
 real(summa_prec),intent(in)             :: dt                       ! length of the time step (seconds)
 ...
 ! input: state vectors
 real(summa_prec),intent(in)             :: stateVecTrial(:)         ! trial state vector
 real(summa_prec),intent(inout)          :: xMin,xMax                ! brackets of the root
 real(summa_prec),intent(in)             :: fScale(:)                ! function scaling vector
 real(summa_prec),intent(in)             :: xScale(:)                ! "variable" scaling vector, i.e., for state variables
 real(summa_prec),intent(in)             :: rVec(:)   ! NOTE: qp     ! residual vector
 real(summa_prec),intent(in)             :: sMul(:)   ! NOTE: qp     ! state vector multiplier (used in the residual calculations)
 real(summa_prec),intent(inout)          :: dMat(:)                  ! diagonal matrix (excludes flux derivatives)
 real(summa_prec),intent(in)             :: fOld                     ! old function evaluation
 ! input: data structures
 ...
 real(summa_prec),intent(inout)          :: dBaseflow_dMatric(:,:)   ! derivative in baseflow w.r.t. matric head (s-1)
 ! output: flux and residual vectors
 real(summa_prec),intent(out)            :: stateVecNew(:)           ! new state vector
 real(summa_prec),intent(out)            :: fluxVecNew(:)            ! new flux vector
 real(summa_prec),intent(out)            :: resSinkNew(:)            ! sink terms on the RHS of the flux equation
 real(summa_prec),intent(out)            :: resVecNew(:) ! NOTE: qp  ! new residual vector
 real(summa_prec),intent(out)            :: fNew                     ! new function evaluation
 ...
 real(summa_prec)                        :: nJac(nState,nState)      ! numerical Jacobian matrix
 real(summa_prec)                        :: aJac(nLeadDim,nState)      ! Jacobian matrix
 real(summa_prec)                        :: aJacScaled(nLeadDim,nState)      ! Jacobian matrix (scaled)
 real(summa_prec)                        :: aJacScaledTemp(nLeadDim,nState)  ! Jacobian matrix (scaled) -- temporary copy since decomposed in lapack
 ! solution/step vectors
 real(summa_prec),dimension(nState)      :: rVecScaled               ! residual vector (scaled)
 real(summa_prec),dimension(nState)      :: newtStepScaled           ! full newton step (scaled)
 ...
 rVecScaled(1:nState) = fScale(:)*real(rVec(:), summa_prec)   ! NOTE: residual vector is in quadruple precision
 ...
 contains
  ...
  real(summa_prec),intent(in)            :: stateVecTrial(:)         ! trial state vector
  real(summa_prec),intent(in)            :: newtStepScaled(:)        ! scaled newton step
  real(summa_prec),intent(in)            :: aJacScaled(:,:)          ! scaled jacobian matrix
  real(summa_prec),intent(in)            :: rVecScaled(:)            ! scaled residual vector
  real(summa_prec),intent(in)            :: fOld                     ! old function value
  ! output
  real(summa_prec),intent(out)           :: stateVecNew(:)           ! new state vector
  real(summa_prec),intent(out)           :: fluxVecNew(:)            ! new flux vector
  real(summa_prec),intent(out)           :: resVecNew(:) ! NOTE: qp  ! new residual vector
  ...
  ! local
  character(len=256)             :: cmessage                 ! error message of downwind routine
  real(summa_prec)                       :: gradScaled(nState)       ! scaled gradient
  real(summa_prec)                       :: xInc(nState)             ! iteration increment (re-scaled to original units of the state vector)
  ...
  real(summa_prec),parameter             :: alpha=1.e-4_summa_prec           ! check on gradient
  real(summa_prec)                       :: xLambda                  ! backtrack magnitude
  real(summa_prec)                       :: xLambdaTemp              ! temporary backtrack magnitude
  real(summa_prec)                       :: slopeInit                ! initial slope
  real(summa_prec)                       :: rhs1,rhs2                ! rhs used to compute the cubic
  real(summa_prec)                       :: aCoef,bCoef              ! coefficients in the cubic
  real(summa_prec)                       :: disc                     ! temporary variable used in cubic
  real(summa_prec)                       :: xLambdaPrev              ! previous lambda value (used in the cubic)
  real(summa_prec)                       :: fPrev                    ! previous function evaluation (used in the cubic)
  ! --------------------------------------------------------------------------------------------------------
  ! initialize error control
  ...
  lineSearch: do iLine=1,maxLineSearch  ! try to refine the function by shrinking the step size
   ...
   if(iLine==1)then
    xLambdaTemp = -slopeInit / (2._summa_prec*(fNew - fOld - slopeInit) )
    if(xLambdaTemp > 0.5_summa_prec*xLambda) xLambdaTemp = 0.5_summa_prec*xLambda
   ...
   ! subsequent backtracks: use cubic
    ...
    if(aCoef==0._summa_prec)then
     xLambdaTemp = -slopeInit/(2._summa_prec*bCoef)

    ! calculate cubic
    else
     disc = bCoef*bCoef - 3._summa_prec*aCoef*slopeInit
     if(disc < 0._summa_prec)then
      xLambdaTemp = 0.5_summa_prec*xLambda
     else
      xLambdaTemp = (-bCoef + sqrt(disc))/(3._summa_prec*aCoef)
     end if
   ...
   ! constrain lambda
   xLambda = max(xLambdaTemp, 0.1_summa_prec*xLambda)
  ...
  ! local variables
  character(len=256)             :: cmessage                 ! error message of downwind routine
  real(summa_prec),parameter             :: relTolerance=0.005_summa_prec    ! force bi-section if trial is slightly larger than (smaller than) xmin (xmax)
  real(summa_prec)                       :: xTolerance               ! relTolerance*(xmax-xmin)
  real(summa_prec)                       :: xInc(nState)             ! iteration increment (re-scaled to original units of the state vector)
  real(summa_prec)                       :: rVec(nState)             ! residual vector (re-scaled to original units of the state equation)
  ...
  real(summa_prec),parameter             :: delX=1._summa_prec               ! trial increment
  !real(summa_prec)                      :: xIncrement(nState)       ! trial increment (not used)
  ! --------------------------------------------------------------------------------------------------------
  ...
  rVec = real(rVecScaled, summa_prec)*real(fScale, summa_prec)

  ! update brackets
  if(rVec(1)<0._summa_prec)then
  ...
  if(xInc(1)*rVec(1) > 0._summa_prec)then
   ...
   ! use bi-section
   stateVecNew(1) = 0.5_summa_prec*(xMin + xMax)
  ...
  if(bracketsDefined)then
   ...
   if(doBisection) stateVecNew(1) = 0.5_summa_prec*(xMin+xMax)
  endif
  ...
  real(summa_prec),intent(in)            :: stateVecTrial(:)         ! trial state vector
  real(summa_prec),intent(out)           :: stateVecNew(:)           ! new state vector
  real(summa_prec),intent(out)           :: xMin,xMax                ! constraints
  ...
  logical(lgt)                   :: feasible                 ! feasibility of the solution
  real(summa_prec),parameter             :: delX=1._summa_prec               ! trial increment
  real(summa_prec)                       :: xIncrement(nState)       ! trial increment
  ! initialize
  ...
  do iCheck=1,nCheck
   ...
   ! update brackets
   if(real(resVecNew(1), summa_prec)<0._summa_prec)then
    xMin = stateVecNew(1)
   else
    xMax = stateVecNew(1)
   endif
  ...
  implicit none
  ! dummies
  real(summa_prec),intent(in)            :: stateVec(:)                ! trial state vector
  real(summa_prec),intent(in)            :: dMat(:)                    ! diagonal matrix
  ! output
  real(summa_prec),intent(out)           :: nJac(:,:)                  ! numerical Jacobian
  integer(i4b),intent(out)       :: err                        ! error code
  character(*),intent(out)       :: message                    ! error message
  ! ----------------------------------------------------------------------------------------------------------
  ! local
  ...
  real(summa_prec),dimension(nState)     :: stateVecPerturbed          ! perturbed state vector
  real(summa_prec),dimension(nState)     :: fluxVecInit,fluxVecJac     ! flux vector (mized units)
  real(summa_prec),dimension(nState)     :: resVecInit,resVecJac ! qp  ! residual vector (mixed units)
  real(summa_prec)                       :: func                       ! function value
  logical(lgt)                   :: feasible                   ! flag to denote the feasibility of the solution
  ...
  do iJac=1,nState
   ...
   select case(ixNumType)
    case(ixNumRes);  nJac(:,iJac) = real(resVecJac - resVecInit, kind(summa_prec) )/dx  ! Jacobian based on residuals
  ...
  character(*),intent(out)        :: message                  ! error message
  ! local variables
  real(summa_prec)                        :: fullJac(nState,nState)   ! full Jacobian matrix
  real(summa_prec)                        :: bandJac(nLeadDim,nState) ! band Jacobian matrix
  ...
  implicit none
  ! input
  real(summa_prec),intent(in)            :: stateVecNew(:)           ! updated state vector
  ! output
  real(summa_prec),intent(out)           :: fluxVecNew(:)            ! updated flux vector
  real(summa_prec),intent(out)           :: resVecNew(:) ! NOTE: qp  ! updated residual vector
  real(summa_prec),intent(out)           :: fNew                     ! new function value
  ...
  implicit none
  ! dummies
  real(summa_prec),intent(in)       :: rVec(:)                ! residual vector (mixed units)
  real(summa_prec),intent(in)       :: xInc(:)                ! iteration increment (mixed units)
  real(summa_prec),intent(in)       :: xVec(:)                ! state vector (mixed units)
  logical(lgt)              :: checkConv              ! flag to denote convergence
  ! locals
  real(summa_prec),dimension(mSoil) :: psiScale               ! scaling factor for matric head
  real(summa_prec),parameter        :: xSmall=1.e-0_summa_prec        ! a small offset
  real(summa_prec),parameter        :: scalarTighten=0.1_summa_prec   ! scaling factor for the scalar solution
  real(summa_prec)                  :: soilWatbalErr          ! error in the soil water balance
  real(summa_prec)                  :: canopy_max             ! absolute value of the residual in canopy water (kg m-2)
  real(summa_prec),dimension(1)     :: energy_max             ! maximum absolute value of the energy residual (J m-3)
  real(summa_prec),dimension(1)     :: liquid_max             ! maximum absolute value of the volumetric liquid water content residual (-)
  real(summa_prec),dimension(1)     :: matric_max             ! maximum absolute value of the matric head iteration increment (m)
  real(summa_prec)                  :: aquifer_max            ! absolute value of the residual in aquifer water (m)
  ...
  if(ixVegHyd/=integerMissing)then
   canopy_max = real(abs(rVec(ixVegHyd)), summa_prec)*iden_water
  ...
  if(size(ixNrgOnly)>0)then
   energy_max = real(maxval(abs( rVec(ixNrgOnly) )), summa_prec)
  ...
  if(size(ixHydOnly)>0)then
   liquid_max = real(maxval(abs( rVec(ixHydOnly) ) ), summa_prec)
  ...
  if(size(ixMatOnly)>0)then
   soilWatBalErr = sum( real(rVec(ixMatOnly), summa_prec)*mLayerDepth(nSnow+ixMatricHead) )
  ...
  if(ixAqWat/=integerMissing)then
   aquifer_max = real(abs(rVec(ixAqWat)), summa_prec)*iden_water
  ...
  ! dummies
  real(summa_prec),intent(in)             :: stateVecTrial(:)             ! trial state vector
  real(summa_prec),intent(inout)          :: xInc(:)                      ! iteration increment
  integer(i4b),intent(out)        :: err                          ! error code
  character(*),intent(out)        :: message                      ! error message
  ! -----------------------------------------------------------------------------------------------------
  ! temporary variables for model constraints
  real(summa_prec)                        :: cInc                         ! constrained temperature increment (K) -- simplified bi-section
  real(summa_prec)                        :: xIncFactor                   ! scaling factor for the iteration increment (-)
  integer(i4b)                    :: iMax(1)                      ! index of maximum temperature
  real(summa_prec)                        :: scalarTemp                   ! temperature of an individual snow layer (K)
  real(summa_prec)                        :: volFracLiq                   ! volumetric liquid water content of an individual snow layer (-)
  ...
  real(summa_prec)                        :: xPsi00                       ! matric head after applying the iteration increment (m)
  real(summa_prec)                        :: TcSoil                       ! critical point when soil begins to freeze (K)
  real(summa_prec)                        :: critDiff                     ! temperature difference from critical (K)
  real(summa_prec),parameter              :: epsT=1.e-7_summa_prec                ! small interval above/below critical (K)
  real(summa_prec),parameter              :: zMaxTempIncrement=1._summa_prec      ! maximum temperature increment (K)
  ...
  if(ixVegNrg/=integerMissing)then
   ...
   if(critDiff > 0._summa_prec)then
  ...
  if(ixVegHyd/=integerMissing)then
   ...
   if(stateVecTrial(ixVegHyd)+xInc(ixVegHyd) < 0._summa_prec)then
    ! scale iteration increment
    cInc       = -0.5_summa_prec*stateVecTrial(ixVegHyd)                                  ! constrained iteration increment (K) -- simplified bi-section
  ...
  if(nSnowOnlyNrg > 0)then
   ...
   checksnow: do iLayer=1,nSnow  ! necessary to ensure that NO layers rise above Tfreeze
    ...
    if(stateVecTrial(iState) + xInc(iState) > Tfreeze)then
     ! scale iteration increment
     cInc       = 0.5_summa_prec*(Tfreeze - stateVecTrial(iState) )        ! constrained temperature increment (K) -- simplified bi-section
  ...
  if(nSnowOnlyHyd>0)then
   ...
   do iLayer=1,nSnow
    ...
    if(-xInc(ixSnowOnlyHyd(iLayer)) > volFracLiq)then
     ...
     xInc(ixSnowOnlyHyd(iLayer)) = -0.5_summa_prec*volFracLiq
    endif
  ...
  if(size(ixMatOnly)>0)then
   do iState=1,size(ixMatOnly)
    ...
    if(xInc(ixLiq) > 1._summa_prec .and. stateVecTrial(ixLiq) > 0._summa_prec)then
     xInc(ixLiq) = 1._summa_prec
