...
contains
 ...
 real(rk),intent(out)       :: dsec                ! seconds
 integer(i4b),intent(out)   :: ih_tz,imin_tz       ! time zone information (hour/minute)
 real(rk),intent(out)       :: dsec_tz             ! time zone information (seconds)
 ...
 ih=0; imin=0; dsec=0._rk; ih_tz=0; imin_tz=0; dsec_tz=0._rk;
 ...
 if(dsec <  0._rk)then; err=20; message=trim(message)//'second < 0';  return; end if
 if(dsec > 60._rk)then; err=20; message=trim(message)//'second > 60'; return; end if
 ...
 if(dsec_tz <    0._rk)then; err=20; message=trim(message)//'time zone second < 0';  return; end if
 if(dsec_tz >   60._rk)then; err=20; message=trim(message)//'time zone second > 60'; return; end if
 ...
 real(rk),intent(in)       :: dsec         ! seconds
 ! output
 real(rk),intent(out)      :: juldayss
  integer(i4b),intent(out) :: err          ! error code
  character(*),intent(out) :: message      ! error message
 ! local variables
 ...
 real(rk)                  :: jfrac        ! fraction of julian day
 ...
 juldayss = real(julday,kind(rk)) + jfrac
 ...
 ! input variables
 real(rk), intent(in)          :: julday       ! julian day
 ...
 real(rk),parameter           :: hr_per_day = 24.0_rk
 real(rk),parameter           :: min_per_hour = 60.0_rk
 ...
 real(rk)              :: frac_day  ! fractional day
 real(rk)              :: remainder ! remainder of modulus operation
 ...
 real(rk)                       :: elapsedSec                         ! elapsed time in seconds
 ! local variables
 ...
 elapsedSec = (endTime(8)-startTime(8))*.001_rk + (endTime(7)-startTime(7)) + (endTime(6)-startTime(6))*secprmin + (endTime(5)-startTime(5))*secprhour
 ...
 integer(i4b),intent(in)   :: ih,imin      ! hour, minute
 real(rk),intent(in)       :: dsec         ! seconds
 real(rk)                  :: fracDay      ! fraction of a day
 ! local variable

 fracDay = (real(ih,kind(rk))*secprhour + real(imin,kind(rk))*secprmin + dsec) / secprday
