...
contains
 ...
 real(rk),dimension(4)           :: zmax_lower          ! lower value of maximum layer depth
 real(rk),dimension(4)           :: zmax_upper          ! upper value of maximum layer depth
 real(rk)                        :: zmaxCheck           ! value of zmax for a given snow layer
 ...
 real(rk)                        :: depthOriginal       ! original layer depth before sub-division (m)
 real(rk),parameter              :: fracTop=0.5_rk      ! fraction of old layer used for the top layer
 real(rk)                        :: surfaceLayerSoilTemp  ! temperature of the top soil layer (K)
 real(rk)                        :: maxFrozenSnowTemp   ! maximum temperature when effectively all water is frozen (K)
 ...
 real(rk),parameter              :: verySmall=1.e-10_rk ! a very small number (used for error checking)
 ! --------------------------------------------------------------------------------------------------------
 ...
 if(nSnow==0)then
  ...
  if(createLayer)then
   ...
   mLayerVolFracIce(1) = (1._rk - fracLiq)*volFracWater*(iden_water/iden_ice)   ! volumetric fraction of ice (-)
    ...
    select case(model_decisions(iLookDECISIONS%alb_method)%iDecision)
     ...
     case(variableDecay)
      ...
      prog_data%var(iLookPROG%scalarSnowAlbedo)%dat(1)                  = (        mpar_data%var(iLookPARAM%Frad_vis)%dat(1))*mpar_data%var(iLookPARAM%albedoMaxVisible)%dat(1) + &
                                                                          (1._rk - mpar_data%var(iLookPARAM%Frad_vis)%dat(1))*mpar_data%var(iLookPARAM%albedoMaxNearIR)%dat(1)
 ...
 ! ***** sub-divide snow layers, if necessary
  ...
  do iLayer=1,nCheck
   ...
   if(prog_data%var(iLookPROG%mLayerDepth)%dat(iLayer) > zmaxCheck)then
    ...
    mLayerDepth(iLayer+1) = (1._rk - fracTop)*depthOriginal
 ...
 if(divideLayer)then
  ...
  do jLayer=1,nLayers
   ...
   mLayerHeight(jLayer) = (iLayerHeight(jLayer-1) + iLayerHeight(jLayer))/2._rk
  end do
 ...
 do ivar=1,size(metaStruct)
  ...
  select type(dataStruct)
   ...
   type is (var_dlength)
    ...
    call cloneStruc(tempVec_rk, ix_lower, source=dataStruct%var(ivar)%dat, err=err, message=cmessage)
    ...
    if(stateVariable)then
     if(ix_upper > 0)then  ! (only copy data if the vector exists -- can be a variable for snow, with no layers)
      if(ix_divide > 0)then
       dataStruct%var(ivar)%dat(1:ix_divide)            = tempVec_rk(1:ix_divide)  ! copy data
       dataStruct%var(ivar)%dat(ix_divide+1)            = tempVec_rk(ix_divide)    ! repeat data for the sub-divided layer
      end if
      if(ix_upper > ix_divide) &
       dataStruct%var(ivar)%dat(ix_divide+2:ix_upper+1) = tempVec_rk(ix_divide+1:ix_upper)  ! copy data
