...
contains
 ...
 real(rk), DIMENSION(:), INTENT(OUT) :: y2
 integer(i4b),intent(out)      :: err
 character(*),intent(out)      :: message
 ! local variables
 character(len=128) :: cmessage
 INTEGER(I4B) :: n
 real(rk), DIMENSION(size(x)) :: a,b,c,r
 ...
 r(1:n-1)=6.0_rk*((y(2:n)-y(1:n-1))/c(1:n-1))
 r(2:n-1)=r(2:n-1)-r(1:n-2)
 a(2:n-1)=c(1:n-2)
 b(2:n-1)=2.0_rk*(c(2:n-1)+a(2:n-1))
 b(1)=1.0
 b(n)=1.0
 if (yp1 > 0.99e30_rk) then
  r(1)=0.0
  c(1)=0.0
 else
  r(1)=(3.0_rk/(x(2)-x(1)))*((y(2)-y(1))/(x(2)-x(1))-yp1)
  c(1)=0.5
 end if
 if (ypn > 0.99e30_rk) then
  r(n)=0.0
  a(n)=0.0
 else
  r(n)=(-3.0_rk/(x(n)-x(n-1)))*((y(n)-y(n-1))/(x(n)-x(n-1))-ypn)
  a(n)=0.5
 end if
 ...
 real(rk), DIMENSION(:), INTENT(IN)  :: xa,ya,y2a
 real(rk), INTENT(IN)  :: x
 ...
 khi=klo+1
 h=xa(khi)-xa(klo)
 if (h == 0.0_rk) then; err=20; message="f-splint/badXinput"; return; end if
 a=(xa(khi)-x)/h
 b=(x-xa(klo))/h
 y=a*ya(klo)+b*ya(khi)+((a**3-a)*y2a(klo)+(b**3-b)*y2a(khi))*(h**2)/6.0_rk
 ...
 IMPLICIT NONE
 real(rk), DIMENSION(:), INTENT(IN) :: xx
 real(rk), INTENT(IN) :: x
 INTEGER(I4B) :: locate
 ...
 IMPLICIT NONE
 ! dummy variables
 real(rk), DIMENSION(:), INTENT(IN) :: a,b,c,r
 real(rk), DIMENSION(:), INTENT(OUT) :: u
 integer(i4b),intent(out)      :: err
 character(*),intent(out)      :: message
 ! local variables
 real(rk), DIMENSION(size(b)) :: gam
 INTEGER(I4B) :: n,j
 real(rk) :: bet
 ! initialize error control
 ...
 ! start procedure
 bet=b(1)
 if (bet == 0.0_rk) then; err=20; message="f-tridag/errorAtCodeStage-1"; return; end if
 u(1)=r(1)/bet
 do j=2,n
  gam(j)=c(j-1)/bet
  bet=b(j)-a(j-1)*gam(j)
  if (bet == 0.0_rk) then; err=20; message="f-tridag/errorAtCodeStage-2"; return; end if
