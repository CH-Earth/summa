...
module computJacobFidaCpVar_module
...
private
public::computJacobFidaCpVar
contains

 ! **********************************************************************************************************
 ! public subroutine computJacobFidaCpVar: compute the Jacobian matrix
 ! **********************************************************************************************************
 subroutine computJacobFidaCpVar(&
                        ! input: model control
                        cj,                         & ! intent(in)
 ...
 real(rkind),intent(in)               :: mLayerVolFracWat(:)
 ...
 err=0; message='computJacobFidaCpVar/'
 ...
 beta1 = iden_water * Cp_Ice - iden_water * iden_air * Cp_air / iden_Ice
 beta2 = iden_water * Cp_water -  iden_air * Cp_air
 gamma1 = beta2 - beta1
 ...
 do iLayer=1,nLayers
   select case(layerType(iLayer))
    case(iname_snow); liqSnow = mLayerFracLiqSnow(iLayer)
    case default; liqSnow = 0
   end select
  if(ixSnowSoilNrg(iLayer)/=integerMissing) then
       dMat(ixSnowSoilNrg(iLayer)) = ( mLayerVolHtCapBulk(iLayer) + (LH_fus*iden_water + gamma1*mLayerTemp(iLayer))*mLayerdTheta_dTk(iLayer) ) * cj &
                                     ...
                                     + mLayerVolFracWatPrime(iLayer) * ( beta1 + gamma1*liqSnow + gamma1*dFracLiqSnow_dTk(iLayer)*mLayerTemp(iLayer) ) &
                                     + gamma1*mLayerTempPrime(iLayer) * ( mLayerd2Theta_dTk2(iLayer)*mLayerTemp(iLayer) + 2*mLayerdTheta_dTk(iLayer) )   
  end if
 end do
...
! print *, '2'
 ...
 select case(ixMatrix)
  ...
  case(ixFullMatrix)  ! ixFullMatrix ixBandMatrix
   ...
   if(nSnowOnlyHyd>0)then
    ...
    !   print *, '3' 
       aJac(nrgState,watState) = ( -LH_fus*iden_water*(1._rkind - mLayerFracLiqSnow(iLayer)) &
                                 + ( beta1 + gamma1*mLayerFracLiqSnow(iLayer) ) * mLayerTemp(iLayer) ) * cj  &        
                                 + LH_fus*iden_water * mLayerTempPrime(iLayer) * dFracLiqSnow_dTk(iLayer) &
                                 + ( gamma1*dFracLiqSnow_dTk(iLayer)*mLayerTemp(iLayer) + beta1 + gamma1*mLayerFracLiqSnow(iLayer) ) &
                                    * mLayerTempPrime(iLayer)   ! (dF/dLiq)
 ...
 !     print *, '4'
       aJac(nrgState,watState) = dVolTot_dPsi0(iLayer)*(beta2*mLayerTemp(iLayer) - LH_fus*iden_water)*cj  &
                               + (beta2 - LH_fus*iden_water)*d2VolTot_d2Psi0(iLayer)*mLayerMatricHeadPrime(iLayer) &
                               + beta2*mLayerTempPrime(iLayer)*dVolTot_dPsi0(iLayer)   ! dNrg/dMat (J m-3 m-1) -- dMat changes volumetric water, and hence ice content
      else
 ...
 end subroutine computJacobFidaCpVar
...
end module computJacobFidaCpVar_module
