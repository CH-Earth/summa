...
contains
 ...
 integer(i4b),parameter               :: n_inc=3              ! minimum number of iterations to increase time step
 integer(i4b),parameter               :: n_dec=5              ! maximum number of iterations to increase time step
 real(dp),parameter                   :: F_inc = 1.1_dp       ! factor used to increase time step
 ...
 do  ! continuous do statement with exit clause (alternative to "while")
  ...
  print*, 'scalarSnowfall = ', scalarSnowfall
  ...
  call sfcMassFlx(mLayerTemp(1),err,cmessage)
  if(err/=0)then; err=10; message=trim(message)//trim(cmessage); return; endif
  ...
  do iter=1,maxiter
   ...
   if(niter>15) stop 'number of iterations is greater than 10!!!'
   if(niter>15) printflag=.true.
   if(printflag) print*, '****************************'
   ...
   print*, 'mLayerPoreSpace = ', mLayerPoreSpace
   print*, 'mLayerRelsat = ', mLayerRelsat
   ...
   !print*, 'abs(mLayerMatricIncrement)', abs(mLayerMatricIncrement(1:10))
   !print*, 'matric_tol*abs(mLayerMatricHeadNew)', matric_tol*abs(mLayerMatricHeadNew(1:10))
   !print*, 'abs(mLayerMatricIncrement) - matric_tol*abs(mLayerMatricHeadNew)', abs(mLayerMatricIncrement(1:10)) - matric_tol*abs(mLayerMatricHeadNew(1:10))
   !pause
