...
contains
 ...
 do  ! continuous do statement with exit clause (alternative to "while")
  ...
  do iter=1,maxiter
   ...
   ! ***** compute the derivative in the freezing curve (K-1)
   do iLayer=1,nLayers
    ...
    Tcrit(iLayer) = crit_soilT(theta,theta_res,theta_sat,vGn_alpha,vGn_n,vGn_m)
    ! compute the derivatuve of the freezing curve w.r.t. temperature (K)
    if(mLayerTempIter(iLayer)<Tcrit(iLayer))then
   ...
   where(mLayerTempIter>=Tcrit .and. mLayerTempNew<Tcrit) mLayerTempNew=Tcrit
   where(mLayerTempIter<=Tcrit .and. mLayerTempNew>Tcrit) mLayerTempNew=Tcrit+tiny(Tcrit)
   print*, 'mLayerTempIter = ', mLayerTempIter
   print*, 'mLayerTempNew = ', mLayerTempNew
   ! compute change in liquid water and ice
   where(mLayerTempIter<Tcrit)
    ! compute the new volumetric liquid water content
    mLayerVolFracLiqNew = theta_res + (theta_sat - theta_res) / &
                           (1._dp + (vGn_alpha*kappa*(mLayerTempNew - Tfreeze))**vGn_n)**vGn_m
    ! compute the new volumetric ice content
    mLayerVolFracIceNew = mLayerVolFracIceIter - (mLayerVolFracLiqNew - mLayerVolFracLiqIter)*(iden_water/iden_ice)
   elsewhere
   ...
   print*, 'del_ice = ', mLayerVolFracIceNew - mLayerVolFracIceIter
 ...
 deallocate(mLayerDelTemp,mLayerCrossFlag,mLayerNrgMelt,mLayerDelIce,&
            nrg_diff,matric_diff,Tcrit,stat=err)
