...
contains
 ...
 ! local pointers to evaporation/dew
 ...
 real(dp)                      :: inflvol                    ! volumetric fraction of liquid water associated with infiltrating flux (-)
 real(dp)                      :: crit_Tk                    ! emperature when the fraction of liquid water is at cfrac (K)
 real(dp)                      :: critvol                    ! volumetric fraction of water required to raise temperatures to Tfreeze (-)
 real(dp)                      :: fracliq                    ! fraction of the infiltrating flux that is liquid (-)
 real(dp)                      :: liqFlux                    ! unfrozen liquid water flux (m s-1)
 real(dp)                      :: liq_vol                    ! volume of the infiltrating flux of liquid water (-)
 ...
 ! assign pointers to evaporation/dew
 ...
 iLayerLiquidWaterFlux(0) = (rainfall + scalarMassLiquid)/iden_water

 ! check the meltwater exponent is >=1
 if(mw_exp<1._dp)then; err=20; message=trim(message)//'meltwater exponent < 1'; return; endif

 print*, 'iLayerLiquidWaterFlux = ', iLayerLiquidWaterFlux

 do iLayer=1,nSnow
  ! *******************************************************************************************************
  ! part one: compute the change in volumetric ice content (-) and the infiltrating liquid flux (m s-1)
  ! *******************************************************************************************************
  if(iLayerLiquidWaterFlux(iLayer-1)>0._dp)then
   ...
   inflvol = dt*ifcFlux/mLayerDepth(iLayer)
   ! compute the temperature when the fraction of liquid water is at cfrac
   crit_Tk = max(mLayerTempIter(iLayer),templiquid(cfrac,snowfrz_scale))
   ! compute the volumetric fraction of water required to raise temperatures to Tfreeze (-)
   critvol = (mLayerVolFracIceIter(iLayer)*Cp_ice/LH_fus)*(crit_Tk - mLayerTempIter(iLayer))
   ! compute the fraction of the infiltrating flux that is liquid (-)
   fracliq = 1._dp - min(critvol/inflvol, 1._dp)
   ! compute the infiltrating flux of liquid water (m s-1)
   liqFlux = fracliq*ifcFlux
   ! compute the volume of the infiltrating flux of liquid water (-)
   liq_vol = fracliq*inflvol
   ! adjust time step if necessary
   if(liq_vol>0.1_dp*mLayerVolFracAir(iLayer))then
   ...
   mLayerInfilFreezeNew(iLayer) = (iden_water/iden_ice)*(inflvol - liq_vol)
  ...
  if(mLayerVolFracLiq(iLayer)+liq_vol > Fcapil*mLayerPoreSpace(iLayer))then 
   ...
   mLayerVolFracLiqNew(iLayer)   = mLayerVolFracLiq(iLayer) + liq_vol
  endif
 end do
 ...
 real(dp),allocatable          :: tridagSolution(:)        ! solution to the tri-diagonal system (units either m or -)
 integer(i4b),allocatable      :: itypeSolution(:)         ! type of solution used (moisture-based or mixed form of RE)
 integer(i4b),parameter        :: ix_theta=1001            ! look-up value for theta form of RE
 integer(i4b),parameter        :: ix_mixed=1002            ! look-up value for mixed form of RE
 ...
 real(dp)                      :: lowerBoundTheta          ! volumetric liquid water content at the lower boundary (-)
 real(dp)                      :: upperBoundTheta          ! volumetric liquid water content at the lower boundary (-)
 real(dp)                      :: sink                     ! sink term associated with evaporation/dew (-)
 ...
 allocate(rv_temp(nSoil),tridagSolution(nSoil),itypeSolution(nSoil),stat=err)
 ...
 if(nSnow==0)then
  sink = (scalarMassLiquid/iden_water) * dt/mLayerDepth(1)
  !print*, 'sink = ', sink
 else
  sink = 0._dp
 endif
 ...
 do iLayer=1,nSoil
  ...
  !print*, iLayer, mLayerVolFracLiqIter(iLayer), 0.5_dp*(theta_sat+theta_res)

  ! switch between the moisture-based and mixed form of Richards' equation
  if(mLayerVolFracLiqIter(iLayer) < theta_res)then
  !if(mLayerVolFracLiqIter(iLayer) < 0.5_dp*(theta_sat+theta_res))then

   ! ************************************************************************************************
   ! ***** USE THE MOISTURE-BASED FORM OF RICHARDS' EQUATION ****************************************
   ! ************************************************************************************************
   itypeSolution(iLayer)=ix_theta
   ! compute the common elements of the RHS vector
   rv_temp(iLayer) = mLayerVolFracLiq(iLayer) - &
                     (iden_ice/iden_water)*(mLayerVolFracIceIter(iLayer) - mLayerVolFracIce(iLayer))
   ! compute the off-diagonal elements
   if(iLayer<nSoil) d_p1(iLayer)   = -dt_dldz(iLayer)*iLayerHydCond(iLayer)/mLayerdTheta_dPsi(iLayer)
   if(iLayer>1)     d_m1(iLayer-1) = -dt_dudz(iLayer)*iLayerHydCond(iLayer-1)/mLayerdTheta_dPsi(iLayer)
   ! ***** define the diagonal elements and the RHS vector
   if(iLayer==nSoil)then
    ! ** lower-most layer
    if(bc_lower==diriclet)then       ! (diriclet boundary condition)
     lowerBoundTheta = volFracLiq(lowerBoundHead,vGn_alpha,theta_res,theta_sat,vGn_n,vGn_m)
     diag(iLayer) = 1._dp + dt_dudz(iLayer)*iLayerHydCond(iLayer-1)/mLayerdTheta_dPsi(iLayer) + &
                            dt_dldz(iLayer)*iLayerHydCond(iLayer)/mLayerdTheta_dPsi(iLayer)
     rvec(iLayer) = rv_temp(iLayer) + (dt/mLayerDepth(iLayer))*(iLayerHydCond(iLayer-1) - iLayerHydCond(iLayer)) + &
                     (dt_dldz(iLayer)*iLayerHydCond(iLayer)/mLayerdTheta_dPsi(iLayer))*lowerBoundTheta
   ...
   elseif(iLayer==1)then
    ...
    if(bc_upper==diriclet)then       ! (diriclet boundary condition)
     upperBoundTheta = volFracLiq(upperBoundHead,vGn_alpha,theta_res,theta_sat,vGn_n,vGn_m)
     diag(iLayer) = 1._dp + dt_dldz(iLayer)*iLayerHydCond(iLayer)/mLayerdTheta_dPsi(iLayer) + &
                            dt_dudz(iLayer)*iLayerHydCond(iLayer-1)/mLayerdTheta_dPsi(iLayer)
     rvec(iLayer) = rv_temp(iLayer) + (dt/mLayerDepth(iLayer))*(iLayerHydCond(iLayer-1) - iLayerHydCond(iLayer)) + &
                    (dt_dudz(iLayer)*iLayerHydCond(iLayer-1)/mLayerdTheta_dPsi(iLayer))*upperBoundTheta + sink
    elseif(bc_upper==neumann)then    ! (Neumann boundary conditions)
     diag(iLayer) = 1._dp + dt_dldz(iLayer)*iLayerHydCond(iLayer)/mLayerdTheta_dPsi(iLayer)
     rvec(iLayer) = rv_temp(iLayer) + (dt/mLayerDepth(iLayer))*(q_top - iLayerHydCond(iLayer)) + sink
    else
  ...
  else ! (if mLayerdTheta_dPsi(iLayer)<0.1_dp)

   ! ************************************************************************************************
   ! ***** USE THE MIXED FORM OF RICHARDS' EQUATION *************************************************
   ! ************************************************************************************************
   itypeSolution(iLayer)=ix_mixed
   ! compute the common elements in the RHS vector (-)
   rv_temp(iLayer) = mLayerdTheta_dPsi(iLayer)*mLayerMatricHeadIter(iLayer) &
                     - (iden_ice/iden_water)*(mLayerVolFracIceIter(iLayer) - mLayerVolFracIce(iLayer)) &
                     - (mLayerVolFracLiqIter(iLayer) - mLayerVolFracLiq(iLayer))
   ! compute the off-diagonal elements
   ...
   elseif(iLayer==1)then 
    ...
    if(bc_upper==diriclet)then       ! (diriclet boundary condition)
     diag(iLayer) = mLayerdTheta_dPsi(iLayer) + dt_dldz(iLayer)*iLayerHydCond(iLayer) + dt_dudz(iLayer)*iLayerHydCond(iLayer-1)
     rvec(iLayer) = rv_temp(iLayer) + (dt/mLayerDepth(iLayer))*(iLayerHydCond(iLayer-1) - iLayerHydCond(iLayer)) + &
                     dt_dudz(iLayer)*iLayerHydCond(iLayer-1)*upperBoundHead + sink
    elseif(bc_upper==neumann)then    ! (Neumann boundary conditions)
     diag(iLayer) = mLayerdTheta_dPsi(iLayer) + dt_dldz(iLayer)*iLayerHydCond(iLayer)
     rvec(iLayer) = rv_temp(iLayer) + (dt/mLayerDepth(iLayer))*(q_top - iLayerHydCond(iLayer)) + sink
     !print*, '(dt/mLayerDepth(iLayer))*(q_top - iLayerHydCond(iLayer)), sink = ', (dt/mLayerDepth(iLayer))*(q_top - iLayerHydCond(iLayer)), sink
    else
     err=50; message=trim(message)//'cannot identify the type of boundary conditions'
    endif
   else
  ...
  endif   ! (switch between the moisture-based form and the mixed form of Richards' equation)

 end do  ! (looping through layers)

 ! check the values
 !print*, 'input to tridiag'
 !do iLayer=1,10
 ...
 call tridag(d_m1,diag,d_p1,rvec,tridagSolution,err,cmessage)
 if(err/=0)then; message=trim(message)//trim(cmessage); return; endif

 ! compute matric head and volumetric fraction of liquid water
 do iLayer=1,nSoil
  !print*, iLayer, itypeSolution(iLayer), tridagSolution(iLayer)
  ! update variables for the moisture-based form of Richards' equation
  if(itypeSolution(iLayer)==ix_theta) then
   mLayerMatricHeadNew(iLayer) = matricHead(tridagSolution(iLayer),vGn_alpha,theta_res,theta_sat,vGn_n,vGn_m)
   mLayervolFracLiqNew(iLayer) = min(tridagSolution(iLayer),theta_sat)
  ! update variables for the mixed form of Richards' equation
  elseif(itypeSolution(iLayer)==ix_mixed)then
   mLayerMatricHeadNew(iLayer) = tridagSolution(iLayer)
   mLayervolFracLiqNew(iLayer) = volFracLiq(tridagSolution(iLayer),vGn_alpha,theta_res,theta_sat,vGn_n,vGn_m)
  else
   stop 'case not found'
  endif
 end do
 ...
 ! deallocate vectors
 deallocate(dz_node,dt_dudz,dt_dldz,rv_temp,tridagSolution,itypeSolution,d_m1,diag,d_p1,rvec,stat=err)
