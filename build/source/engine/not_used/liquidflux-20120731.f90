...
contains
 ...
 real(dp),dimension(size(mLayerMatricHeadIter))   :: dHydCond_dMatric           ! derivative in hydraulic conductivity at layer mid-points w.r.t. matric head
 real(dp)                                         :: dHydCond_dMatric__test     ! derivative in hydraulic conductivity at layer mid-points w.r.t. matric head
 real(dp)                                         :: mLayerHydCondFDIncr1       ! finite difference increment for hydraulic conductivity at layer mid-points
 real(dp)                                         :: mLayerHydCondFDIncr2       ! finite difference increment for hydraulic conductivity at layer mid-points
 real(dp)                                         :: iLayerHydCondFDIncr1       ! finite difference increment for hydraulic conductivity at layer interfaces
 real(dp)                                         :: iLayerHydCondFDIncr2       ! finite difference increment for hydraulic conductivity at layer interfaces
 real(dp),dimension(0:size(mLayerMatricHeadIter)) :: dHydCondIface_dMatricAbove ! derivative in hydraulic conductivity at layer interfaces w.r.t. matric head in the upper layer
 real(dp),dimension(0:size(mLayerMatricHeadIter)) :: dHydCondIface_dMatricBelow ! derivative in hydraulic conductivity at layer interfaces w.r.t. matric head in the upper layer
 real(dp)                                         :: dHydCondIface_dMatricAbove__test ! derivative in hydraulic conductivity at layer interfaces w.r.t. matric head in the upper layer
 real(dp)                                         :: dHydCondIface_dMatricBelow__test ! derivative in hydraulic conductivity at layer interfaces w.r.t. matric head in the upper layer
 ...
 real(dp)                                         :: dq_dMatricAbove__test      ! change in the flux in layer interfaces w.r.t. matric head in the layer above
 real(dp)                                         :: dq_dMatricBelow__test      ! change in the flux in layer interfaces w.r.t. matric head in the layer above
 ...
 call dq_dMatHead(dMethod,              & ! intent(in): method used to compute derivatives (analytical or numerical)
 ...
 ! compute the derivative in hydraulic conductivity at layer mid-points w.r.t. matric head
 do iLayer=1,nSoil
  dHydCond_dMatric(iLayer) = dHydCond_dPsi(mLayerMatricHeadIter(iLayer),k_soil,vGn_alpha,vGn_n,vGn_m,.true.)  ! analytical
  dHydCond_dMatric__test   = dHydCond_dPsi(mLayerMatricHeadIter(iLayer),k_soil,vGn_alpha,vGn_n,vGn_m,.false.) ! numerical
  if( abs(log(dHydCond_dMatric(iLayer)) - log(dHydCond_dMatric__test)) > 1.e-3_dp)then
   print*, 'iLayer, dHydCond_dMatric(iLayer), dHydCond_dMatric__test = ', iLayer, dHydCond_dMatric(iLayer), dHydCond_dMatric__test
   err=20; message=trim(message)//'numerical and analytical derivatives for dHydCond_dPsi do not match'; return
  endif
 end do

 ! compute the derivative in hydraulic conductivity at layer interfaces w.r.t. matric head
 do iLayer=0,nSoil  ! loop through interfaces
  ...
  if(iLayer==0)then  ! (upper boundary)
   ...
   dq_dMatricAbove__test   = -huge(k_soil)
   if(bc_upper==diriclet)then      ! head boundary
    ...
    flux0 = -iLayerHydCond(0)*( mLayerMatricHeadIter(1)     - upperBoundHead) / (mLayerDepth(1)*0.5_dp) + iLayerHydCond(0)
    flux1 = -iLayerHydCond(0)*((mLayerMatricHeadIter(1)+dx) - upperBoundHead) / (mLayerDepth(1)*0.5_dp) + iLayerHydCond(0)
    dq_dMatricBelow__test = (flux1 - flux0)/dx
   elseif(bc_upper==neumann)then
    dq_dMatricBelow(iLayer) = 0._dp
    dq_dMatricBelow__test = 0._dp
   else
  ...
  elseif(iLayer==nSoil)then  ! (lower boundary)
   ...
   dq_dMatricBelow__test   = -huge(k_soil)
   if(bc_lower==diriclet)then      ! head boundary
    ! derivatives in the flux w.r.t. matric head
    dq_dMatricAbove(iLayer) = iLayerHydCond(nSoil)/(mLayerDepth(nSoil)/2._dp)
    ! compute numerical derivatives
    flux0 = -iLayerHydCond(nSoil)*(lowerBoundHead -  mLayerMatricHeadIter(nSoil)    ) / (mLayerDepth(nSoil)*0.5_dp) + iLayerHydCond(nSoil)
    flux1 = -iLayerHydCond(nSoil)*(lowerBoundHead - (mLayerMatricHeadIter(nSoil)+dx)) / (mLayerDepth(nSoil)*0.5_dp) + iLayerHydCond(nSoil)
    dq_dMatricAbove__test = (flux1 - flux0)/dx
   elseif(bc_lower==neumann)then  ! flux boundary 
    ! derivatives in the flux w.r.t. matric head
    dq_dMatricAbove(iLayer) = kAnisotropic*k_soil * exp(mLayerMatricHeadIter(nSoil)/zScale - mLayerHeight(nSoil)/zScale)/zScale
    ! compute numerical derivatives
    flux0 = kAnisotropic*k_soil * exp(-(mLayerHeight(nSoil) -  mLayerMatricHeadIter(nSoil)    )/zScale)
    flux1 = kAnisotropic*k_soil * exp(-(mLayerHeight(nSoil) - (mLayerMatricHeadIter(nSoil)+dx))/zScale)
    dq_dMatricAbove__test = (flux1 - flux0)/dx
   else
  ...
  ! ***** internal layers
   ...
   dHydCondIface_dMatricAbove(iLayer) = dHydCond_dMatric(iLayer)  *mLayerHydCond(iLayer+1) * 0.5_dp*(mLayerHydCond(iLayer)*mLayerHydCond(iLayer+1))**(-0.5_dp)
   dHydCondIface_dMatricBelow(iLayer) = dHydCond_dMatric(iLayer+1)*mLayerHydCond(iLayer)   * 0.5_dp*(mLayerHydCond(iLayer)*mLayerHydCond(iLayer+1))**(-0.5_dp)
   ! spatial differences in matric head and height
   dPsi  = mLayerMatricHeadIter(iLayer+1) - mLayerMatricHeadIter(iLayer)
   dz    = mLayerHeight(iLayer+1) - mLayerHeight(iLayer)
   ! derivatives in the flux w.r.t. matric head
   dq_dMatricAbove(iLayer) = -dHydCondIface_dMatricAbove(iLayer)*dPsi/dz + iLayerHydCond(iLayer)/dz + dHydCondIface_dMatricAbove(iLayer)
   dq_dMatricBelow(iLayer) = -dHydCondIface_dMatricBelow(iLayer)*dPsi/dz - iLayerHydCond(iLayer)/dz + dHydCondIface_dMatricBelow(iLayer)
   ! compute numerical derivatives
   flux0 = darcyFlux(mLayerMatricHeadIter(iLayer),   mLayerMatricHeadIter(iLayer+1)   ,mLayerHeight(iLayer),mLayerHeight(iLayer+1),k_soil,vGn_alpha,vGn_n,vGn_m)
   flux1 = darcyFlux(mLayerMatricHeadIter(iLayer)+dx,mLayerMatricHeadIter(iLayer+1)   ,mLayerHeight(iLayer),mLayerHeight(iLayer+1),k_soil,vGn_alpha,vGn_n,vGn_m)
   flux2 = darcyFlux(mLayerMatricHeadIter(iLayer),   mLayerMatricHeadIter(iLayer+1)+dx,mLayerHeight(iLayer),mLayerHeight(iLayer+1),k_soil,vGn_alpha,vGn_n,vGn_m)
   dq_dMatricAbove__test = (flux1 - flux0)/dx
   dq_dMatricBelow__test = (flux2 - flux0)/dx
  endif
  write(*,'(i4,1x,4(e20.10,1x))') iLayer, dq_dMatricAbove(iLayer), dq_dMatricAbove__test, dq_dMatricBelow(iLayer), dq_dMatricBelow__test
 end do
 ...
 pause ' computing derivative in hydraulic conductivity w.r.t. matric head'
