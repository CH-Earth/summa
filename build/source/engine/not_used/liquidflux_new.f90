...
contains
 ...
 real(dp),parameter            :: atol=1.d-2                 ! absolute iteration tolerance (-)
 ...
 real(dp),pointer              :: iLayerInitLiqFlux(:)     ! liquid flux at layer interfaces at the start of the time step (m s-1)
 ! local pointers to model index variables
 integer(i4b),pointer          :: layerType(:)             ! type of the layer (ix_soil or ix_snow)
 ! define local variables
 ...
 mLayerInitTranspire => mvar_data%var(iLookMVAR%mLayerTranspire)%dat    ! (soil only) ! transpiration loss from each soil layer at start-of-step (m s-1)
 ...
 iLayerInitLiqFlux   => mvar_data%var(iLookMVAR%iLayerInitLiqFlux)%dat           ! liquid flux at layer interfaces at the start of the time step (m s-1)
 ...
 if(iter==1)then
  ...
  call iLayer_liq(mLayerMatricHead,  & ! intent(in): trial temperature (K)
                  iLayerInitLiqFlux, & ! intent(out): energy flux at the layer interfaces (W m-2)
                  err,cmessage)        ! intent(out): error control
  if(err/=0)then; message=trim(message)//trim(cmessage); return; endif
 endif 
 ...
 contains
  ...
  if(bc_lower==neumann)then
   q_bot = mLayerHydCond(nSoil) 
   iLayerHydCond(nSoil) = q_bot
  endif
  ...
  ! ************************************************************************************************
  subroutine iLayer_liq(mLayerMatricHeadTrial, & ! intent(in): matric head (m)
  ...
  select case(bc_upper)
   case(neumann);  iLayerLiqFlux(0) = iLayerHydCond(0)
   case(diriclet)
  ...
  select case(bc_lower)
   case(neumann);  iLayerLiqFlux(nSoil) = iLayerHydCond(nSoil)
   case(diriclet)
    cflux = -iLayerHydCond(nSoil)*(lowerBoundHead - mLayerMatricHeadTrial(nSoil)) / (mLayerDepth(nSoil)*0.5_dp)
    print*, 'diff, cflux = ', lowerBoundHead - mLayerMatricHeadTrial(nSoil), cflux
  ...
  ! local variables
  character(LEN=256)            :: cmessage                  ! error message of downwind routine 
  real(dp)                      :: iLayerLiqFlux(0:size(mLayerMatricHeadIter)) ! liquid flux at layer interfaces (m s-1) 
  integer(i4b)                  :: iSoil                     ! index of soil layers
  ! initialize error control
  err=0; message="vlFrcLiqRes/"

  ! compute liquid water fluxes at layer interfaces
  call iLayer_liq(mLayerMatricHeadTrial, & ! intent(in): trial temperature (K)
  ...
  do iSoil=1,nSoil
   rvec(iSoil) = mLayerVolFracLiqTrial(iSoil) - &
                 ( &
                   ...
                   (-(iLayerInitLiqFlux(iSoil) - iLayerInitLiqFlux(iSoil-1)) + mLayerInitTranspire(iSoil))*(dt/mLayerDepth(iSoil))*wimplicit + &
   ...
   write(*,'(a,3(f20.12,1x))') 'mlayerMatricHeadTrail(iSoil), rvec(iSoil) = ', mLayerMatricHeadTrial(iSoil), rvec(iSoil)
  end do  ! (looping through soil layers)
  pause ' computing residual'
