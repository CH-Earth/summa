...
contains
 ...
 select case(trim(varName))
  ...
  case('minstep'                  ); get_ixparam = iLookPARAM%minstep                ! minimum length of the time step bEuler, not currently used
  case('maxstep'                  ); get_ixparam = iLookPARAM%maxstep                ! maximum length of the time step bEuler
  case('wimplicit'                ); get_ixparam = iLookPARAM%wimplicit              ! weight assigned to start-of-step fluxes bEuler, not currently used
  case('maxiter'                  ); get_ixparam = iLookPARAM%maxiter                ! maximum number of iterations bEuler or nonlinear iterations Sundials
  case('relConvTol_liquid'        ); get_ixparam = iLookPARAM%relConvTol_liquid      ! relative convergence tolerance for vol frac liq water (-) bEuler
  case('absConvTol_liquid'        ); get_ixparam = iLookPARAM%absConvTol_liquid      ! absolute convergence tolerance for vol frac liq water (-) bEuler
  case('relConvTol_matric'        ); get_ixparam = iLookPARAM%relConvTol_matric      ! relative convergence tolerance for matric head (-) bEuler
  case('absConvTol_matric'        ); get_ixparam = iLookPARAM%absConvTol_matric      ! absolute convergence tolerance for matric head (m) bEuler
  case('relConvTol_energy'        ); get_ixparam = iLookPARAM%relConvTol_energy      ! relative convergence tolerance for energy (-) bEuler
  case('absConvTol_energy'        ); get_ixparam = iLookPARAM%absConvTol_energy      ! absolute convergence tolerance for energy (J m-3) bEuler
  case('relConvTol_aquifr'        ); get_ixparam = iLookPARAM%relConvTol_aquifr      ! relative convergence tolerance for aquifer storage (-) bEuler
  case('absConvTol_aquifr'        ); get_ixparam = iLookPARAM%absConvTol_aquifr      ! absolute convergence tolerance for aquifer storage (m) bEuler
