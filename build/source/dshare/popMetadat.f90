...
subroutine popMetadat(err,message)
  ...
  mpar_meta(iLookPARAM%minstep)               = var_info('minstep'               , 'minimum length of the time step be_numrec, not currently used'    , 's'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%maxstep)               = var_info('maxstep'               , 'maximum length of the time step be_numrec'                        , 's'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%be_steps)              = var_info('be_steps'              , 'minimum number of substeps to take in a maxstep be_numrec'        , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%wimplicit)             = var_info('wimplicit'             , 'weight assigned to the start-of-step fluxes ,be_numrec, not currently used', '-'      , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%maxiter)               = var_info('maxiter'               , 'maximum number of iterations be_numrec or nonlinear iterations Sundials', '-'         , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%relConvTol_liquid)     = var_info('relConvTol_liquid'     , 'relative convergence tolerance for vol frac liq water be_numrec'  , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%absConvTol_liquid)     = var_info('absConvTol_liquid'     , 'absolute convergence tolerance for vol frac liq water be_numrec'  , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%relConvTol_matric)     = var_info('relConvTol_matric'     , 'relative convergence tolerance for matric head be_numrec'         , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%absConvTol_matric)     = var_info('absConvTol_matric'     , 'absolute convergence tolerance for matric head be_numrec'         , 'm'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%relConvTol_energy)     = var_info('relConvTol_energy'     , 'relative convergence tolerance for energy be_numrec'              , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%absConvTol_energy)     = var_info('absConvTol_energy'     , 'absolute convergence tolerance for energy be_numrec'              , 'J m-3'           , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%relConvTol_aquifr)     = var_info('relConvTol_aquifr'     , 'relative convergence tolerance for aquifer storage be_numrec'     , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%absConvTol_aquifr)     = var_info('absConvTol_aquifr'     , 'absolute convergence tolerance for aquifer storage be_numrec'     , 'm'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%relConvTol_ida)        = var_info('relConvTol_ida'        , 'relative convergence tolerance for sundials ida'                  , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  mpar_meta(iLookPARAM%absConvTol_ida)        = var_info('absConvTol_ida'        , 'absolute convergence tolerance for sundials ida'                  , '-'               , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  ...
  diag_meta(iLookDIAG%dVolHtCapBulk_dPsi0)             = var_info('dVolHtCapBulk_dPsi0'            , 'derivative in bulk heat capacity w.r.t. matric potential'              , 'J m-4 K-1'     , get_ixVarType('midSoil'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dVolHtCapBulk_dTheta)            = var_info('dVolHtCapBulk_dTheta'           , 'derivative in bulk heat capacity w.r.t. volumetric water content'      , 'J m-3 K-1'     , get_ixVarType('midToto'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dVolHtCapBulk_dCanWat)           = var_info('dVolHtCapBulk_dCanWat'          , 'derivative in bulk heat capacity w.r.t. volumetric water content'      , 'J m-3 K-1'     , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dVolHtCapBulk_dTk)               = var_info('dVolHtCapBulk_dTk'              , 'derivative in bulk heat capacity w.r.t. temperature'                   , 'J m-3 K-2'     , get_ixVarType('midToto'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dVolHtCapBulk_dTkCanopy)         = var_info('dVolHtCapBulk_dTkCanopy'        , 'derivative in bulk heat capacity w.r.t. temperature'                   , 'J m-3 K-2'     , get_ixVarType('scalarv'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dThermalC_dTempAbove)            = var_info('dThermalC_dTempAbove'           , 'derivative in the thermal conductivity w.r.t. energy in the layer above','J m-2 s-1 K-1' , get_ixVarType('ifcToto'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dThermalC_dTempBelow)            = var_info('dThermalC_dTempBelow'           , 'derivative in the thermal conductivity w.r.t. energy in the layer above','J m-2 s-1 K-1' , get_ixVarType('ifcToto'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dThermalC_dWatAbove)             = var_info('dThermalC_dWatAbove'            , 'derivative in the thermal conductivity w.r.t. water in the layer above', 'unknown'       , get_ixVarType('ifcToto'), iMissVec, iMissVec, .false.)
  diag_meta(iLookDIAG%dThermalC_dWatBelow)             = var_info('dThermalC_dWatBelow'            , 'derivative in the thermal conductivity w.r.t. water in the layer above', 'unknown'       , get_ixVarType('ifcToto'), iMissVec, iMissVec, .false.)
  ! enthalpy
