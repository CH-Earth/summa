MODULE var_lookup
 ...
 type, public  ::  iLook_param
  ...
  integer(i4b)    :: theta_mp                  = 74  ! volumetric liquid water content when macropore flow begins (-)
  integer(i4b)    :: theta_sat                 = 75  ! porosity (-)
  integer(i4b)    :: theta_res                 = 76  ! volumetric residual water content (-)
  integer(i4b)    :: vGn_alpha                 = 77  ! van Genuchten "alpha" parameter (m-1)
  integer(i4b)    :: vGn_n                     = 78  ! van Genuchten "n" parameter (-)
  integer(i4b)    :: mpExp                     = 79  ! empirical exponent in macropore flow equation (-)
  integer(i4b)    :: k_soil                    = 80  ! hydraulic conductivity of soil (m s-1) 
  integer(i4b)    :: k_macropore               = 81  ! saturated hydraulic conductivity for macropores (m s-1)
  integer(i4b)    :: kAnisotropic              = 82  ! anisotropy factor for lateral hydraulic conductivity (-)
  integer(i4b)    :: zScale_TOPMODEL           = 83  ! TOPMODEL scaling factor used in lower boundary condition for soil (m)
  integer(i4b)    :: compactedDepth            = 84  ! depth where k_soil reaches the compacted value given by CH78 (m)
  integer(i4b)    :: aquiferScaleFactor        = 85  ! scaling factor for aquifer storage in the big bucket (m)
  integer(i4b)    :: aquiferBaseflowExp        = 86  ! baseflow exponent (-)
  integer(i4b)    :: qSurfScale                = 87  ! scaling factor in the surface runoff parameterization (-)
  integer(i4b)    :: specificYield             = 88  ! specific yield (-)
  integer(i4b)    :: specificStorage           = 89  ! specific storage coefficient (m-1)
  integer(i4b)    :: f_impede                  = 90  ! ice impedence factor (-)
  ! algorithmic control parameters
  integer(i4b)    :: minwind                   = 91  ! minimum wind speed (m s-1)
  integer(i4b)    :: minstep                   = 92  ! minimum length of the time step
  integer(i4b)    :: maxstep                   = 93  ! maximum length of the time step
  integer(i4b)    :: wimplicit                 = 94  ! weight assigned to the start-of-step fluxes
  integer(i4b)    :: maxiter                   = 95  ! maximum number of iteration 
  integer(i4b)    :: relConvTol_liquid         = 96  ! relative convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: absConvTol_liquid         = 97  ! absolute convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: relConvTol_matric         = 98  ! relative convergence tolerance for matric head (-)
  integer(i4b)    :: absConvTol_matric         = 99  ! absolute convergence tolerance for matric head (m)
  integer(i4b)    :: relConvTol_energy         = 100 ! relative convergence tolerance for energy (-)
  integer(i4b)    :: absConvTol_energy         = 101 ! absolute convergence tolerance for energy (J m-3)
  integer(i4b)    :: relConvTol_aquifr         = 102 ! relative convergence tolerance for aquifer storage (-)
  integer(i4b)    :: absConvTol_aquifr         = 103  ! absolute convergence tolerance for aquifer storage (J m-3)
  integer(i4b)    :: zmin                      = 104 ! minimum layer depth (m)
  integer(i4b)    :: zmax                      = 105 ! maximum layer depth (m)
  integer(i4b)    :: zminLayer1                = 106 ! minimum layer depth for the 1st (top) layer (m)
  integer(i4b)    :: zminLayer2                = 107 ! minimum layer depth for the 2nd layer (m) 
  integer(i4b)    :: zminLayer3                = 108 ! minimum layer depth for the 3rd layer (m) 
  integer(i4b)    :: zminLayer4                = 109 ! minimum layer depth for the 4th layer (m) 
  integer(i4b)    :: zminLayer5                = 110 ! minimum layer depth for the 5th (bottom) layer (m) 
  integer(i4b)    :: zmaxLayer1_lower          = 111 ! maximum layer depth for the 1st (top) layer when only 1 layer (m) 
  integer(i4b)    :: zmaxLayer2_lower          = 112 ! maximum layer depth for the 2nd layer when only 2 layers (m) 
  integer(i4b)    :: zmaxLayer3_lower          = 113 ! maximum layer depth for the 3rd layer when only 3 layers (m) 
  integer(i4b)    :: zmaxLayer4_lower          = 114 ! maximum layer depth for the 4th layer when only 4 layers (m) 
  integer(i4b)    :: zmaxLayer1_upper          = 115 ! maximum layer depth for the 1st (top) layer when > 1 layer (m) 
  integer(i4b)    :: zmaxLayer2_upper          = 116 ! maximum layer depth for the 2nd layer when > 2 layers (m) 
  integer(i4b)    :: zmaxLayer3_upper          = 117 ! maximum layer depth for the 3rd layer when > 3 layers (m) 
  integer(i4b)    :: zmaxLayer4_upper          = 118 ! maximum layer depth for the 4th layer when > 4 layers (m) 
 endtype ilook_param
 ! ***********************************************************************************************************
 ...
 type, public :: iLook_mvar
  ...
  integer(i4b)    :: scalarAquiferBalError           = 142 ! error in the aquifer water balance (kg m-2)
  integer(i4b)    :: scalarTotalSoilLiq              = 143 ! total mass of liquid water in the soil (kg m-2)
  integer(i4b)    :: scalarTotalSoilIce              = 144 ! total mass of ice in the soil (kg m-2)
  ! define variables at the mid-point of each layer -- domain geometry
  integer(i4b)    :: mLayerDepth                     = 145 ! depth of each layer (m)
  integer(i4b)    :: mLayerHeight                    = 146 ! height at the mid-point of each layer (m)
  integer(i4b)    :: mLayerRootDensity               = 147 ! fraction of roots in each soil layer (-)
  ! define variables at the mid-point of each layer -- coupled energy and mass
  integer(i4b)    :: mLayerTemp                      = 148 ! temperature of each layer (K)
  integer(i4b)    :: mLayerVolFracAir                = 149 ! volumetric fraction of air in each layer (-)
  integer(i4b)    :: mLayerVolFracIce                = 150 ! volumetric fraction of ice water in each layer (-)
  integer(i4b)    :: mLayerVolFracLiq                = 151 ! volumetric fraction of liquid water in each layer (-)
  integer(i4b)    :: mLayerVolHtCapBulk              = 152 ! volumetric heat capacity in each layer (J m-3 K-1)
  integer(i4b)    :: mLayerTcrit                     = 153 ! critical soil temperature above which all water is unfrozen (K)
  integer(i4b)    :: mLayerdTheta_dTk                = 154 ! derivative in volumetric liquid water content wrt temperature (K-1)
  integer(i4b)    :: mLayerThermalC                  = 155 ! thermal conductivity at the mid-point of each layer (W m-1 K-1)
  integer(i4b)    :: mLayerRadCondFlux               = 156 ! temporal derivative in energy from radiative and conductive flux (J m-2 s-1)  
  integer(i4b)    :: mLayerMeltFreeze                = 157 ! rate of ice content change from melt/freeze in each layer (kg m-3 s-1)
  integer(i4b)    :: mLayerInfilFreeze               = 158 ! rate of ice content change by freezing infiltrating flux (kg m-3 s-1)
  integer(i4b)    :: mLayerSatHydCond                = 159 ! saturated hydraulic conductivity in each layer (m s-1)
  integer(i4b)    :: mLayerSatHydCondMP              = 160 ! saturated hydraulic conductivity of macropores in each layer (m s-1)
  integer(i4b)    :: mLayerMatricHead                = 161 ! matric head of water in the soil (m)
  integer(i4b)    :: mLayerdTheta_dPsi               = 162 ! derivative in the soil water characteristic (m-1)
  integer(i4b)    :: mLayerdPsi_dTheta               = 163 ! derivative in the soil water characteristic (m)
  integer(i4b)    :: mLayerThetaResid                = 164 ! residual volumetric water content in each snow layer (-)
  integer(i4b)    :: mLayerPoreSpace                 = 165 ! total pore space in each snow layer (-)
  integer(i4b)    :: mLayerTranspireLim              = 166 ! soil moist & veg limit on transpiration for each layer (-)
  integer(i4b)    :: mLayerInitTranspire             = 167 ! transpiration loss from each soil layer at the start of the step (kg m-2 s-1)
  integer(i4b)    :: mLayerTranspire                 = 168 ! transpiration loss from each soil layer (kg m-2 s-1)
  integer(i4b)    :: mLayerInitQMacropore            = 169 ! liquid flux from micropores to macropores at the start-of-step (m s-1)
  integer(i4b)    :: mLayerQMacropore                = 170 ! liquid flux from micropores to macropores (m s-1)
  integer(i4b)    :: mLayerInitBaseflow              = 171 ! baseflow from each soil layer at the start of the time step (m s-1)
  integer(i4b)    :: mLayerBaseflow                  = 172 ! baseflow from each soil layer (m s-1)
  integer(i4b)    :: mLayerColumnInflow              = 173 ! total inflow to each layer in a given soil column (m3 s-1)
  integer(i4b)    :: mLayerColumnOutflow             = 174 ! total outflow from each layer in a given soil column (m3 s-1)
  ! define variables at the interface of each layer
  integer(i4b)    :: iLayerHeight                    = 175 ! height of the layer interface; top of soil = 0 (m)
  integer(i4b)    :: iLayerThermalC                  = 176 ! thermal conductivity at the interface of each layer (W m-1 K-1)
  integer(i4b)    :: iLayerConductiveFlux            = 177 ! conductive energy flux at layer interfaces at end of time step (W m-2)
  integer(i4b)    :: iLayerAdvectiveFlux             = 178 ! advective energy flux at layer interfaces at end of time step (W m-2)
  integer(i4b)    :: iLayerInitNrgFlux               = 179 ! energy flux at layer interfaces at the start of the time step (W m-2) 
  integer(i4b)    :: iLayerNrgFlux                   = 180 ! energy flux at layer interfaces at the end of the time step (W m-2) 
