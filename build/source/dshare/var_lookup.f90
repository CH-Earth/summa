MODULE var_lookup
 ...
 type, public :: iLook_mvar
  ...
  integer(i4b)    :: scalarTranspireLimAqfr = 24 ! soil moist & vegetation limit on transpiration in the aquifer (-)
  integer(i4b)    :: scalarPotentialET      = 25 ! potential evapotranspiration (kg m-2 s-1)
  integer(i4b)    :: scalarMassLiquid       = 26 ! evaporation or dew (kg m-2 s-1)
  ...
  integer(i4b)    :: scalarSoilInflux       = 31 ! influx of water at the top of the soil profile (m s-1)
  integer(i4b)    :: scalarSoilDrainage     = 32 ! drainage from the bottom of the soil profile (m s-1) 
  integer(i4b)    :: scalarSoilEjection     = 33 ! total water ejected from each soil layer (m s-1) 
  ...
  integer(i4b)    :: scalarSoilWatBalError  = 36 ! error in the total soil water balance (kg m-2)
  integer(i4b)    :: scalarTotalSoilLiq     = 37 ! total mass of liquid water in the soil (kg m-2)
  integer(i4b)    :: scalarTotalSoilIce     = 38 ! total mass of ice in the soil (kg m-2)
  ! define variables at the mid-point of each layer -- domain geometry
  integer(i4b)    :: mLayerDepth            = 39 ! depth of each layer (m)
  integer(i4b)    :: mLayerHeight           = 40 ! height at the mid-point of each layer (m)
  integer(i4b)    :: mLayerRootDensity      = 41 ! fraction of roots in each soil layer (-)
  ! define variables at the mid-point of each layer -- coupled energy and mass
  integer(i4b)    :: mLayerTemp             = 42 ! temperature of each layer (K)
  integer(i4b)    :: mLayerVolFracAir       = 43 ! volumetric fraction of air in each layer (-)
  integer(i4b)    :: mLayerVolFracIce       = 44 ! volumetric fraction of ice water in each layer (-)
  integer(i4b)    :: mLayerVolFracLiq       = 45 ! volumetric fraction of liquid water in each layer (-)
  integer(i4b)    :: mLayerVolHtCapBulk     = 46 ! volumetric heat capacity in each layer (J m-3 K-1)
  integer(i4b)    :: mLayerTcrit            = 47 ! critical soil temperature above which all water is unfrozen (K)
  integer(i4b)    :: mLayerdTheta_dTk       = 48 ! derivative in volumetric liquid water content wrt temperature (K-1)
  integer(i4b)    :: mLayerThermalC         = 49 ! thermal conductivity at the mid-point of each layer (W m-1 K-1)
  integer(i4b)    :: mLayerRadCondFlux      = 50 ! temporal derivative in energy from radiative and conductive flux (J m-2 s-1)  
  integer(i4b)    :: mLayerMeltFreeze       = 51 ! melt/freeze in each layer (kg m-3)
  integer(i4b)    :: mLayerMatricHead       = 52 ! matric head of water in the soil (m)
  integer(i4b)    :: mLayerdTheta_dPsi      = 53 ! derivative in the soil water characteristic (m-1)
  integer(i4b)    :: mLayerdPsi_dTheta      = 54 ! derivative in the soil water characteristic (m-1)
  integer(i4b)    :: mLayerThetaResid       = 55 ! residual volumetric water content in each snow layer (-)
  integer(i4b)    :: mLayerPoreSpace        = 56 ! total pore space in each snow layer (-)
  integer(i4b)    :: mLayerInfilFreeze      = 57 ! volumetric ice content increase by freezing infiltrating flux (-)
  integer(i4b)    :: mLayerTranspireLim     = 58 ! soil moist & veg limit on transpiration for each layer (-)
  integer(i4b)    :: mLayerInitTranspire    = 59 ! transpiration loss from each soil layer at the start of the step (kg m-2 s-1)
  integer(i4b)    :: mLayerTranspire        = 60 ! transpiration loss from each soil layer (kg m-2 s-1)
  integer(i4b)    :: mLayerInitEjectWater   = 61 ! water ejected from each soil layer at the start-of-step (m s-1)
  integer(i4b)    :: mLayerEjectWater       = 62 ! water ejected from each soil layer (m s-1)
  ! define variables at the interface of each layer
  integer(i4b)    :: iLayerHeight           = 63 ! height of the layer interface; top of soil = 0 (m)
  integer(i4b)    :: iLayerThermalC         = 64 ! thermal conductivity at the interface of each layer (W m-1 K-1)
  integer(i4b)    :: iLayerInitNrgFlux      = 65 ! energy flux at layer interfaces at the start of the time step (W m-2) 
  integer(i4b)    :: iLayerNrgFlux          = 66 ! energy flux at layer interfaces at the end of the time step (W m-2) 
  integer(i4b)    :: iLayerInitLiqFluxSnow  = 67 ! liquid flux at snow layer interfaces at the start of the time step (m s-1) 
  integer(i4b)    :: iLayerInitLiqFluxSoil  = 68 ! liquid flux at soil layer interfaces at the start of the time step (m s-1) 
  integer(i4b)    :: iLayerLiqFluxSnow      = 69 ! liquid flux at snow layer interfaces at the end of the time step (m s-1)
  integer(i4b)    :: iLayerLiqFluxSoil      = 70 ! liquid flux at soil layer interfaces at the end of the time step (m s-1) 
  ! define derived variables
  integer(i4b)    :: scalarExNeut           = 71 ! exchange coefficient in neutral conditions
  integer(i4b)    :: scalarBprime           = 72 ! stable b parameter in Louis (1979) stability function
  integer(i4b)    :: scalarCparam           = 73 ! c parameter in Louis (1979) stability function
  integer(i4b)    :: scalarVGn_m            = 74 ! van Genuchten "m" parameter (-)
  integer(i4b)    :: scalarKappa            = 75 ! constant in the freezing curve function (m K-1)
  integer(i4b)    :: scalarVolHtCap_air     = 76 ! volumetric heat capacity air         (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_ice     = 77 ! volumetric heat capacity ice         (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_soil    = 78 ! volumetric heat capacity dry soil    (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_water   = 79 ! volumetric heat capacity liquid wat  (J m-3 K-1)
  integer(i4b)    :: scalarLambda_drysoil   = 80 ! thermal conductivity of dry soil     (W m-1)
  integer(i4b)    :: scalarLambda_wetsoil   = 81 ! thermal conductivity of wet soil     (W m-1)
  integer(i4b)    :: scalarVolLatHt_fus     = 82 ! volumetric latent heat of fusion     (J m-3)
  integer(i4b)    :: scalarAquiferRootFrac  = 83 ! fraction of roots below the soil profile (-)
 endtype iLook_mvar
 ...
 integer(i4b),parameter,public :: maxvarMvar     = 83
