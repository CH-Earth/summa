...
MODULE var_lookup
 ...
 type, public  ::  iLook_param
  ...
  integer(i4b)    :: jmax_Ha                   = 66  ! activation energy in the jmax function (J mol-1)
  integer(i4b)    :: jmax_Hd                   = 67  ! deactivation energy in the jmax function (J mol-1)
  integer(i4b)    :: jmax_Sv                   = 68  ! entropy term in the jmax function (J mol-1 K-1)
  integer(i4b)    :: fractionJ                 = 69  ! fraction of light lost by other than the chloroplast lamellae (-)
  integer(i4b)    :: quantamYield              = 70  ! quantam yield (mol e mol-1 q)
  integer(i4b)    :: vpScaleFactor             = 71  ! vapor pressure scaling factor in stomatal conductance function (Pa)
  integer(i4b)    :: cond2photo_slope          = 72  ! slope of conductance-photosynthesis relationship (-)
  integer(i4b)    :: minStomatalConductance    = 73  ! minimum stomatal conductance (umol H2O m-2 s-1)
  ! vegetation properties
  integer(i4b)    :: winterSAI                 = 74  ! stem area index prior to the start of the growing season (m2 m-2)
  integer(i4b)    :: summerLAI                 = 75  ! maximum leaf area index at the peak of the growing season (m2 m-2)
  integer(i4b)    :: rootScaleFactor1          = 76  ! 1st scaling factor (a) in Y = 1 - 0.5*( exp(-aZ) + exp(-bZ) )  (m-1)
  integer(i4b)    :: rootScaleFactor2          = 77  ! 2nd scaling factor (b) in Y = 1 - 0.5*( exp(-aZ) + exp(-bZ) )  (m-1)
  integer(i4b)    :: rootingDepth              = 78  ! rooting depth (m)
  integer(i4b)    :: rootDistExp               = 89  ! exponent controlling the vertical distribution of root density (-)
  integer(i4b)    :: plantWiltPsi              = 80  ! matric head at wilting point (m)
  integer(i4b)    :: soilStressParam           = 81  ! parameter in the exponential soil stress function
  integer(i4b)    :: critSoilWilting           = 82  ! critical vol. liq. water content when plants are wilting (-)
  integer(i4b)    :: critSoilTranspire         = 83  ! critical vol. liq. water content when transpiration is limited (-)
  integer(i4b)    :: critAquiferTranspire      = 84  ! critical aquifer storage value when transpiration is limited (m)
  integer(i4b)    :: minStomatalResistance     = 85  ! minimum canopy resistance (s m-1)
  integer(i4b)    :: leafDimension             = 86  ! characteristic leaf dimension (m)
  integer(i4b)    :: heightCanopyTop           = 87  ! height of top of the vegetation canopy above ground surface (m)
  integer(i4b)    :: heightCanopyBottom        = 88  ! height of bottom of the vegetation canopy above ground surface (m)
  integer(i4b)    :: specificHeatVeg           = 89  ! specific heat of vegetation (J kg-1 K-1)
  integer(i4b)    :: maxMassVegetation         = 90  ! maximum mass of vegetation (full foliage) (kg m-2)
  integer(i4b)    :: throughfallScaleSnow      = 91  ! scaling factor for throughfall (snow) (-)
  integer(i4b)    :: throughfallScaleRain      = 92  ! scaling factor for throughfall (rain) (-)
  integer(i4b)    :: refInterceptCapSnow       = 93  ! reference canopy interception capacity per unit leaf area (snow) (kg m-2)
  integer(i4b)    :: refInterceptCapRain       = 94  ! canopy interception capacity per unit leaf area (rain) (kg m-2)
  integer(i4b)    :: snowUnloadingCoeff        = 95  ! time constant for unloading of snow from the forest canopy (s-1)
  integer(i4b)    :: canopyDrainageCoeff       = 96  ! time constant for drainage of liquid water from the forest canopy (s-1)
  integer(i4b)    :: ratioDrip2Unloading       = 97  ! ratio of canopy drip to unloading of snow from the forest canopy (-)
  integer(i4b)    :: canopyWettingFactor       = 98  ! maximum wetted fraction of the canopy (-)
  integer(i4b)    :: canopyWettingExp          = 99  ! exponent in canopy wetting function (-)
  ! soil properties
  integer(i4b)    :: soil_dens_intr            = 100 ! intrinsic soil density (kg m-3)
  integer(i4b)    :: thCond_soil               = 101 ! thermal conductivity of soil (W m-1 K-1)
  integer(i4b)    :: frac_sand                 = 102 ! fraction of sand (-)
  integer(i4b)    :: frac_silt                 = 103 ! fraction of silt (-)
  integer(i4b)    :: frac_clay                 = 104 ! fraction of clay (-)
  integer(i4b)    :: fieldCapacity             = 105 ! field capacity (-)
  integer(i4b)    :: wettingFrontSuction       = 106 ! Green-Ampt wetting front suction (m)
  integer(i4b)    :: theta_mp                  = 107 ! volumetric liquid water content when macropore flow begins (-)
  integer(i4b)    :: theta_sat                 = 108 ! porosity (-)
  integer(i4b)    :: theta_res                 = 109 ! volumetric residual water content (-)
  integer(i4b)    :: vGn_alpha                 = 110 ! van Genuchten "alpha" parameter (m-1)
  integer(i4b)    :: vGn_n                     = 111 ! van Genuchten "n" parameter (-)
  integer(i4b)    :: mpExp                     = 112 ! empirical exponent in macropore flow equation (-)
  integer(i4b)    :: k_soil                    = 113 ! hydraulic conductivity of soil (m s-1)
  integer(i4b)    :: k_macropore               = 114 ! saturated hydraulic conductivity for macropores (m s-1)
  integer(i4b)    :: kAnisotropic              = 115 ! anisotropy factor for lateral hydraulic conductivity (-)
  integer(i4b)    :: zScale_TOPMODEL           = 116 ! TOPMODEL scaling factor used in lower boundary condition for soil (m)
  integer(i4b)    :: compactedDepth            = 117 ! depth where k_soil reaches the compacted value given by CH78 (m)
  integer(i4b)    :: aquiferScaleFactor        = 118 ! scaling factor for aquifer storage in the big bucket (m)
  integer(i4b)    :: aquiferBaseflowExp        = 119 ! baseflow exponent (-)
  integer(i4b)    :: qSurfScale                = 120 ! scaling factor in the surface runoff parameterization (-)
  integer(i4b)    :: specificYield             = 121 ! specific yield (-)
  integer(i4b)    :: specificStorage           = 122 ! specific storage coefficient (m-1)
  integer(i4b)    :: f_impede                  = 123 ! ice impedence factor (-)
  integer(i4b)    :: soilIceScale              = 124 ! scaling factor for depth of soil ice, used to get frozen fraction (m)
  integer(i4b)    :: soilIceCV                 = 125 ! CV of depth of soil ice, used to get frozen fraction (-)
  ! algorithmic control parameters
  integer(i4b)    :: minwind                   = 126 ! minimum wind speed (m s-1)
  integer(i4b)    :: minstep                   = 127 ! minimum length of the time step
  integer(i4b)    :: maxstep                   = 128 ! maximum length of the time step
  integer(i4b)    :: wimplicit                 = 129 ! weight assigned to the start-of-step fluxes
  integer(i4b)    :: maxiter                   = 130 ! maximum number of iteration
  integer(i4b)    :: relConvTol_liquid         = 131 ! relative convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: absConvTol_liquid         = 132 ! absolute convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: relConvTol_matric         = 133 ! relative convergence tolerance for matric head (-)
  integer(i4b)    :: absConvTol_matric         = 134 ! absolute convergence tolerance for matric head (m)
  integer(i4b)    :: relConvTol_energy         = 135 ! relative convergence tolerance for energy (-)
  integer(i4b)    :: absConvTol_energy         = 136 ! absolute convergence tolerance for energy (J m-3)
  integer(i4b)    :: relConvTol_aquifr         = 137 ! relative convergence tolerance for aquifer storage (-)
  integer(i4b)    :: absConvTol_aquifr         = 138 ! absolute convergence tolerance for aquifer storage (J m-3)
  integer(i4b)    :: zmin                      = 139 ! minimum layer depth (m)
  integer(i4b)    :: zmax                      = 140 ! maximum layer depth (m)
  integer(i4b)    :: zminLayer1                = 141 ! minimum layer depth for the 1st (top) layer (m)
  integer(i4b)    :: zminLayer2                = 142 ! minimum layer depth for the 2nd layer (m)
  integer(i4b)    :: zminLayer3                = 143 ! minimum layer depth for the 3rd layer (m)
  integer(i4b)    :: zminLayer4                = 144 ! minimum layer depth for the 4th layer (m)
  integer(i4b)    :: zminLayer5                = 145 ! minimum layer depth for the 5th (bottom) layer (m)
  integer(i4b)    :: zmaxLayer1_lower          = 146 ! maximum layer depth for the 1st (top) layer when only 1 layer (m)
  integer(i4b)    :: zmaxLayer2_lower          = 147 ! maximum layer depth for the 2nd layer when only 2 layers (m)
  integer(i4b)    :: zmaxLayer3_lower          = 148 ! maximum layer depth for the 3rd layer when only 3 layers (m)
  integer(i4b)    :: zmaxLayer4_lower          = 149 ! maximum layer depth for the 4th layer when only 4 layers (m)
  integer(i4b)    :: zmaxLayer1_upper          = 150 ! maximum layer depth for the 1st (top) layer when > 1 layer (m)
  integer(i4b)    :: zmaxLayer2_upper          = 151 ! maximum layer depth for the 2nd layer when > 2 layers (m)
  integer(i4b)    :: zmaxLayer3_upper          = 152 ! maximum layer depth for the 3rd layer when > 3 layers (m)
  integer(i4b)    :: zmaxLayer4_upper          = 153 ! maximum layer depth for the 4th layer when > 4 layers (m)
 endtype ilook_param
 ! ***********************************************************************************************************
 ...
 integer(i4b),parameter,public :: maxvarMpar     = 153
