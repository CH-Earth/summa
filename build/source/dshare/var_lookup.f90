MODULE var_lookup
 ...
 type, public :: iLook_mvar
  ...
  integer(i4b)    :: scalarPsycConstCanopy          = 65  ! psychometric constant for the vegetation canopy (Pa K-1)
  integer(i4b)    :: scalarPsycConstGround          = 66  ! psychometric constant for the ground surface (Pa K-1)
  integer(i4b)    :: scalarSatVP_CanopyTemp         = 67  ! saturation vapor pressure at the temperature of vegetation canopy (Pa)
  integer(i4b)    :: scalarSatVP_GroundTemp         = 68  ! saturation vapor pressure at the temperature of the ground (Pa)
  integer(i4b)    :: scalarWindReductionFactor      = 69  ! canopy wind reduction factor (-)
  integer(i4b)    :: scalarZeroPlaneDisplacement    = 70  ! zero plane displacement (m) 
  integer(i4b)    :: scalarSfc2AtmExchangeCoeff     = 71  ! surface-atmosphere turbulent exchange coefficient (-)
  integer(i4b)    :: scalarEddyDiffusCanopyTop      = 72  ! eddy diffusivity for heat at the top of the canopy (m2 s-1)
  integer(i4b)    :: scalarWindspdCanopyTop         = 73  ! windspeed at the top of the canopy (m s-1)
  integer(i4b)    :: scalarGroundResistance         = 74  ! below canopy aerodynamic resistance (s m-1) 
  integer(i4b)    :: scalarCanopyResistance         = 75  ! above canopy aerodynamic resistance (s m-1)
  integer(i4b)    :: scalarLeafResistance           = 76  ! mean leaf boundary layer resistance per unit leaf area (s m-1)
  integer(i4b)    :: scalarSoilResistance           = 77  ! soil surface resistance (s m-1)
  integer(i4b)    :: scalarSoilRelHumidity          = 78  ! relative humidity in the soil pores in the upper-most soil layer (-)
  integer(i4b)    :: scalarSenHeatCanopy            = 79  ! sensible heat from the canopy to the canopy air space (W m-2)
  integer(i4b)    :: scalarSenHeatGround            = 80  ! sensible heat from the ground (below canopy or non-vegetated) (W m-2)
  integer(i4b)    :: scalarLatHeatCanopyEvap        = 81  ! evaporation latent heat from the canopy to the canopy air space (W m-2) 
  integer(i4b)    :: scalarLatHeatCanopyTrans       = 82  ! transpiration latent heat from the canopy to the canopy air space (W m-2)
  integer(i4b)    :: scalarLatHeatGround            = 83  ! latent heat from the ground (below canopy or non-vegetated) (W m-2)
  integer(i4b)    :: scalarCanopyTranspiration      = 84  ! canopy transpiration (kg m-2 s-1)
  integer(i4b)    :: scalarCanopyEvaporation        = 85  ! canopy evaporation/condensation (kg m-2 s-1)
  integer(i4b)    :: scalarCanopySublimation        = 86  ! canopy sublimation/frost (kg m-2 s-1)
  integer(i4b)    :: scalarGroundEvaporation        = 87  ! ground evaporation/condensation (below canopy or non-vegetated) (kg m-2 s-1)
  integer(i4b)    :: scalarGroundSublimation        = 88  ! ground sublimation/frost (below canopy or non-vegetated) (kg m-2 s-1)
  ! define NOAH-MP vegetation variables -- transpiration
  integer(i4b)    :: scalarTranspireLim             = 89  ! aggregate soil moisture + aquifer storage limit on transpiration (-)
  integer(i4b)    :: scalarTranspireLimAqfr         = 90  ! aquifer storage limit on transpiration (-)
  integer(i4b)    :: scalarFoliageNitrogenFactor    = 91  ! foliage nitrogen concentration, 1=saturated (-)
  integer(i4b)    :: scalarStomResistSunlit         = 92  ! stomatal resistance for sunlit leaves (s m-1)
  integer(i4b)    :: scalarStomResistShaded         = 93  ! stomatal resistance for shaded leaves (s m-1)
  integer(i4b)    :: scalarPhotosynthesisSunlit     = 94  ! sunlit photosynthesis (umolco2 m-2 s-1)
  integer(i4b)    :: scalarPhotosynthesisShaded     = 95  ! shaded photosynthesis (umolco2 m-2 s-1)
  ! define NOAH-MP vegetation variables --hydrology
  integer(i4b)    :: scalarCanopyWetFraction        = 96  ! fraction of canopy that is wet
  integer(i4b)    :: temp1                          = 97  ! placeholder
  integer(i4b)    :: temp2                          = 98  ! placeholder
