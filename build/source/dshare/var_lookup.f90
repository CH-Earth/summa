MODULE var_lookup
 ...
 type, public  ::  iLook_param
  ...
  integer(i4b)    :: minStomatalResistance     = 55  ! minimum canopy resistance (s m-1)
  integer(i4b)    :: leafDimension             = 56  ! characteristic leaf dimension (m)
  integer(i4b)    :: heightCanopyTop           = 57  ! height of top of the vegetation canopy above ground surface (m)
  integer(i4b)    :: heightCanopyBottom        = 58  ! height of bottom of the vegetation canopy above ground surface (m)
  integer(i4b)    :: specificHeatVeg           = 59  ! specific heat of vegetation (J kg-1 K-1)
  integer(i4b)    :: maxMassVegetation         = 60  ! maximum mass of vegetation (full foliage) (kg m-2)
  integer(i4b)    :: throughfallScaleSnow      = 61  ! scaling factor for throughfall (snow) (-)
  integer(i4b)    :: throughfallScaleRain      = 62  ! scaling factor for throughfall (rain) (-) 
  integer(i4b)    :: refInterceptCapSnow       = 63  ! reference canopy interception capacity per unit leaf area (snow) (kg m-2)
  integer(i4b)    :: refInterceptCapRain       = 64  ! canopy interception capacity per unit leaf area (rain) (kg m-2)
  integer(i4b)    :: snowUnloadingCoeff        = 65  ! time constant for unloading of snow from the forest canopy (s-1)
  integer(i4b)    :: canopyDrainageCoeff       = 66  ! time constant for drainage of liquid water from the forest canopy (s-1)
  integer(i4b)    :: ratioDrip2Unloading       = 67  ! ratio of canopy drip to unloading of snow from the forest canopy (-)
  ! soil properties
  integer(i4b)    :: soil_dens_intr            = 68  ! intrinsic soil density (kg m-3)
  integer(i4b)    :: thCond_soil               = 69  ! thermal conductivity of soil (W m-1 K-1)
  integer(i4b)    :: frac_sand                 = 70  ! fraction of sand (-)
  integer(i4b)    :: frac_silt                 = 71  ! fraction of silt (-)
  integer(i4b)    :: frac_clay                 = 72  ! fraction of clay (-)
  integer(i4b)    :: fieldCapacity             = 73  ! field capacity (-)
  integer(i4b)    :: wettingFrontSuction       = 74  ! Green-Ampt wetting front suction (m) 
  integer(i4b)    :: theta_mp                  = 75  ! volumetric liquid water content when macropore flow begins (-)
  integer(i4b)    :: theta_sat                 = 76  ! porosity (-)
  integer(i4b)    :: theta_res                 = 77  ! volumetric residual water content (-)
  integer(i4b)    :: vGn_alpha                 = 78  ! van Genuchten "alpha" parameter (m-1)
  integer(i4b)    :: vGn_n                     = 79  ! van Genuchten "n" parameter (-)
  integer(i4b)    :: mpExp                     = 80  ! empirical exponent in macropore flow equation (-)
  integer(i4b)    :: k_soil                    = 81  ! hydraulic conductivity of soil (m s-1) 
  integer(i4b)    :: k_macropore               = 82  ! saturated hydraulic conductivity for macropores (m s-1)
  integer(i4b)    :: kAnisotropic              = 83  ! anisotropy factor for lateral hydraulic conductivity (-)
  integer(i4b)    :: zScale_TOPMODEL           = 84  ! TOPMODEL scaling factor used in lower boundary condition for soil (m)
  integer(i4b)    :: compactedDepth            = 85  ! depth where k_soil reaches the compacted value given by CH78 (m)
  integer(i4b)    :: aquiferScaleFactor        = 86  ! scaling factor for aquifer storage in the big bucket (m)
  integer(i4b)    :: aquiferBaseflowExp        = 87  ! baseflow exponent (-)
  integer(i4b)    :: qSurfScale                = 88  ! scaling factor in the surface runoff parameterization (-)
  integer(i4b)    :: specificYield             = 89  ! specific yield (-)
  integer(i4b)    :: specificStorage           = 90  ! specific storage coefficient (m-1)
  integer(i4b)    :: f_impede                  = 91  ! ice impedence factor (-)
  integer(i4b)    :: soilIceScale              = 92  ! scaling factor for depth of soil ice, used to get frozen fraction (m)
  integer(i4b)    :: soilIceCV                 = 93  ! CV of depth of soil ice, used to get frozen fraction (-)
  ! algorithmic control parameters
  integer(i4b)    :: minwind                   = 94  ! minimum wind speed (m s-1)
  integer(i4b)    :: minstep                   = 95  ! minimum length of the time step
  integer(i4b)    :: maxstep                   = 96  ! maximum length of the time step
  integer(i4b)    :: wimplicit                 = 97  ! weight assigned to the start-of-step fluxes
  integer(i4b)    :: maxiter                   = 98  ! maximum number of iteration 
  integer(i4b)    :: relConvTol_liquid         = 99  ! relative convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: absConvTol_liquid         = 100 ! absolute convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: relConvTol_matric         = 101  ! relative convergence tolerance for matric head (-)
  integer(i4b)    :: absConvTol_matric         = 102 ! absolute convergence tolerance for matric head (m)
  integer(i4b)    :: relConvTol_energy         = 103 ! relative convergence tolerance for energy (-)
  integer(i4b)    :: absConvTol_energy         = 104 ! absolute convergence tolerance for energy (J m-3)
  integer(i4b)    :: relConvTol_aquifr         = 105 ! relative convergence tolerance for aquifer storage (-)
  integer(i4b)    :: absConvTol_aquifr         = 106 ! absolute convergence tolerance for aquifer storage (J m-3)
  integer(i4b)    :: zmin                      = 107 ! minimum layer depth (m)
  integer(i4b)    :: zmax                      = 108 ! maximum layer depth (m)
  integer(i4b)    :: zminLayer1                = 109 ! minimum layer depth for the 1st (top) layer (m)
  integer(i4b)    :: zminLayer2                = 110 ! minimum layer depth for the 2nd layer (m) 
  integer(i4b)    :: zminLayer3                = 111 ! minimum layer depth for the 3rd layer (m) 
  integer(i4b)    :: zminLayer4                = 112 ! minimum layer depth for the 4th layer (m) 
  integer(i4b)    :: zminLayer5                = 113 ! minimum layer depth for the 5th (bottom) layer (m) 
  integer(i4b)    :: zmaxLayer1_lower          = 114 ! maximum layer depth for the 1st (top) layer when only 1 layer (m) 
  integer(i4b)    :: zmaxLayer2_lower          = 115 ! maximum layer depth for the 2nd layer when only 2 layers (m) 
  integer(i4b)    :: zmaxLayer3_lower          = 116 ! maximum layer depth for the 3rd layer when only 3 layers (m) 
  integer(i4b)    :: zmaxLayer4_lower          = 117 ! maximum layer depth for the 4th layer when only 4 layers (m) 
  integer(i4b)    :: zmaxLayer1_upper          = 118 ! maximum layer depth for the 1st (top) layer when > 1 layer (m) 
  integer(i4b)    :: zmaxLayer2_upper          = 119 ! maximum layer depth for the 2nd layer when > 2 layers (m) 
  integer(i4b)    :: zmaxLayer3_upper          = 120 ! maximum layer depth for the 3rd layer when > 3 layers (m) 
  integer(i4b)    :: zmaxLayer4_upper          = 121 ! maximum layer depth for the 4th layer when > 4 layers (m) 
 endtype ilook_param
 ! ***********************************************************************************************************
 ...
 type, public :: iLook_mvar
  ...
  integer(i4b)    :: scalarInfilArea                 = 128 ! fraction of unfrozen area where water can infiltrate (-)
  integer(i4b)    :: scalarFrozenArea                = 129 ! fraction of area that is considered impermeable due to soil ice (-)
  integer(i4b)    :: scalarInfiltration              = 130 ! infiltration of water into the soil profile (m s-1)
  integer(i4b)    :: scalarExfiltration              = 131 ! exfiltration of water from the top of the soil profile (m s-1)
  integer(i4b)    :: scalarSurfaceRunoff             = 132 ! surface runoff (m s-1)
  integer(i4b)    :: scalarInitAquiferRecharge       = 133 ! recharge to the aquifer at the start of the step (m s-1)
  integer(i4b)    :: scalarAquiferRecharge           = 134 ! recharge to the aquifer (m s-1)
  integer(i4b)    :: scalarInitAquiferTranspire      = 135 ! transpiration from the aquifer at the start of the step (m s-1)
  integer(i4b)    :: scalarAquiferTranspire          = 136 ! transpiration from the aquifer (m s-1)
  integer(i4b)    :: scalarInitAquiferBaseflow       = 137 ! baseflow from the aquifer at the start of the step (m s-1)
  integer(i4b)    :: scalarAquiferBaseflow           = 138 ! baseflow from the aquifer (m s-1)
  ! scalar variables -- sub-step average fluxes for the soil zone
  integer(i4b)    :: scalarSoilInflux                = 139 ! sub-step average: influx of water at the top of the soil profile (m s-1)
  integer(i4b)    :: scalarSoilBaseflow              = 140 ! sub-step average: total baseflow from throughout the soil profile (m s-1)
  integer(i4b)    :: scalarSoilDrainage              = 141 ! sub-step average: drainage from the bottom of the soil profile (m s-1) 
  integer(i4b)    :: scalarSoilTranspiration         = 142 ! sub-step average: total transpiration from the soil (m s-1)
  ! define scalar variables -- mass balance check
  integer(i4b)    :: scalarSoilWatBalError           = 143 ! error in the total soil water balance (kg m-2)
  integer(i4b)    :: scalarAquiferBalError           = 144 ! error in the aquifer water balance (kg m-2)
  integer(i4b)    :: scalarTotalSoilLiq              = 145 ! total mass of liquid water in the soil (kg m-2)
  integer(i4b)    :: scalarTotalSoilIce              = 146 ! total mass of ice in the soil (kg m-2)
  ! define variables at the mid-point of each layer -- domain geometry
  integer(i4b)    :: mLayerDepth                     = 147 ! depth of each layer (m)
  integer(i4b)    :: mLayerHeight                    = 148 ! height at the mid-point of each layer (m)
  integer(i4b)    :: mLayerRootDensity               = 149 ! fraction of roots in each soil layer (-)
  ! define variables at the mid-point of each layer -- coupled energy and mass
  integer(i4b)    :: mLayerTemp                      = 150 ! temperature of each layer (K)
  integer(i4b)    :: mLayerVolFracAir                = 151 ! volumetric fraction of air in each layer (-)
  integer(i4b)    :: mLayerVolFracIce                = 152 ! volumetric fraction of ice water in each layer (-)
  integer(i4b)    :: mLayerVolFracLiq                = 153 ! volumetric fraction of liquid water in each layer (-)
  integer(i4b)    :: mLayerVolHtCapBulk              = 154 ! volumetric heat capacity in each layer (J m-3 K-1)
  integer(i4b)    :: mLayerTcrit                     = 155 ! critical soil temperature above which all water is unfrozen (K)
  integer(i4b)    :: mLayerdTheta_dTk                = 156 ! derivative in volumetric liquid water content wrt temperature (K-1)
  integer(i4b)    :: mLayerThermalC                  = 157 ! thermal conductivity at the mid-point of each layer (W m-1 K-1)
  integer(i4b)    :: mLayerRadCondFlux               = 158 ! temporal derivative in energy from radiative and conductive flux (J m-2 s-1)  
  integer(i4b)    :: mLayerMeltFreeze                = 159 ! rate of ice content change from melt/freeze in each layer (kg m-3 s-1)
  integer(i4b)    :: mLayerInfilFreeze               = 160 ! rate of ice content change by freezing infiltrating flux (kg m-3 s-1)
  integer(i4b)    :: mLayerSatHydCond                = 161 ! saturated hydraulic conductivity in each layer (m s-1)
  integer(i4b)    :: mLayerSatHydCondMP              = 162 ! saturated hydraulic conductivity of macropores in each layer (m s-1)
  integer(i4b)    :: mLayerMatricHead                = 163 ! matric head of water in the soil (m)
  integer(i4b)    :: mLayerdTheta_dPsi               = 164 ! derivative in the soil water characteristic (m-1)
  integer(i4b)    :: mLayerdPsi_dTheta               = 165 ! derivative in the soil water characteristic (m)
  integer(i4b)    :: mLayerThetaResid                = 166 ! residual volumetric water content in each snow layer (-)
  integer(i4b)    :: mLayerPoreSpace                 = 167 ! total pore space in each snow layer (-)
  integer(i4b)    :: mLayerTranspireLim              = 168 ! soil moist & veg limit on transpiration for each layer (-)
  integer(i4b)    :: mLayerInitTranspire             = 169 ! transpiration loss from each soil layer at the start of the step (kg m-2 s-1)
  integer(i4b)    :: mLayerTranspire                 = 170 ! transpiration loss from each soil layer (kg m-2 s-1)
  integer(i4b)    :: mLayerInitQMacropore            = 171 ! liquid flux from micropores to macropores at the start-of-step (m s-1)
  integer(i4b)    :: mLayerQMacropore                = 172 ! liquid flux from micropores to macropores (m s-1)
  integer(i4b)    :: mLayerInitBaseflow              = 173 ! baseflow from each soil layer at the start of the time step (m s-1)
  integer(i4b)    :: mLayerBaseflow                  = 174 ! baseflow from each soil layer (m s-1)
  integer(i4b)    :: mLayerColumnInflow              = 175 ! total inflow to each layer in a given soil column (m3 s-1)
  integer(i4b)    :: mLayerColumnOutflow             = 176 ! total outflow from each layer in a given soil column (m3 s-1)
  ! define variables at the interface of each layer
  integer(i4b)    :: iLayerHeight                    = 177 ! height of the layer interface; top of soil = 0 (m)
  integer(i4b)    :: iLayerThermalC                  = 178 ! thermal conductivity at the interface of each layer (W m-1 K-1)
  integer(i4b)    :: iLayerConductiveFlux            = 179 ! conductive energy flux at layer interfaces at end of time step (W m-2)
  integer(i4b)    :: iLayerAdvectiveFlux             = 180 ! advective energy flux at layer interfaces at end of time step (W m-2)
  integer(i4b)    :: iLayerInitNrgFlux               = 181 ! energy flux at layer interfaces at the start of the time step (W m-2) 
  integer(i4b)    :: iLayerNrgFlux                   = 182 ! energy flux at layer interfaces at the end of the time step (W m-2) 
  integer(i4b)    :: iLayerSatHydCond                = 183 ! saturated hydraulic conductivity at each layer interface (m s-1)
  integer(i4b)    :: iLayerInitLiqFluxSnow           = 184 ! liquid flux at snow layer interfaces at the start of the time step (m s-1) 
  integer(i4b)    :: iLayerInitLiqFluxSoil           = 185 ! liquid flux at soil layer interfaces at the start of the time step (m s-1) 
  integer(i4b)    :: iLayerInitFluxReversal          = 186 ! start of step liquid flux at soil layer interfaces from impedance (m s-1)
  integer(i4b)    :: iLayerLiqFluxSnow               = 187 ! liquid flux at snow layer interfaces at the end of the time step (m s-1)
  integer(i4b)    :: iLayerLiqFluxSoil               = 188 ! liquid flux at soil layer interfaces at the end of the time step (m s-1) 
  integer(i4b)    :: iLayerFluxReversal              = 189 ! end of step liquid flux at soil layer interfaces from impedance (m s-1)
  ! define derived variables
  integer(i4b)    :: scalarVGn_m                     = 190 ! van Genuchten "m" parameter (-)
  integer(i4b)    :: scalarKappa                     = 191 ! constant in the freezing curve function (m K-1)
  integer(i4b)    :: scalarVolHtCap_air              = 192 ! volumetric heat capacity air         (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_ice              = 193 ! volumetric heat capacity ice         (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_soil             = 194 ! volumetric heat capacity dry soil    (J m-3 K-1)
  integer(i4b)    :: scalarVolHtCap_water            = 195 ! volumetric heat capacity liquid wat  (J m-3 K-1)
  integer(i4b)    :: scalarLambda_drysoil            = 196 ! thermal conductivity of dry soil     (W m-1)
  integer(i4b)    :: scalarLambda_wetsoil            = 197 ! thermal conductivity of wet soil     (W m-1)
  integer(i4b)    :: scalarVolLatHt_fus              = 198 ! volumetric latent heat of fusion     (J m-3)
  integer(i4b)    :: scalarAquiferRootFrac           = 199 ! fraction of roots below the soil profile (-)
 endtype iLook_mvar
 ...
 integer(i4b),parameter,public :: maxvarMpar     = 121
 integer(i4b),parameter,public :: maxvarMvar     = 199
