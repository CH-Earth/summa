...
MODULE var_lookup
 ...
 type, public  ::  iLook_decision
  ...
  integer(i4b)    :: snowDenNew       = 39 ! choice of method for new snow density
 ...
 type, public  ::  iLook_param
  ...
  ! new snow density
  integer(i4b)    :: newSnowDenMin             = 25  ! Hedstrom and Pomeroy 1998, minimum new snow density (kg m-3)
  integer(i4b)    :: newSnowDenMult            = 26  ! Hedstrom and Pomeroy 1998, multiplier for new snow density (kg m-3)
  integer(i4b)    :: newSnowDenScal            = 27  ! Hedstrom and Pomeroy 1998, scaling factor for new snow density (K)
  integer(i4b)    :: constSnowDen              = 28  ! constDens, Constant new snow density (kg m-3)
  integer(i4b)    :: newSnowDenAdd             = 29  ! Pahaut 1976, additive factor for new snow density (kg m-3)  
  integer(i4b)    :: newSnowDenMultTemp        = 30  ! Pahaut 1976, multiplier for new snow density applied to air temperature (kg m-3 K-1)
  integer(i4b)    :: newSnowDenMultWind        = 31  ! Pahaut 1976, multiplier for new snow density applied to wind speed (kg m-7/2 s-1/2)
  integer(i4b)    :: newSnowDenMultAnd         = 32  ! Anderson 1976, multiplier for new snow density for Anderson function (K-1)
  integer(i4b)    :: newSnowDenBase            = 33  ! Anderson 1976, base value that is rasied to the (3/2) power (K)
  ! snow compaction
  integer(i4b)    :: densScalGrowth            = 34  ! density scaling factor for grain growth (kg-1 m3)
  integer(i4b)    :: tempScalGrowth            = 35  ! temperature scaling factor for grain growth (K-1)
  ...
  integer(i4b)    :: mw_exp                    = 42  ! exponent for meltwater flow (-)
  ! turbulent heat fluxes
  integer(i4b)    :: z0Snow                    = 43  ! roughness length of snow (m)
  integer(i4b)    :: z0Soil                    = 44  ! roughness length of bare soil below the canopy (m)
  integer(i4b)    :: z0Canopy                  = 45  ! roughness length of the canopy (m)
  integer(i4b)    :: zpdFraction               = 46  ! zero plane displacement / canopy height (-)
  integer(i4b)    :: critRichNumber            = 47  ! critical value for the bulk Richardson number (-)
  integer(i4b)    :: Louis79_bparam            = 48  ! parameter in Louis (1979) stability function (-)
  integer(i4b)    :: Louis79_cStar             = 49  ! parameter in Louis (1979) stability function (-)
  integer(i4b)    :: Mahrt87_eScale            = 50  ! exponential scaling factor in the Mahrt (1987) stability function (-)
  integer(i4b)    :: leafExchangeCoeff         = 51  ! turbulent exchange coeff between canopy surface and canopy air ( m s-(1/2) )
  integer(i4b)    :: windReductionParam        = 52  ! canopy wind reduction parameter (-)
  ! stomatal conductance
  integer(i4b)    :: Kc25                      = 53  ! Michaelis-Menten constant for CO2 at 25 degrees C (umol mol-1)
  integer(i4b)    :: Ko25                      = 54  ! Michaelis-Menten constant for O2 at 25 degrees C (mol mol-1)
  integer(i4b)    :: Kc_qFac                   = 55  ! factor in the q10 function defining temperature controls on Kc (-)
  integer(i4b)    :: Ko_qFac                   = 56  ! factor in the q10 function defining temperature controls on Ko (-)
  integer(i4b)    :: kc_Ha                     = 57  ! activation energy for the Michaelis-Menten constant for CO2 (J mol-1)
  integer(i4b)    :: ko_Ha                     = 58  ! activation energy for the Michaelis-Menten constant for O2 (J mol-1)
  integer(i4b)    :: vcmax25_canopyTop         = 59  ! potential carboxylation rate at 25 degrees C at the canopy top (umol co2 m-2 s-1)
  integer(i4b)    :: vcmax_qFac                = 60  ! factor in the q10 function defining temperature controls on vcmax (-)
  integer(i4b)    :: vcmax_Ha                  = 61  ! activation energy in the vcmax function (J mol-1)
  integer(i4b)    :: vcmax_Hd                  = 62  ! deactivation energy in the vcmax function (J mol-1)
  integer(i4b)    :: vcmax_Sv                  = 63  ! entropy term in the vcmax function (J mol-1 K-1)
  integer(i4b)    :: vcmax_Kn                  = 64  ! foliage nitrogen decay coefficient (-)
  integer(i4b)    :: jmax25_scale              = 65  ! scaling factor to relate jmax25 to vcmax25 (-)
  integer(i4b)    :: jmax_Ha                   = 67  ! activation energy in the jmax function (J mol-1)
  integer(i4b)    :: jmax_Hd                   = 68  ! deactivation energy in the jmax function (J mol-1)
  integer(i4b)    :: jmax_Sv                   = 69  ! entropy term in the jmax function (J mol-1 K-1)
  integer(i4b)    :: fractionJ                 = 70  ! fraction of light lost by other than the chloroplast lamellae (-)
  integer(i4b)    :: quantamYield              = 71  ! quantam yield (mol e mol-1 q)
  integer(i4b)    :: vpScaleFactor             = 72  ! vapor pressure scaling factor in stomatal conductance function (Pa)
  integer(i4b)    :: cond2photo_slope          = 73  ! slope of conductance-photosynthesis relationship (-)
  integer(i4b)    :: minStomatalConductance    = 74  ! minimum stomatal conductance (umol H2O m-2 s-1)
  ! vegetation properties
  integer(i4b)    :: winterSAI                 = 75  ! stem area index prior to the start of the growing season (m2 m-2)
  integer(i4b)    :: summerLAI                 = 76  ! maximum leaf area index at the peak of the growing season (m2 m-2)
  integer(i4b)    :: rootScaleFactor1          = 77  ! 1st scaling factor (a) in Y = 1 - 0.5*( exp(-aZ) + exp(-bZ) )  (m-1)
  integer(i4b)    :: rootScaleFactor2          = 78  ! 2nd scaling factor (b) in Y = 1 - 0.5*( exp(-aZ) + exp(-bZ) )  (m-1)
  integer(i4b)    :: rootingDepth              = 79  ! rooting depth (m)
  integer(i4b)    :: rootDistExp               = 80  ! exponent controlling the vertical distribution of root density (-)
  integer(i4b)    :: plantWiltPsi              = 81  ! matric head at wilting point (m)
  integer(i4b)    :: soilStressParam           = 82  ! parameter in the exponential soil stress function
  integer(i4b)    :: critSoilWilting           = 83  ! critical vol. liq. water content when plants are wilting (-)
  integer(i4b)    :: critSoilTranspire         = 84  ! critical vol. liq. water content when transpiration is limited (-)
  integer(i4b)    :: critAquiferTranspire      = 85  ! critical aquifer storage value when transpiration is limited (m)
  integer(i4b)    :: minStomatalResistance     = 86  ! minimum canopy resistance (s m-1)
  integer(i4b)    :: leafDimension             = 87  ! characteristic leaf dimension (m)
  integer(i4b)    :: heightCanopyTop           = 88  ! height of top of the vegetation canopy above ground surface (m)
  integer(i4b)    :: heightCanopyBottom        = 89  ! height of bottom of the vegetation canopy above ground surface (m)
  integer(i4b)    :: specificHeatVeg           = 90  ! specific heat of vegetation (J kg-1 K-1)
  integer(i4b)    :: maxMassVegetation         = 91  ! maximum mass of vegetation (full foliage) (kg m-2)
  integer(i4b)    :: throughfallScaleSnow      = 92  ! scaling factor for throughfall (snow) (-)
  integer(i4b)    :: throughfallScaleRain      = 93  ! scaling factor for throughfall (rain) (-)
  integer(i4b)    :: refInterceptCapSnow       = 94  ! reference canopy interception capacity per unit leaf area (snow) (kg m-2)
  integer(i4b)    :: refInterceptCapRain       = 95  ! canopy interception capacity per unit leaf area (rain) (kg m-2)
  integer(i4b)    :: snowUnloadingCoeff        = 96  ! time constant for unloading of snow from the forest canopy (s-1)
  integer(i4b)    :: canopyDrainageCoeff       = 97  ! time constant for drainage of liquid water from the forest canopy (s-1)
  integer(i4b)    :: ratioDrip2Unloading       = 98  ! ratio of canopy drip to unloading of snow from the forest canopy (-)
  integer(i4b)    :: canopyWettingFactor       = 99  ! maximum wetted fraction of the canopy (-)
  integer(i4b)    :: canopyWettingExp          = 100 ! exponent in canopy wetting function (-)
  ! soil properties
  integer(i4b)    :: soil_dens_intr            = 101 ! intrinsic soil density (kg m-3)
  integer(i4b)    :: thCond_soil               = 102 ! thermal conductivity of soil (W m-1 K-1)
  integer(i4b)    :: frac_sand                 = 103 ! fraction of sand (-)
  integer(i4b)    :: frac_silt                 = 104 ! fraction of silt (-)
  integer(i4b)    :: frac_clay                 = 105 ! fraction of clay (-)
  integer(i4b)    :: fieldCapacity             = 106 ! field capacity (-)
  integer(i4b)    :: wettingFrontSuction       = 107 ! Green-Ampt wetting front suction (m)
  integer(i4b)    :: theta_mp                  = 108 ! volumetric liquid water content when macropore flow begins (-)
  integer(i4b)    :: theta_sat                 = 109 ! porosity (-)
  integer(i4b)    :: theta_res                 = 110 ! volumetric residual water content (-)
  integer(i4b)    :: vGn_alpha                 = 111 ! van Genuchten "alpha" parameter (m-1)
  integer(i4b)    :: vGn_n                     = 112 ! van Genuchten "n" parameter (-)
  integer(i4b)    :: mpExp                     = 113 ! empirical exponent in macropore flow equation (-)
  integer(i4b)    :: k_soil                    = 114 ! hydraulic conductivity of soil (m s-1)
  integer(i4b)    :: k_macropore               = 115 ! saturated hydraulic conductivity for macropores (m s-1)
  integer(i4b)    :: kAnisotropic              = 116 ! anisotropy factor for lateral hydraulic conductivity (-)
  integer(i4b)    :: zScale_TOPMODEL           = 117 ! TOPMODEL scaling factor used in lower boundary condition for soil (m)
  integer(i4b)    :: compactedDepth            = 118 ! depth where k_soil reaches the compacted value given by CH78 (m)
  integer(i4b)    :: aquiferScaleFactor        = 119 ! scaling factor for aquifer storage in the big bucket (m)
  integer(i4b)    :: aquiferBaseflowExp        = 120 ! baseflow exponent (-)
  integer(i4b)    :: qSurfScale                = 121 ! scaling factor in the surface runoff parameterization (-)
  integer(i4b)    :: specificYield             = 122 ! specific yield (-)
  integer(i4b)    :: specificStorage           = 123 ! specific storage coefficient (m-1)
  integer(i4b)    :: f_impede                  = 124 ! ice impedence factor (-)
  integer(i4b)    :: soilIceScale              = 125 ! scaling factor for depth of soil ice, used to get frozen fraction (m)
  integer(i4b)    :: soilIceCV                 = 126 ! CV of depth of soil ice, used to get frozen fraction (-)
  ! algorithmic control parameters
  integer(i4b)    :: minwind                   = 127 ! minimum wind speed (m s-1)
  integer(i4b)    :: minstep                   = 128 ! minimum length of the time step
  integer(i4b)    :: maxstep                   = 129 ! maximum length of the time step
  integer(i4b)    :: wimplicit                 = 130 ! weight assigned to the start-of-step fluxes
  integer(i4b)    :: maxiter                   = 131 ! maximum number of iteration
  integer(i4b)    :: relConvTol_liquid         = 132 ! relative convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: absConvTol_liquid         = 133 ! absolute convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: relConvTol_matric         = 134 ! relative convergence tolerance for matric head (-)
  integer(i4b)    :: absConvTol_matric         = 135 ! absolute convergence tolerance for matric head (m)
  integer(i4b)    :: relConvTol_energy         = 136 ! relative convergence tolerance for energy (-)
  integer(i4b)    :: absConvTol_energy         = 137 ! absolute convergence tolerance for energy (J m-3)
  integer(i4b)    :: relConvTol_aquifr         = 138 ! relative convergence tolerance for aquifer storage (-)
  integer(i4b)    :: absConvTol_aquifr         = 139 ! absolute convergence tolerance for aquifer storage (J m-3)
  integer(i4b)    :: zmin                      = 140 ! minimum layer depth (m)
  integer(i4b)    :: zmax                      = 141 ! maximum layer depth (m)
  integer(i4b)    :: zminLayer1                = 142 ! minimum layer depth for the 1st (top) layer (m)
  integer(i4b)    :: zminLayer2                = 143 ! minimum layer depth for the 2nd layer (m)
  integer(i4b)    :: zminLayer3                = 144 ! minimum layer depth for the 3rd layer (m)
  integer(i4b)    :: zminLayer4                = 145 ! minimum layer depth for the 4th layer (m)
  integer(i4b)    :: zminLayer5                = 146 ! minimum layer depth for the 5th (bottom) layer (m)
  integer(i4b)    :: zmaxLayer1_lower          = 147 ! maximum layer depth for the 1st (top) layer when only 1 layer (m)
  integer(i4b)    :: zmaxLayer2_lower          = 148 ! maximum layer depth for the 2nd layer when only 2 layers (m)
  integer(i4b)    :: zmaxLayer3_lower          = 149 ! maximum layer depth for the 3rd layer when only 3 layers (m)
  integer(i4b)    :: zmaxLayer4_lower          = 150 ! maximum layer depth for the 4th layer when only 4 layers (m)
  integer(i4b)    :: zmaxLayer1_upper          = 151 ! maximum layer depth for the 1st (top) layer when > 1 layer (m)
  integer(i4b)    :: zmaxLayer2_upper          = 152 ! maximum layer depth for the 2nd layer when > 2 layers (m)
  integer(i4b)    :: zmaxLayer3_upper          = 153 ! maximum layer depth for the 3rd layer when > 3 layers (m)
  integer(i4b)    :: zmaxLayer4_upper          = 154 ! maximum layer depth for the 4th layer when > 4 layers (m)
 endtype ilook_param
 ! ***********************************************************************************************************
 ...
 integer(i4b),parameter,public :: maxvarDecisions= 39
 ...
 integer(i4b),parameter,public :: maxvarMpar     = 154
