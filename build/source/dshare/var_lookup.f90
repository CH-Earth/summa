...
MODULE var_lookup
 ...
 type, public  ::  iLook_param
  ...
  integer(i4b)    :: winterSAI                 = 53  ! stem area index prior to the start of the growing season (m2 m-2)
  integer(i4b)    :: summerLAI                 = 54  ! maximum leaf area index at the peak of the growing season (m2 m-2)
  integer(i4b)    :: rootingDepth              = 55  ! rooting depth (m)
  integer(i4b)    :: rootDistExp               = 56  ! exponent controlling the vertical distribution of root density (-)
  integer(i4b)    :: plantWiltPsi              = 57  ! matric head at wilting point (m)
  integer(i4b)    :: soilStressParam           = 58  ! parameter in the exponential soil stress function
  integer(i4b)    :: critSoilWilting           = 59  ! critical vol. liq. water content when plants are wilting (-)
  integer(i4b)    :: critSoilTranspire         = 60  ! critical vol. liq. water content when transpiration is limited (-)
  integer(i4b)    :: critAquiferTranspire      = 61  ! critical aquifer storage value when transpiration is limited (m)
  integer(i4b)    :: minStomatalResistance     = 62  ! minimum canopy resistance (s m-1)
  integer(i4b)    :: leafDimension             = 63  ! characteristic leaf dimension (m)
  integer(i4b)    :: heightCanopyTop           = 64  ! height of top of the vegetation canopy above ground surface (m)
  integer(i4b)    :: heightCanopyBottom        = 65  ! height of bottom of the vegetation canopy above ground surface (m)
  integer(i4b)    :: specificHeatVeg           = 66  ! specific heat of vegetation (J kg-1 K-1)
  integer(i4b)    :: maxMassVegetation         = 67  ! maximum mass of vegetation (full foliage) (kg m-2)
  integer(i4b)    :: throughfallScaleSnow      = 68  ! scaling factor for throughfall (snow) (-)
  integer(i4b)    :: throughfallScaleRain      = 69  ! scaling factor for throughfall (rain) (-)
  integer(i4b)    :: refInterceptCapSnow       = 70  ! reference canopy interception capacity per unit leaf area (snow) (kg m-2)
  integer(i4b)    :: refInterceptCapRain       = 71  ! canopy interception capacity per unit leaf area (rain) (kg m-2)
  integer(i4b)    :: snowUnloadingCoeff        = 72  ! time constant for unloading of snow from the forest canopy (s-1)
  integer(i4b)    :: canopyDrainageCoeff       = 73  ! time constant for drainage of liquid water from the forest canopy (s-1)
  integer(i4b)    :: ratioDrip2Unloading       = 74  ! ratio of canopy drip to unloading of snow from the forest canopy (-)
  ! soil properties
  integer(i4b)    :: soil_dens_intr            = 75  ! intrinsic soil density (kg m-3)
  integer(i4b)    :: thCond_soil               = 76  ! thermal conductivity of soil (W m-1 K-1)
  integer(i4b)    :: frac_sand                 = 77  ! fraction of sand (-)
  integer(i4b)    :: frac_silt                 = 78  ! fraction of silt (-)
  integer(i4b)    :: frac_clay                 = 79  ! fraction of clay (-)
  integer(i4b)    :: fieldCapacity             = 80  ! field capacity (-)
  integer(i4b)    :: wettingFrontSuction       = 81  ! Green-Ampt wetting front suction (m)
  integer(i4b)    :: theta_mp                  = 82  ! volumetric liquid water content when macropore flow begins (-)
  integer(i4b)    :: theta_sat                 = 83  ! porosity (-)
  integer(i4b)    :: theta_res                 = 84  ! volumetric residual water content (-)
  integer(i4b)    :: vGn_alpha                 = 85  ! van Genuchten "alpha" parameter (m-1)
  integer(i4b)    :: vGn_n                     = 86  ! van Genuchten "n" parameter (-)
  integer(i4b)    :: mpExp                     = 87  ! empirical exponent in macropore flow equation (-)
  integer(i4b)    :: k_soil                    = 88  ! hydraulic conductivity of soil (m s-1)
  integer(i4b)    :: k_macropore               = 89  ! saturated hydraulic conductivity for macropores (m s-1)
  integer(i4b)    :: kAnisotropic              = 90  ! anisotropy factor for lateral hydraulic conductivity (-)
  integer(i4b)    :: zScale_TOPMODEL           = 91  ! TOPMODEL scaling factor used in lower boundary condition for soil (m)
  integer(i4b)    :: compactedDepth            = 92  ! depth where k_soil reaches the compacted value given by CH78 (m)
  integer(i4b)    :: aquiferScaleFactor        = 93  ! scaling factor for aquifer storage in the big bucket (m)
  integer(i4b)    :: aquiferBaseflowExp        = 94  ! baseflow exponent (-)
  integer(i4b)    :: qSurfScale                = 95  ! scaling factor in the surface runoff parameterization (-)
  integer(i4b)    :: specificYield             = 96  ! specific yield (-)
  integer(i4b)    :: specificStorage           = 97  ! specific storage coefficient (m-1)
  integer(i4b)    :: f_impede                  = 98  ! ice impedence factor (-)
  integer(i4b)    :: soilIceScale              = 99  ! scaling factor for depth of soil ice, used to get frozen fraction (m)
  integer(i4b)    :: soilIceCV                 = 100  ! CV of depth of soil ice, used to get frozen fraction (-)
  ! algorithmic control parameters
  integer(i4b)    :: minwind                   = 101  ! minimum wind speed (m s-1)
  integer(i4b)    :: minstep                   = 102  ! minimum length of the time step
  integer(i4b)    :: maxstep                   = 103  ! maximum length of the time step
  integer(i4b)    :: wimplicit                 = 104  ! weight assigned to the start-of-step fluxes
  integer(i4b)    :: maxiter                   = 105  ! maximum number of iteration
  integer(i4b)    :: relConvTol_liquid         = 106 ! relative convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: absConvTol_liquid         = 107 ! absolute convergence tolerance for vol frac liq water (-)
  integer(i4b)    :: relConvTol_matric         = 108 ! relative convergence tolerance for matric head (-)
  integer(i4b)    :: absConvTol_matric         = 109 ! absolute convergence tolerance for matric head (m)
  integer(i4b)    :: relConvTol_energy         = 110 ! relative convergence tolerance for energy (-)
  integer(i4b)    :: absConvTol_energy         = 111 ! absolute convergence tolerance for energy (J m-3)
  integer(i4b)    :: relConvTol_aquifr         = 112 ! relative convergence tolerance for aquifer storage (-)
  integer(i4b)    :: absConvTol_aquifr         = 113 ! absolute convergence tolerance for aquifer storage (J m-3)
  integer(i4b)    :: zmin                      = 114 ! minimum layer depth (m)
  integer(i4b)    :: zmax                      = 115 ! maximum layer depth (m)
  integer(i4b)    :: zminLayer1                = 116 ! minimum layer depth for the 1st (top) layer (m)
  integer(i4b)    :: zminLayer2                = 117 ! minimum layer depth for the 2nd layer (m)
  integer(i4b)    :: zminLayer3                = 118 ! minimum layer depth for the 3rd layer (m)
  integer(i4b)    :: zminLayer4                = 119 ! minimum layer depth for the 4th layer (m)
  integer(i4b)    :: zminLayer5                = 120 ! minimum layer depth for the 5th (bottom) layer (m)
  integer(i4b)    :: zmaxLayer1_lower          = 121 ! maximum layer depth for the 1st (top) layer when only 1 layer (m)
  integer(i4b)    :: zmaxLayer2_lower          = 122 ! maximum layer depth for the 2nd layer when only 2 layers (m)
  integer(i4b)    :: zmaxLayer3_lower          = 123 ! maximum layer depth for the 3rd layer when only 3 layers (m)
  integer(i4b)    :: zmaxLayer4_lower          = 124 ! maximum layer depth for the 4th layer when only 4 layers (m)
  integer(i4b)    :: zmaxLayer1_upper          = 125 ! maximum layer depth for the 1st (top) layer when > 1 layer (m)
  integer(i4b)    :: zmaxLayer2_upper          = 126 ! maximum layer depth for the 2nd layer when > 2 layers (m)
  integer(i4b)    :: zmaxLayer3_upper          = 127 ! maximum layer depth for the 3rd layer when > 3 layers (m)
  integer(i4b)    :: zmaxLayer4_upper          = 128 ! maximum layer depth for the 4th layer when > 4 layers (m)
 endtype ilook_param
 ! ***********************************************************************************************************
 ...
 integer(i4b),parameter,public :: maxvarMpar     = 128
