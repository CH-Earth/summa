...
contains
 ...
 ! ******************************************************************************************************
 subroutine allocStat(metaStruct,statStruct,err,message)
 USE nrtype
 ! data structures
 ...
 ! dummies
 type(var_info),intent(in)  :: metaStruct(:)  ! meta structure
 class(*)      ,intent(out) :: statStruct     ! stats structure 
 ...
 do iGRU = 1,size(gru_struc)
  ...
  select type(statStruct)   
   type is (gru_hru_doubleVec)
    ...
    do iHRU = 1,gru_struc(iGRU)%hruCount
     ...
     do iVar = 1,size(metaStruct)
      allocate(statStruct%gru(iGRU)%hru(iHRU)%var(iVar)%dat(maxvarStat),stat=err)
   ...
   type is (gru_doubleVec)
    ...
    ! (3) allocate the variable level structure
    allocate(statStruct%gru(iGRU)%var(size(metaStruct))     ,stat=err)
    if (err.ne.0) then; message=trim(message)//'VAR allocate error (no HRU)'; return; endif;
    ! (4) allocate the data (statistics) level structure
    do iVar = 1,size(metaStruct)
     allocate(statStruct%gru(iGRU)%var(iVar)%dat(maxvarStat),stat=err)
...
! ! ******************************************************************************************************
! subroutine compileStats(statStruc,dataStruc,metaStruc,iStep,err,message)
! USE nrtype
! USE data_types,only:var_info,ilength,dlength       ! metadata structure type
! USE data_types, only:nFreq                         ! output frequencies
! implicit none
!
! ! dummy variables
! type(dlength) ,intent(out)     :: statStruc(:)     ! statistics
! type(dlength) ,intent(in)      :: dataStruc(:)     ! statistics
! type(var_info),intent(in)      :: metaStruc(:)     ! metadata
...
!
!
! do iFreq = 1,nFreq ! loop through output frequencies 
!  do iVar = 1,size(metaStruc)  ! model variables
!   if (metaStruc(iVar)%varType.eq.iLookVarType%scalarv) then
!    call calc_stats(metaStruc(iVar),statStruc%var(iVar),dataStruc%var(iVar)%dat(1),iFreq,iStep,err,cmessage)  
