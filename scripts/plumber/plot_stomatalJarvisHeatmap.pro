pro plot_stomatalJarvisHeatmap

; define plotting parameters
window, 0, xs=700, ys=600, retain=2
device, decomposed=0
LOADCT, 39
!P.BACKGROUND=255
!P.CHARSIZE=1.5
...
simPairs = [ ['011', '012']  ]      ; Ball-Berry and Jarvis

; define x title
xtitleVec = ['Stomatal conductance (m s!e-1!n)!C' + '[Ball-Berry]']

; define y title
ytitleVec = ['Stomatal conductance (m s!e-1!n)!C' + '[Jarvis]']

; get size
iSize = size(simPairs, /dimensions)
if(n_elements(iSize) gt 1)then nCompare = iSize[1] else nCompare=1
; define the heat map
dx       = 0.2
dy       = 0.2
nx       = 100
...
for iCompare=0,nCompare-1 do begin
 ...
 heatMapSaveFile = 'xIDLsave/stomatalJarvisHeatmap_'+models[0]+'_'+models[1]+'.sav'
...
colorbar, 0.87, 0.90, ypos1, ypos2, xLabels, xColors, every=15, charsize=1.5, /nobox, /norm
xyouts, 0.985, 0.5*(ypos1 + ypos2), 'log!i10!n count', alignment=0.5, orientation=90, charsize=3, /normal

; write figure
write_png, 'figures/stomatalJarvis.png', tvrd(true=1)
